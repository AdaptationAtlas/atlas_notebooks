...
//   `);
// 
//   //HACK: We need to cache and reuse the old selected rows, as the inputs compare
//   //JS objects not the values and the db query returns new objects each time.
//   // Otherwise, the multiselect will clear values when a new country is selected
//   function stable(obj) {
//     const key = `${obj.iso3}|${obj.admin1_name ?? "FULL"}`;
//     if (!admin1Cache.has(key)) admin1Cache.set(key, obj);
//     return admin1Cache.get(key);
//   }
// 
//   const fullCountryRows = admin0Select.map((d) =>
//     stable({
...
//   );
// 
//   return [...fullCountryRows, ...regions.map(stable)];
// };
...

{
  ...
  if (a1NoMatch) {
    ...
    console.log("Cleared admin1 selection");
    return;
  }
