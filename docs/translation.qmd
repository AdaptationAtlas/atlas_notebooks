...
- `<id>.en.md`, `<id>.fr.md` — one file per narrative **block**. The filename is the
  block id; the file is a deliberately narrow front matter (a single `title:` line)
  ...
  Blocks whose id matches a section anchor in the `.qmd` (e.g. `{#overview}`) provide
  that section's heading text; other blocks are free-floating chunks the notebook
  places wherever — for example an `indicator-definitions` block rendered inside a
  dropdown, or an `overview-closing` block placed after a figure.

  **Developers control heading level and placement in the `.qmd`; authors control the
  displayed text in the block file.** The heading text written in the `.qmd` is a
  structural fallback only — `scripts/build/cmsContent.lua` replaces it from the block's
  `title:` in every build, including English. The same filter fills
  `::: {.nb-prose data-section="overview"}` markers with the block body at render
  time, so default-language prose is static HTML — crawlable and visible before OJS
  boots. A `data-section` pointing at a missing or empty block **fails the render**.
  At runtime, `Lang.applyTranslations()` swaps headings and `.nb-prose` blocks when
  the language toggle changes (blocks are parsed with `Lang.parseBlock`), and restores
  the built English on toggle back.
...
titles, and every block referenced by a `.qmd` actually existing.

The narrative blocks are editable in the browser at `/admin/` (Sveltia CMS, configured
in `admin/config.yml` — one folder collection per notebook; authors can edit but not
create or delete blocks). Edits are committed to the repo and published by the normal
build. The JSON strings are deliberately *not* in the CMS — their keys are coupled to
notebook code, so they are edited in the repo through review. See
`notebooks/roi/notebook.qmd` for the reference implementation.
