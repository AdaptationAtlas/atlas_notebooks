...
- `<id>.en.md`, `<id>.fr.md` — one file per narrative **block**. The filename is the
  ...
  **Developers control heading level and placement in the `.qmd`; authors control the
  ...
  example an `overview-closing` block placed after a figure. Blocks can also be
  consumed directly in OJS code via `sections["indicator-definitions"]` (e.g.
  rendered inside a dropdown). A shortcode pointing at a missing or empty block
  **fails the render**, and CI flags block files that nothing references (a typo'd
  id would otherwise fail silently). Default-language prose is static HTML —
