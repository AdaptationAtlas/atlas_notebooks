...
Use `atlasMultiSelect()` for a normal multiple selector. `commit: "change"`
updates dependent cells after every selection; `commit: "close"` waits until
the menu closes and is preferable when each update triggers an expensive
query.
