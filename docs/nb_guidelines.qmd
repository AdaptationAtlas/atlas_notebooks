...
## Table of Contents
Standard notebooks receive their table of contents from the shared header. H1
and H2 headings are always included, so no notebook-specific cell or manifest
option is needed.

The canonical table of contents is the `<atlas-toc>` web component. Static or
exceptional pages can use it directly:
...
Defining IDs for each header makes it possible to specify which ones to ignore.
When multiple levels are selected, the least-indented headings remain visible
and their child headings appear only while that parent section is active.
Ignoring or hiding a parent section also removes its descendant headings.
