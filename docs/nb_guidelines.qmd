...
`overview` and `methods`, and `summary` is optional.
Editors can update its page-level copy, hero image and contributor lists under
**Notebook Settings** in the CMS. Use the shared **Contributor Directory** for
regular collaborators and the custom option for a one-off contributor;
structural fields remain hidden.

The narrative should include:

1. **Overview** — required introductory context and purpose.
2. **Narrative sections** — the evidence, figures, and interpretation that tell
   the notebook's story.
3. **Summary** — optional closing takeaways or next steps.
4. **Methods and sources** — required explanation of the data, analysis, and
   limitations.

Notebook entry files use the shared notebook header, runtime, and footer.
These provide the hero, language wiring, contributors, appendix, and
source-code link; the table of contents comes from Quarto itself. Notebook-specific charts and narrative remain
in the entry file.
...
both languages as `.nb-i18n` spans (Quarto renders it as markdown), so the same CSS
that shows the active language everywhere else covers the TOC too — no JavaScript.
...
Use the shared `atlasTable()` helper for searchable and static data tables.
It applies the Atlas table styling without injecting per-instance CSS:
...
the menu closes, and only if the selection changed while it was open — never per
click. That is deliberate and not configurable: a selection of several units
...
silently selecting only the first items. **Deselect All** remains available
when empty selections are allowed.
...
The `/helpers` folder contains code and modules that are used across the notebooks. This includes the `lang` module for the translations, style builders for elements such as the hero image, and functions to handle other tasks. This folder also includes a post-render script which is automatically run to minify the CSS and JS files.
