...
## Table of Contents
Standard notebooks receive their table of contents from the shared header. Set
`toc.selector` in the notebook manifest when H2 or H3 headings should also be
shown; no notebook-specific TOC cell is needed.
...
For translated or otherwise dynamic notebook headings, the existing
`atlasTOC()` helper remains available as a compatibility wrapper around the
same component:
...
The helper defaults to H1 headings only; include H2 or H3 in `selector` when
readers benefit from more detail.
