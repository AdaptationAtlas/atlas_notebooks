...
```{=html}
<script>
window.atlasTocConfig = { headingLevels: [1, 2] };
</script>
```
...
const LANGUAGES = [
  ...
  { code: "FR", label: "Français", placeholder: "Rechercher dans le glossaire..." },
];
const state = {
  currentLang: "EN",
  searchQuery: "",
...
function renderGlossary() {
  ...
  state.sections.forEach(([section, entries]) => {
    ...
    filteredEntries.forEach(entry => {
      ...
      html += `<h2 style="color: #555; margin-bottom: 5px; font-size: 1.3em;">${term}${
        acronym ? ` (${acronym})` : ""
      }</h2>`;
      if (tooltip) {
