...
Thanks for considering contributing. Community engagement is one of the Atlas'
core values and a key driver of many project decisions. We believe in the power
of open data and community collaboration to drive meaningful action.

This project is built with a few core principles in mind:

- Accessibility: No complex server setup required. The entire site is static and
  can run in any modern web browser.
- Open-source tooling: The Atlas uses free and open-source tools familiar to
  scientists and researchers.
- Interactivity: We use Observable JavaScript (OJS) to create rich, interactive
  charts, maps, and text that respond to user input.
- Reproducibility: Built with Quarto, our notebooks are reproducible and easy to
  modify.
- Performance: We leverage cloud-optimized data formats (COG, GeoParquet) to
  efficiently handle large datasets directly in the browser.
...
We aim to keep the Atlas easy to contribute to, modify, and extend with new data
stories. Please get in touch to discuss ideas, provide suggestions, or ask
questions.
...
The project has very few dependencies. In most cases, installing Quarto is
enough to get started.

Project dependencies:

- [Quarto](https://quarto.org/docs/get-started/)
- An IDE or text editor (VSCode, RStudio, Neovim, Positron, etc.)
- A web browser for previewing

To build and preview the project locally:

1. **Install Quarto** Follow the instructions for your operating system at
...
In general, `quarto preview` will be used most often while developing notebooks.
However, rendering the full site is useful for deployment testing (for example,
on GitHub Pages) and for catching edge cases where preview and render paths
differ.

Render the project from the project root. This creates a `_site` directory
containing rendered HTML, JS, and other dependencies that can be deployed as a
static site.
...
- Use `quarto preview` and only render when pushing to GitHub Pages or similar.
...
2. **Create a new branch based on `template`**:
...
Python, or Julia code will not run after render. TypeScript cannot be used
directly inside Quarto notebooks, but it can be used for pre-render and
post-render scripts. Shiny as a back-end is not currently available. If
interested, both [R](https://docs.r-wasm.org/webr/latest/) and
[Python](https://pyodide.org/en/stable/) have been compiled to WebAssembly and
could technically be possible to use; however, this is currently untested for
the Atlas and may result in decreased performance.

The `template` branch provides a clean starting point for new notebooks without
including unrelated code or data. This allows you to work on the project without
needing to render all notebooks and pages.

You can clone just the `template` branch with:
...
- `components/`: Browser-facing UI components (custom elements used directly in
  rendered pages).
- `helpers/`: Reusable notebook/runtime modules shared across notebooks.
- `scripts/`: Build-time and utility scripts (for example, post-render
  optimization, asset processing, data conversion).
- `data/`: Project data assets used by notebooks/docs. Keep only required source
  data in-repo and avoid committing temporary local files.
- `docs/`: Contributor and user-facing project documentation.

Use this structure to decide where new files should live. If adding a new
utility, prefer extending an existing folder before creating a new top-level
directory.
...
- Use [JSDoc](https://jsdoc.app/) for reusable functions, classes, or modules
  shared across notebooks.
- Use concise inline comments to explain:
  - The purpose of a block or group of code (for example,
    `"// Import dependencies"` or `"<!-- code for notebook section 1 -->"`).
...
- Use comments to structure your code into clear sections (e.g. data prep,
  transformation, plotting, sections).

## Style Guidelines

### Global Guidelines

**Line Length**: Lines should be a maximum of 120 characters. **Indents**:
Spaces, not tabs; 2 spaces is generally preferred over 4. **Organization:**

- Keep code in logical groups and use modular approaches where applicable.
- Notebook code is generally collected and documented in a bottom appendix
  section of the notebook.
  - The 'code-appendix' will not be visible to users
- Code above the 'code-appendix' should generally be kept for elements visible
  to the user, and structured in the order it appears on the page. This will
  include the code for headings, plots, dynamic text, etc. but not the helpers
...
**Headings**: Dynamic Headings should use inline code alongside quarto/markdown
headings
...
**Code Blocks**: Quarto allows markdown-style code blocks as well as inline
code.

- Inline code should be used for simple selectors and dynamic headings, such as
  for translations.
- Quarto code blocks can hold multiple OJS "cells". This should be done to group
  multiple similar pieces of OJS code. This could include placing multiple
  imports in a block, defining variables specific to a plot, etc. The code in
  the quarto code blocks should be the smallest logical group to keep things
...
- Consider using a formatter such as Prettier or Biome to automatically handle
  some styling of JavaScript code. However, be careful as it can occasionally
  break the OJS code due to differences in the languages.
- Although not officially supported, the
  [typescript-language-server](https://github.com/typescript-language-server/typescript-language-server)
  can be used to assist with completions, editor hints, refactoring, etc.
...
You may encounter `Cannot find name 'Deno'` warnings in some `.ts` files when
using editors like VS Code. These occur because Quarto manages Deno/TS
dependencies and using an internal version of Deno that your editor cannot
access. This is also why package lock files are not needed or included. These
