...
- **⚠️ HEADLESS MIS-REPRODUCES SECTION-GATED DuckDB-WASM CHARTS — the render verdict is NOT trustworthy for them.** (2026-06-10, CR-119.) Headless chromium — even with COOP/COEP cross-origin isolation + SharedArrayBuffer enabled — left BOTH gated DuckDB sections (Future Projections AND Hazard Exposure) stuck on "Loading data…" forever, yet Hazard Exposure renders fine in Pete's real browser. So a headless "chart never rendered / stuck on Loading…" can be a **false negative** for any section-gated DuckDB-WASM chart. This skill is reliable for **network/gating behaviour** (which fetches fire, when, gated vs not, byte ranges) — trust those. It is **NOT reliable for the render OUTCOME** of a gated DuckDB chart. Mitigations: (1) keep a **control section known to work in production** — if it ALSO hangs headless, distrust the harness and escalate to a real browser (ask the user to scroll + report); (2) diagnose the *data/file* layer with server-side tools that ARE faithful — `parquet_metadata` (row-group stats / prunability), isolated raw `@duckdb/duckdb-wasm` from CDN run in `page.evaluate` (query timing, no notebook context), `aws s3api` (versions/sizes). Don't conclude "the cell never executes" or "the query hangs" from the in-page headless render alone. See memory `feedback_headless-mis-reproduces-duckdb-wasm-sections`.
