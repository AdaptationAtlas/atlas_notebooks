---
nb-title: "Prioritize Livestock Investments"
nb-author:
...
.content-wrapper-adaptation {
  display: flex;
  flex-wrap: wrap; /* Para que los elementos puedan envolverse en varias líneas si no hay suficiente espacio */
  gap: 16px;
}
...
@media (max-width: 600px) {
  ...
  .icons-emissions {
    flex-basis: 50%; /* Tamaño inicial de los iconos */
  }
}
 .color-box {
...
.custom-slider {
  ...
  width: 100%;
  height: 15px;
  background: linear-gradient(to right, ${initial_data[0].color}, ${initial_data[0].color} 50%, ${initial_data[1].color} 50%, ${initial_data[1].color});
  border-radius: 5px;
  outline: none;
  margin: 20px 0;
}
...
.progress::before {
  ...
  background: conic-gradient(#40C70D var(--angle),lightgrey 0);
...
function create_map(colorBlind) {
  ...
  // const countriesFil = filterCountries1(mode, cobined, NaN, countd);

  //crear componentes

  const projection = d4.geoCylindricalStereographic().scale(1000);

  const path = d4.geoPath().projection(projection);

  const root = d4.create("div").style("position", "relative");

  const svg = root.append("figure").attr("id", "svg_contain");

  svg.append(() => map_legend(colorBlind)).attr("transform", "translate(10,0)");

  const svg_map = svg
    .append("svg")
    .attr("viewBox", [150, -250, width, 1150])
    .attr("id", "svg_map");

  const tooltip = root.append("div").attr("class", "cooltip");

  //Agregar Paises al svg
  const countries = svg_map
    .append("g")
    .attr("stroke", "white")
    ...
    .attr("fill", function (d) {
      ...
      if (index === -1) return getColorBlindRGB(colorBlind, "#EFEFEF"); // Si no está en countriesFil, color predeterminado
      const fillScale = d4
        .scaleLinear()
        .domain([0, countriesFiltered.length]) // Rango de índices en countriesFil
        .range(["#EC5A47", "#F4BB21"]); // Rango de colores deseados
      return getColorBlindRGB(colorBlind, fillScale(index));
    })
    // else {
    //   return countriesFil.includes(d.properties.iso_a3) ||
    //     !composite?.adaptation ||
    //     !composite?.mitigation
    //     ? getColorBlind_Input(colorBlind, "purple")
    //     : color1(getTrafficLight(d.properties.iso_a3), colorBlind, mode);
    // }
    // })
    .attr("d", path)
    .on("mouseover", function (d) {
      ...
      node.innerHTML = "";
      if (countriesFiltered.includes(d.properties.iso_a3)) {
        tooltip.style("display", "");
        this.classList.add("hovered");
        d4.select(this).attr("stroke", "black").raise();
        node.appendChild(getTooltipContents(d, colorBlind));
      }
    })
    .on("mousemove", function (d) {
      if (countriesFiltered.includes(d.properties.iso_a3)) {
        ...
        const mouseX = d4.event.pageX;
        const mouseY = d4.event.pageY - rootBounds.top + 20;
        const tooltipWidth = tooltipBounds.width;
        const containerWidth = root.node().offsetWidth;
        const screenWidth = window.innerWidth;
        const tooltipX = mouseX - 20; // posición predeterminada del tooltip

        const isSmallScreen = screenWidth <= 600;

        // Verificar si el cursor está en la mitad derecha o izquierda del contenedor
        const isCursorOnRightHalf = mouseX > containerWidth / 2;

        let tooltipLeft;
        if (isSmallScreen) {
          tooltipLeft = 20;
        } else {
          tooltipLeft = isCursorOnRightHalf
    ...
    .on("mouseout", function () {
      ...
      d4.select(this).attr("stroke", null).lower();
...
```{ojs}
d4 = require("d3@5", "d3-geo@1", "d3-geo-projection@2");
```
