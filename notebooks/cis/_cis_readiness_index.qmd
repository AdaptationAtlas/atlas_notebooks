...
cisData = cisDataRaw.map(parseCisRow);
```
        ...
        ${Plot.plot({
          ...
          marks: [
            Plot.geo(s11MapData.features, {
              ...
              stroke: d => d.properties.isHighlighted ? "#000" : "#fff",
...
downloadButton(
  filteredData.map(d => {
    ...
    return {
      ...
      "Cloud Coverage Class": classifyTercile(d["cloud-coverage_meanannual"], africaTerciles.cloudCover)
    };
  }),
...
{
  if (s13ViewType === "Map") {
    // 3-panel map visualization for forecast skill
    const shortTermMap = Plot.plot({
      width: 400,
      height: 450,
      projection: {type: "mercator", domain: s13MapData},
      color: {
        type: "linear",
        domain: [0, 1],
        scheme: "RdYlGn",
          ...
          Plot.pointer(
            Plot.centroid({
              ...
              channels: {
                ...
                delta: {
                  label: "Δ (Long − Short)",
                  value: (d) => d.properties.delta
                }
              },
              format: {
                region: true,
                delta: (d) => d != null ? d.toFixed(3) : "N/A"
              }
            })
          )
        )
...
viewof s14Threshold = {
  ...
  const ext = d3.extent(filteredData, d => d["cis_readiness_index"]);
  const _min = round(ext[0], 2);
  const _max = round(ext[1], 2);
  return Inputs.range([_min, _max], {
    label: _lang(cis.thresholdLabel),
    value: _min, // show all by default
    step: 0.001
  });
}
```
  ...
  if (s14ViewType === "Map") {
            ...
            Plot.centroid({
              ...
              channels: {
                ...
                class: {
                  label: "Classification",
                  value: (d) => {
                    const c = d.properties.cisReadinessClass;
                    // Map tercile terms to Low/Medium/High
                    if (c === "Weaker") return "Low (Weaker)";
                    if (c === "Moderate") return "Medium (Moderate)";
                    if (c === "Stronger") return "High (Stronger)";
                    return c;
                  }
                },
                weatherStation: {
              ...
              format: {
                region: true,
                cisReadiness: (d) => d != null ? `${d.toFixed(3)} (Relative to SSA)` : "N/A",
                class: true,
                weatherStation: true,
                cloudCover: true,
    ...
    const tableData = thresholdFilteredData.map(d => {
      ...
      return {
        ...
        "Weather Stn (Raw)": d["weather-station_density"]?.toFixed(3) || "N/A",
        "Weather Stn (Class)": classifyTercile(d["weather-station_density"], africaTerciles.weatherStation),
        "Cloud Cover (Raw)": d["cloud-coverage_meanannual"]?.toFixed(3) || "N/A",
        "Cloud Cover (Class)": classifyTercile(d["cloud-coverage_meanannual"], africaTerciles.cloudCover),
        "Precip Agree (Raw)": d["cv-precipitation_agreement"]?.toFixed(1) || "N/A",
        "Precip Agree (Class)": classifyPrecipAgreement(d["cv-precipitation_agreement"]),
