...
```{ojs}
// Import multi-select component (only once for the entire section)
    ...
    return htl.html`<div style="display: flex; gap: 20px;">
      <div style="flex: 1;">
        ...
        ${Plot.plot({
          ...
          projection: "mercator",
          color: {
            type: useRaw ? "linear" : "categorical",
            domain: useRaw ? [0, 1] : ["Weaker", "Moderate", "Stronger"],
            range: useRaw ? d3.interpolateYlGnBu : ["#fee5d9", "#fcae91", "#fb6a4a"],
            legend: true,
            label: useRaw ? "Density (0-1)" : "Classification"
          },
          marks: [
            Plot.geo(s11MapData, {
      ...
      <div style="flex: 1;">
        ...
        ${Plot.plot({
          ...
          color: {
            ...
            range: useRaw ? d3.interpolateYlGnBu : ["#fee5d9", "#fcae91", "#fb6a4a"],
            legend: true,
            label: useRaw ? "Coverage (0-1)" : "Classification"
          },
          marks: [
            Plot.geo(s11MapData, {
...
// Section 1.2 Map/Table
{
  if (s12ViewType === "Map") {
    ...
    return Plot.plot({
      ...
      color: {
        type: useRaw ? "linear" : "categorical",
        domain: useRaw ? [0, 4] : ["No agreement (Very Low)", "Partial agreement (Moderate)", "High agreement (High)", "Full agreement (Very High)"],
        range: useRaw ? d3.interpolateViridis : ["#fee5d9", "#fcae91", "#fb6a4a", "#a50f15"],
        legend: true,
        label: useRaw ? "Agreement (0-4)" : "Classification"
      },
      marks: [
        Plot.geo(s12MapData, {
...
// Section 1.4 Visualization
{
  if (s14ViewType === "Map") {
    ...
    return Plot.plot({
      width: 900,
      height: Math.max(400, heatmapData.length * 20),
      marginLeft: 150,
      marginBottom: 80,
      x: { 
        label: null,
