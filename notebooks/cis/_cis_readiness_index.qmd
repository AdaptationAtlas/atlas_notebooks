...
```{ojs}
// Note: createSyncedAdminSelector is now defined in the main cis_readiness.qmd file
// before the includes to ensure it's available to all sections
...
// Section 1.4 Visualization
{
  if (s14ViewType === "Map") {
    ...
    const cellHeight = 30;
    const height = Math.max(500, geographyCount * cellHeight);
    const width = 900;
    const maxNameLength = Math.max(...sortedCountries.map(name => name.length), 0);
    const marginLeft = Math.max(200, maxNameLength * 8);
    const margin = { top: 40, right: 100, bottom: 60, left: marginLeft };
    
    // Create tooltip
    d3.select(".plotTooltip-s14").remove();
    
    // Add CSS for consistent tooltip styling
    if (!d3.select("style#plotTooltipStyles").node()) {
      d3.select("head").append("style")
        .attr("id", "plotTooltipStyles")
        .text(`
          .plotTooltip {
    ...
    const tooltip = d3
      .select("body")
      .append("div")
      .attr("class", "plotTooltip plotTooltip-s14")
    ...
    const allValues = heatmapData.flatMap(row => 
      indicators.map(metric => row[metric]).filter(v => v != null)
    );
    const colorScale = d3.scaleSequential(d3.interpolateRdYlBu)
      .domain(d3.extent(allValues).reverse()); // Reverse to have red=high, blue=low
    ...
    g.selectAll("rect")
      ...
      .on("mousemove", (event, d) => {
        ...
        let formattedValue = d.value !== null ? d.value.toFixed(3) : "N/A";
        if (d.metric === "Weather Stations") {
          formattedValue = d.value !== null ? d.value.toFixed(2) : "N/A";
        } else if (d.metric === "Precip Agreement") {
          formattedValue = d.value !== null ? (d.value * 100).toFixed(1) + "%" : "N/A";
        } else if (d.metric === "CIS Readiness") {
          formattedValue = d.value !== null ? d.value.toFixed(3) : "N/A";
        }
        ...
        tooltip
          ...
          .style("opacity", 1)
          .html(`${d.country}<br>${d.metric}: ${formattedValue}`);
      })
      .on("mouseout", () => {
    ...
    const legendGradient = legendG.append("defs")
      ...
      .attr("id", "heatmap-gradient-s14")
      .attr("x1", "0%")
      .attr("x2", "100%");
    
    const numStops = 10;
    for (let i = 0; i <= numStops; i++) {
      const value = d3.interpolateNumber(colorScale.domain()[0], colorScale.domain()[1])(i / numStops);
    ...
    legendG.append("rect")
      ...
      .style("fill", "url(#heatmap-gradient-s14)");
