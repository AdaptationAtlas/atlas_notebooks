...
// Query the CIS data with proper type conversion
cisData = await cisDB.query(`
  SELECT 
    *,
    CAST("weather-station_density" AS DOUBLE) as "weather-station_density",
    CAST("cloud-coverage_meanannual" AS DOUBLE) as "cloud-coverage_meanannual",
    CAST("cv-precipitation_agreement" AS DOUBLE) as "cv-precipitation_agreement",
    CAST("short-term_frcst_skill" AS DOUBLE) as "short-term_frcst_skill",
    CAST("seasonal_frcst_skill" AS DOUBLE) as "seasonal_frcst_skill",
    CAST(cis_readiness_index AS DOUBLE) as cis_readiness_index,
    CAST(gaul0_code AS INTEGER) as gaul0_code,
    CAST(gaul1_code AS INTEGER) as gaul1_code,
    CAST(gaul2_code AS INTEGER) as gaul2_code
  FROM cis_data
`)
```
...
viewof selectedAdmin1 = multiSelect({
  ...
  selected: admin1Options.length > 0 ? admin1Options : []
})
```
...
// Section 1.1 Maps
{
  if (s11ViewType === "Map") {
          ...
          marks: [
            Plot.geo(s11MapData, {
              ...
              title: d => `${d.properties.admin0_name || d.properties.admin1_name || d.properties.admin2_name}\n` +
                          `Weather Station: ${d.properties.weatherStation?.toFixed(3) || 'N/A'}\n` +
                          `Class: ${d.properties.weatherStationClass}`
            })
          ]
        })}
      </div>
        ...
        ${Plot.plot({
          ...
          marks: [
            Plot.geo(s11MapData, {
              ...
              strokeWidth: 0.5,
              title: d => `${d.properties.admin0_name || d.properties.admin1_name || d.properties.admin2_name}\n` +
                          `Cloud Cover: ${d.properties.cloudCover?.toFixed(3) || 'N/A'}\n` +
                          `Class: ${d.properties.cloudCoverClass}`
            })
          ]
        })}
      </div>
    ...
    const tableData = filteredData.map(d => ({
      "Admin Name": d[`admin${currentAdminLevel}_name`] || d.admin0_name,
...
// Section 1.3 Chart/Table
{
  if (s13ViewType === "Chart") {
    ...
    const chartData = s13Data.flatMap(d => [
      { admin: d.admin, type: "Short-term", value: d.shortTerm },
      { admin: d.admin, type: "Long-term", value: d.longTerm }
    ]);
    
    return Plot.plot({
      width: 900,
      height: Math.max(400, s13Data.length * 25),
      marginLeft: 150,
      x: { label: "Forecast Skill (RPSS)", domain: [0, 1] },
      y: { label: null },
      color: { 
      ...
      marks: [
        Plot.barX(chartData, {
          ...
          y: "admin",
          fill: "type",
          title: d => `${d.admin}\n${d.type}: ${d.value?.toFixed(3) || 'N/A'}`
        }),
        Plot.ruleX([0.4, 0.6], { stroke: "#999", strokeDasharray: "4,4" })
      ]
    });
  } else {
