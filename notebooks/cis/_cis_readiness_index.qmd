...
```{ojs}
// Which boundaries to draw: no selection shows all countries, a country shows its
// regions, a region shows its districts. Selecting districts does not drill
// further — it highlights them within admin 2.
currentAdminLevel =
  selectedAdmin0.length === 0 ? 0 : selectedAdmin1.length === 0 ? 1 : 2
...
```{ojs}
// Every section 1 map needs the same thing: the boundaries for whichever admin
// level the selection implies, narrowed to the selection, joined to filteredData,
// with a display name and the admin-2 highlight flag precomputed. The only part
// that differs per map is which columns to read off the joined row, so that is
// the one argument: `fields(dataRow)` returns the properties for this map, and
// receives undefined where a boundary has no matching row.
buildLevelFeatures = (fields) => {
  const gaulCodeField =
    currentAdminLevel === 2
      ? "gaul2_code"
      : currentAdminLevel === 1
  ...
  // Drop boundaries the selection cannot reach. Matching on the gaul code catches
  // rows whose names differ from the boundary file; the name checks catch the
  // reverse. At admin 0 everything in view is already in scope.
  const scopedBoundaries = (() => {
    if (currentAdminLevel === 0) return currentBoundaries;

    const codesInData = new Set(
      filteredData.map((d) => d[gaulCodeField]).filter((c) => c != null),
    );

    return {
      ...currentBoundaries,
      features: currentBoundaries.features.filter(
        (f) =>
          codesInData.has(f.properties[gaulCodeField]) ||
  ...
  const joinKey = (d) =>
    currentAdminLevel === 2
      ? `${d.admin0_name}|${d.admin1_name}|${d.admin2_name}`
      : currentAdminLevel === 1
        ? `${d.admin0_name}|${d.admin1_name}`
        : `${d.admin0_name}`;
  ...
  return geoData.features.map((feature) => {
    ...
    return {
      ...
      properties: {
        ...p,
        // Precomputed so tooltips do not rebuild it on every hover.
        regionName:
          currentAdminLevel === 0
            ? p.admin0_name
            : currentAdminLevel === 1
        ...
        isHighlighted:
          ...
          selectedAdmin2.includes(p.admin2_name),
        ...fields(p.data),
      },
    };
  });
  ...
  if (s11ViewType === "Map") {
    ...
    const tableData = filteredData.map(d => {
      ...
      return {
        ...
        "Weather Station (Raw)": fmt(d["wstation_density"], 3),
        "Weather Station (Class)": classifyThreshold("wstation_density", d["wstation_density"], d),
        "Cloud Cover (Raw)": fmt(d["cloud-coverage_meanannual"], 3),
...
downloadCsvButton(
  filteredData.map((d) => {
    return {
      // Was computed and never emitted, so this export shipped with no way to
      // tell which region a row described. The other five exports all carry it.
...
s11Insight = {
  ...
  const insightData = selectedAdmin2.length > 0 
    ? filteredData.filter(d => selectedAdmin2.includes(d.admin2_name))
    : filteredData;
  ...
  if (s13ViewType === "Map") {
    ...
    const shortTermMap = makeChoropleth({
      ...
      channels: {
        ...
        shortTerm: {
          label: "Short-term",
          value: (d) => fmt(d.properties.shortTerm, 3)
        }
      },
      tooltip: {
        region: true,
    ...
    const longTermMap = makeChoropleth({
      ...
      channels: {
        ...
        longTerm: {
          label: "Long-term",
          value: (d) => fmt(d.properties.longTerm, 3)
        }
      },
      tooltip: {
        region: true,
    ...
    const deltaMap = makeChoropleth({
      ...
      channels: {
        ...
        delta: {
          label: "Difference",
          value: (d) => fmt(d.properties.delta, 3)
        }
      },
      tooltip: {
        region: true,
  ...
  if (s14ViewType === "Map") {
    ...
    const tableData = filteredData
      ...
      return {
        ...
        "Weather Stn (Raw)": fmt(d["wstation_density"], 3),
        "Weather Stn (Class)": classifyThreshold("wstation_density", d["wstation_density"], d),
        "Cloud Cover (Raw)": fmt(d["cloud-coverage_meanannual"], 3),
        "Cloud Cover (Class)": classifyThreshold("cloud-coverage_meanannual", d["cloud-coverage_meanannual"], d),
        "Precip Agree (Raw)": fmt(d["cv-precipitation_agreement"], 1),
        "Precip Agree (Class)": classifyThreshold("cv-precipitation_agreement", d["cv-precipitation_agreement"], d),
        "Short-term (Raw)": fmt(d["short-term_frcst_skill"], 3),
        "Long-term (Raw)": fmt(d["seasonal_frcst_skill"], 3)
      };
    });
