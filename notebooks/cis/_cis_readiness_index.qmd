...
parseCisRow = (row) => ({
  ...
  admin1_name:
    row.admin1_name === "\\N" || row.admin1_name === ""
      ? null
      : row.admin1_name,
  admin2_name:
    row.admin2_name === "\\N" || row.admin2_name === ""
      ? null
      : row.admin2_name,
  ...
  return Array.from(
    new Set(
      cisData
        .filter(
          (d) =>
            admin0Selection.includes(d.admin0_name) && d.admin1_name != null,
        )
        .map((d) => d.admin1_name),
    ),
  ).sort();
...
downloadButton(
  filteredData.map((d) => {
    ...
    return {
      ...
      "Weather Station Class": classifyTercile(
        d["weather-station_density"],
        africaTerciles.weatherStation,
      ),
      "Cloud Coverage": d["cloud-coverage_meanannual"],
      "Cloud Coverage Class": classifyTercile(
        d["cloud-coverage_meanannual"],
        africaTerciles.cloudCover,
      ),
    };
  }),
    ...
    return makeChoropleth({
      ...
      extraMarks: [
        ...
        Plot.geo(
          {
            type: "FeatureCollection",
            features: aboveThreshold.filter(d => d.properties.isHighlighted)
          },
          {
            fill: null,
            stroke: "#000",
