...
// Section 1.3 Map/Chart/Table
{
  if (s13ViewType === "Map") {
    ...
    // One 20px band per geography, plus the two axes. Counting the margins in
    // keeps the bands at 20px instead of shrinking them to absorb the axes.
    const marginTop = 40;
    ...
    // Rendered at full height: one band per unit, so a 600px window hid most of
    // the rows behind a second scrollbar. This is behind a view toggle, not the
    // default.
    return Plot.plot({
      style: chartStyle,
