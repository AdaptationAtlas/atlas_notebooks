...
// Section 1.3 Chart/Table
{
  if (s13ViewType === "Chart") {
    // Prepare data with delta for grouped bar chart
    ...
    // Prepare data for grouped bars - Short-term and Long-term grouped side-by-side
    const skillBars = sorted.flatMap(d => [
      { name: d.name, type: "Short-term", value: d.short, delta: d.delta, short: d.short, long: d.long },
      { name: d.name, type: "Long-term", value: d.long, delta: d.delta, short: d.short, long: d.long }
    ]);
    
    // Prepare data for delta bars (separate, can be negative)
    // Delta = long - short (negative when long < short)
    const deltaBars = sorted.map(d => {
      const delta = (d.long != null && d.short != null) ? d.long - d.short : null;
      return {
        name: d.name,
        type: "Delta",
        value: delta,
    ...
    const chartHeight = Math.min(Math.max(420, itemCount * 40), 600);
    
    // Determine if labels should be rotated
    const rotateLabels = itemCount > 12;
    const marginLeft = rotateLabels ? 150 : 80;
    ...
    const minDelta = Math.min(...deltaBars.map(d => d.value || 0), -0.1);
    ...
    return Plot.plot({
      ...
      color: {
        legend: true,
        domain: ["Short-term", "Long-term", "Delta"],
        range: ["#1f77b4", "#ff7f0e", "#808080"]
      },
      marks: [
        // Grouped bars for Short-term and Long-term (side-by-side)
        Plot.barX(skillBars, {
          x: "value",
          y: "name",
          fill: "type",
          ...
          title: d => {
            const short = d.short != null ? d.short.toFixed(2) : 'N/A';
            const long = d.long != null ? d.long.toFixed(2) : 'N/A';
            const delta = d.delta != null ? (d.delta >= 0 ? "+" : "") + d.delta.toFixed(2) : 'N/A';
            return `${d.name} | Short = ${short} | Long = ${long} | Δ = ${delta}`;
          }
        }),
        // Delta bars (separate, can be negative, extending left from 0)
        Plot.barX(deltaBars, {
          x: d => d.value,
          x1: 0, // Start from 0, extend left for negative values
          y: "name",
          fill: "Delta",
          tip: true,
          channels: {
        ...
        // Reference line at 0 for delta bars
        Plot.ruleX([0], { stroke: "#333", strokeWidth: 1 }),
        // Reference lines at 0.4 and 0.6
