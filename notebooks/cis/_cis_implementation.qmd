...
// Bivariate legend (3x3 grid) - simplified and integrated styling
s3Legend = {
  const size = 45;
  const gap = 2;
    ...
    <div style="margin-top: 20px; padding: 12px; background: #f8f9fa; border-radius: 4px; display: inline-block; font-family: system-ui, -apple-system, sans-serif;">
      <svg width="${totalWidth + 100}" height="${totalHeight}">
        <g transform="translate(0, 15)">
          ...
          <!-- Y-axis labels (Access) - right side -->
          ...
          <!-- Axis titles -->
          <text x="${totalWidth / 2}" y="${size * 3 + gap * 2 + 28}" font-size="11" text-anchor="middle" fill="#333" font-weight="600">${_lang(cis.s3ReadinessLabel)} →</text>
          <text x="${totalWidth + 50}" y="${size * 1.5 + gap}" font-size="11" fill="#333" font-weight="600" transform="rotate(90, ${totalWidth + 50}, ${size * 1.5 + gap})">${s3CurrentAccessLabel} ↑</text>
        </g>
      </svg>
    </div>
  `;
