...
// Query and clean the access data
s3AccessData = await s3AccessDB.query(`
  SELECT 
    admin0_name,
    CASE WHEN admin1_name = '\\N' OR admin1_name = '' THEN NULL ELSE admin1_name END as admin1_name,
    CASE WHEN admin2_name = '\\N' OR admin2_name = '' THEN NULL ELSE admin2_name END as admin2_name,
    iso3,
    CASE WHEN gaul0_code = '\\N' OR gaul0_code = '' THEN NULL ELSE CAST(gaul0_code AS INTEGER) END as gaul0_code,
    CASE WHEN gaul1_code = '\\N' OR gaul1_code = '' THEN NULL ELSE CAST(gaul1_code AS INTEGER) END as gaul1_code,
    CASE WHEN gaul2_code = '\\N' OR gaul2_code = '' THEN NULL ELSE CAST(gaul2_code AS INTEGER) END as gaul2_code,
    CASE WHEN tv = '\\N' OR tv = '' OR tv IS NULL THEN NULL ELSE CAST(tv AS DOUBLE) END as tv,
    CASE WHEN internet = '\\N' OR internet = '' OR internet IS NULL THEN NULL ELSE CAST(internet AS DOUBLE) END as internet,
    CASE WHEN cellphone = '\\N' OR cellphone = '' OR cellphone IS NULL THEN NULL ELSE CAST(cellphone AS DOUBLE) END as cellphone
  FROM access_data
`);
...
// Render based on view type
{
  ...
  const chartHeight = Math.max(600, s3SortedData.length * 25);
    ...
    ${Plot.plot({
      ...
      marks: [
        // Connecting lines
            Plot.link(s3SortedData.filter(d => d.accessValue !== null && !isNaN(d.accessValue)), {
        ...
        // Access dots (right)
            Plot.dot(s3SortedData.filter(d => d.accessValue !== null && !isNaN(d.accessValue)), {
          x: "accessValue",
          y: "admin0_name",
          fill: "#ff7f00",
          r: 5,
