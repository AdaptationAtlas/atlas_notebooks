...
s3BivariateMap = {
  ...
  const tooltip = d3
    .select("body")
    .append("div")
    .attr("class", "plotTooltip s3BivariateTip")
  ...
  // Fit projection to features - leave extra space at bottom and left for legend
  const bottomPadding = 200; // Extra space for rotated diamond legend
  ...
  // Expand viewBox to accommodate rotated legend (negative X for labels, extra Y for bottom)
  const svg = d3
    .create("svg")
    .attr("viewBox", [-50, 0, 950, 800])
  ...
  paths
    .on("mousemove", (event, d) => {
      ...
      const optimal = d.properties.bivariateClass === "High × High" ? `✓ ${_lang(cis.s3OptimalZone)}` : "";
      
      tooltip
        .style("top", event.pageY - 10 + "px")
        .style("left", event.pageX + 10 + "px")
        .style("opacity", 1)
        .html(`${country}<br>${_lang(cis.s3ReadinessLabel)}: ${readiness != null ? readiness.toFixed(2) : "N/A"} (${readinessClass})<br>${s3CurrentAccessLabel}: ${access != null ? (access * 100).toFixed(1) + '%' : "N/A"} (${accessClass})<br>Category: ${category}${optimal ? "<br><strong>" + optimal + "</strong>" : ""}`);
    })
    .on("mouseout", () => {
  ...
  const legendTooltip = d3
    .select("body")
    .append("div")
    .attr("class", "plotTooltip s3BivariateLegendTip")
    .style("position", "absolute")
    .style("opacity", 0)
    .style("background", "rgba(0, 0, 0, 0.8)")
    .style("color", "white")
  ...
  const classes = ["Low", "Moderate", "High"];
        ...
        rect
          .on("mousemove", (event) => {
            legendTooltip
              ...
              .style("left", event.pageX + 10 + "px")
              .style("opacity", 1)
              .html(`${_lang(cis.s3ReadinessLabel)}: ${readinessClass}<br>${s3CurrentAccessLabel}: ${accessClass}<br>Category: ${key}`);
          })
          .on("mouseout", () => {
    ...
    // Access label (diagonal from bottom-right towards top-left)
    const accessLabelX = totalWidth + 35;
    const accessLabelY = totalHeight + 25;
    selection
      .append("text")
      .attr("x", accessLabelX)
      .attr("y", accessLabelY)
      ...
      .attr("transform", `rotate(45, ${accessLabelX}, ${accessLabelY})`)
  ...
  const legendY = 630; // Position below the map area
