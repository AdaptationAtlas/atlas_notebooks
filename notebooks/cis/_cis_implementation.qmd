...
S3_ACCESS_TYPES = new Object({
  combined: { norm: "combined_norm", pct: "combined_pct", label: nbText.s3AccessCombined },
  internet: { norm: "internet_norm", pct: "internet_pct", label: nbText.s3AccessInternet },
  tv: { norm: "tv_norm", pct: "tv_pct", label: nbText.s3AccessTV },
  cellphone: { norm: "cellphone_norm", pct: "cellphone_pct", label: nbText.s3AccessCellphone },
})

s3CurrentAccessField = S3_ACCESS_TYPES[s3AccessType].norm
...
function getS3ScatterFacetData() {
  ...
  return s3ProcessedData.flatMap((d) =>
    Object.entries(S3_ACCESS_TYPES)
      .filter(([, type]) => d[type.norm] != null && !isNaN(d[type.norm]))
      .map(([key, type]) => ({
        ...d,
        accessType: type.label,
        accessValue: d[type.norm],
        accessKey: key,
      })),
  );
}
