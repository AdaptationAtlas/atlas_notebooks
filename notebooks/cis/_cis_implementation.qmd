...
// Uses the shared selector form template for consistency with other CIS sections
Inputs.form([
  renderA0Multi(),
  renderA1Multi(),
  renderA2Multi()
...
// ============================================================================
// ADMIN0 DATA PREPARATION
// ============================================================================
// Section 3 is currently country-level. Use the admin0 CIS readiness rows
// directly so the readiness values stay aligned with Sections 1 and 2.
// ============================================================================

// Load CIS readiness at Admin0 (country) level
s3CisReadinessAdmin0 = {
  if (typeof cisDB === 'undefined' || !cisDB) {
    console.error("cisDB is not available - Section 1 must load first");
    return [];
  }
  ...
  return rows.map((row) => {
    const parsed = parseCisRow(row);
    return {
      admin0_name: parsed.admin0_name,
      iso3: parsed.iso3,
      gaul0_code: parsed.gaul0_code,
      cis_readiness_avg: parsed.cis_readiness_index,
    };
  });
}
...
// Section 3 uses the shared thresholds.json-based readiness classification
// from Section 1. Because this section is aggregated to Admin 0, readiness
// classification should always use the admin0 thresholds.
```
...
// Use full section data (country filter controls removed)
s3FilteredData = {
...
// ============================================================================
// Unlike CIS readiness (which uses thresholds.json), access cutoffs
...
classifyS3Readiness = (value) => classifyThreshold("cis_readiness_index", value, 0);

function bivariateClassifyAccess(readiness, access, accessCutoffs) {
  const readinessClass = classifyS3Readiness(readiness);
    ...
    return html`<div style="display: flex; align-items: center; gap: 18px; flex-wrap: wrap;">
      <div style="flex: 0 0 auto; display: flex; justify-content: center;">
        ${makeBivariateLegend({
          ...
          yLabel: s3AccessType === "combined" ? "Combined Access" : s3CurrentAccessLabel,
          width: 220,
          height: 210,
          fontSize: 14
        })}
...
function createS3BivariateMap() {
  ...
  return makeChoropleth({
    ...
    channels: {
      region: {
        label: currentAdminLevel === 2 ? "District" : currentAdminLevel === 1 ? "Region" : "Country",
        value: d => d.properties.displayName || d.properties.admin0_name || "N/A"
      },
      readiness: {
        label: _lang(cis.s3ReadinessLabel),
        value: d => d.properties.cis_readiness_avg != null
          ? d.properties.cis_readiness_avg.toFixed(2)
          : "N/A"
      },
      readinessClass: {
      ...
      access: {
        ...
        value: d => d.properties.accessValue != null
  ...
  const accessData = s3SortedData.filter(d => d.accessValue !== null && !isNaN(d.accessValue));
    ...
    if (!s3ProcessedData || s3ProcessedData.length === 0) {
      ...
      if (facetData.length === 0) {
        return html`<div style="padding: 20px; background: #f0f0f0; border-radius: 5px;">
          <em>Scatter plot is unavailable because no access values are present for the current selection.</em>
        </div>`;
      }
      ...
      const xExtent = d3.extent(facetData, d => d.cis_readiness_avg);
          ...
          marks: [
            ...
            Plot.dot(
              facetData,
              {
                ...
                fill: d => {
                  const readinessClass = classifyS3Readiness(d.cis_readiness_avg);
