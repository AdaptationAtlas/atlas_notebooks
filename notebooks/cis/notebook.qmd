...
import { makeBivariateLegend, makeChoropleth, mergeDataToBoundaries } from "/helpers/plots.ojs";

// Build reusable SQL filters for admin selections
...
buildAdminSelectionWhereClause = ({
  admin0 = [],
  admin1 = [],
  mode = "strict",
} = {}) => {
  // Country-only filtering (used by sections that aggregate at admin0)
  if (mode === "admin0_only") {
    if (!admin0 || admin0.length === 0) return "TRUE";
    return `admin0_name IN (${toSqlInList(admin0)})`;
  }

  // No admin0 selected -> admin0 view
  if (!admin0 || admin0.length === 0) {
    return "admin1_name IS NULL";
  }
  // Admin0 selected, no admin1 -> admin1 view
  if (!admin1 || admin1.length === 0) {
    return `admin0_name IN (${toSqlInList(admin0)}) AND admin1_name IS NOT NULL AND admin2_name IS NULL`;
  }
  // Admin1 selected -> admin2 view
  return `admin0_name IN (${toSqlInList(admin0)}) AND admin1_name IN (${toSqlInList(admin1)}) AND admin2_name IS NOT NULL`;
};
```
