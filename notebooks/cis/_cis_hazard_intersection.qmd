...
s2BivariateMap = {
  ...
  paths
    .on("mousemove", (event, d) => {
      ...
      const readinessText = `CIS Readiness: ${dataEntry.cis_readiness_index.toFixed(2)} (${dataEntry.readinessClass})`;
      const hazardText = `${hazardName}: ${dataEntry.hazardValue.toFixed(1)} (${dataEntry.hazardClass})`;
      const categoryText = `Category: ${dataEntry.bivariateClass}`;
      const priorityText = dataEntry.bivariateClass === "High × High" ? "🎯 PRIORITY ZONE" : "";
      
      tooltip
        .style("top", event.pageY - 10 + "px")
        .style("left", event.pageX + 10 + "px")
        .style("opacity", 1)
        .html(`${regionName}<br>${readinessText}<br>${hazardText}<br>${categoryText}${priorityText ? "<br><strong>" + priorityText + "</strong>" : ""}`);
    })
  ...
  // Create legend tooltip (separate from map tooltip)
  d3.select(".bivariateLegendTip").remove();
  const legendTooltip = d3
    .select("body")
    .append("div")
    .attr("class", "plotTooltip bivariateLegendTip")
    .style("position", "absolute")
    .style("opacity", 0)
    .style("background", "rgba(0, 0, 0, 0.8)")
    .style("color", "white")
        ...
        rect
          .on("mousemove", (event) => {
            legendTooltip
              ...
              .style("left", event.pageX + 10 + "px")
              .style("opacity", 1)
              .html(`CIS Readiness: ${readinessClass}<br>Hazard Severity: ${classes[2 - i]}<br>Category: ${key}`);
          })
          .on("mouseout", () => {
