...
viewof s2ReadinessThreshold = {
  ...
  const ext = d3.extent(combinedData, d => d.cis_readiness_index);
  const _min = round(ext[0], 2);
  const _max = round(ext[1], 2);

  const input = Inputs.range([_min, _max], {
  label: _lang(cis.s2ReadinessThreshold),
    value: _min, // Show all countries by default
    step: 0.001
  })
  return Inputs.bind(input, viewof s14Threshold)
}
