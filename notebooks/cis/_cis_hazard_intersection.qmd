...
// Load hazard data using CSV (will switch to DuckDB/Parquet in production)
hazardDataRaw = await d3.csv("/data/cis/haz-means_adm_historic.csv", d3.autoType);
...
// Display current selection from Section 1
{
  yield html`<div style="padding: 10px; background: #f0f0f0; border-radius: 5px; margin: 10px 0;">
    <strong>${_lang(cis.currentSelection)}:</strong> ${breadcrumb}
  </div>`;
}
```
...
s2ProcessedData = {
  ...
  // Filter by Section 1's admin selections
  ...
  if (s2CurrentAdminLevel === 2) {
    ...
    filtered = filtered.filter(d => selectedAdmin0.includes(d.admin0_name));
  }
...
s2GeoData = {
  ...
  const features = s2Boundaries.features.map(feature => {
    const gaulCode = feature.properties[gaulCodeField];
    const match = s2MapData.find(d => d[gaulCodeField] === gaulCode);
...
s2Legend = {
  ...
  const totalHeight = size * 3 + padding * 2 + 60;
  ...
  // Draw 3x3 grid (inverted Y axis so High hazard is at top)
  const classes = ["Low", "Moderate", "High"];
  
  classes.forEach((readinessClass, i) => {
    classes.forEach((hazardClass, j) => {
      const key = `${readinessClass} × ${classes[2 - j]}`; // Invert j for Y axis
      g.append("rect")
        .attr("x", i * size)
        .attr("y", j * size)
        .attr("width", size)
      ...
      // Add star to High × High cell
      if (readinessClass === "High" && classes[2 - j] === "High") {
        g.append("text")
          .attr("x", i * size + size / 2)
          .attr("y", j * size + size / 2)
  ...
  // Y-axis label (Hazard Severity)
  svg.append("text")
    .attr("x", -totalHeight / 2 + 30)
    .attr("y", 12)
    .attr("text-anchor", "middle")
    .attr("font-size", "12px")
    .attr("font-weight", "bold")
    .attr("transform", `rotate(-90, 12, ${totalHeight / 2 - 30})`)
...
// Render map or table based on toggle
{
  if (s2ViewToggle === "map") {
      ...
      marks: [
        Plot.geo(s2GeoData, {
          ...
          title: d => {
            ...
            const isPriority = d.properties.bivariateClass === "High × High";
            const priorityLabel = isPriority ? "\n🎯 PRIORITY ZONE" : "";
            
            return `${adminName}\n` +
                   `CIS Readiness: ${d.properties.cis_readiness_index?.toFixed(2)} (${d.properties.readinessClass})\n` +
                   `${hazardName}: ${d.properties.hazardValue?.toFixed(1)} (${d.properties.hazardClass})\n` +
                   `Category: ${d.properties.bivariateClass}${priorityLabel}`;
          }
        })
      ]
    });
    ...
    const tableData = s2MapData.map(d => {
      ...
      return {
        adminLevel: s2CurrentAdminLevel === 0 ? "0" : s2CurrentAdminLevel === 1 ? "1" : "2",
        adminName: adminName,
...
s2Insights = {
  ...
  const highHazardHighReadiness = s2MapData.filter(d => d.bivariateClass === "High × High");
  const percentHighHigh = s2MapData.length > 0 
    ? (highHazardHighReadiness.length / s2MapData.length * 100).toFixed(1)
    : 0;
  ...
  const highHazardAreas = s2MapData.filter(d => d.hazardClass === "High");
