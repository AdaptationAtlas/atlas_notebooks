...
bivariateColors = {
  return {
    "Low × Low": "#e8e8e8",      // Light gray - low priority
    "Low × Moderate": "#b8d6be",  
    "Low × High": "#6c9a8b",      // Dark teal-green - high need, low capacity
    "Moderate × Low": "#c8b2d6",
    "Moderate × Moderate": "#9c9eba",
    "Moderate × High": "#627a8e",
    "High × Low": "#b8a0c2",      
    "High × Moderate": "#8b7fa8",
    "High × High": "#5a5f8f"      // Dark blue-purple - OPTIMAL (priority zone)
  };
}
```
...
viewof s2ViewToggle = Inputs.radio(
  ["map", "table", "plot"],
  {
    ...
    value: "map",
    format: d => d === "map" ? _lang(cis.s2ViewMap) : d === "table" ? _lang(cis.s2ViewTable) : "Bivariate Plot"
  }
);
```
...
// Mutable state for clicked admin0 (for zoom interaction)
mutable s2ClickedAdmin0 = null;

// Prepare admin1 data for zoom (when at admin0 level)
s2Admin1Data = {
  if (s2CurrentAdminLevel !== 0) return [];
  
  // Get admin1 level data from processed data
  const hazardColumn = s2HazardType === "drought" ? "drought_ndws" : "waterlogging_ndwl0";
  const hazardTypeCode = s2HazardType === "drought" ? "NDWS-mean" : "NDWL0-mean";
  
  // Filter to admin1 level data
  const admin1Data = s2ProcessedData.filter(d => 
    d.admin1_name != null && 
    d.admin2_name == null &&
    d.cis_readiness_index >= s2ReadinessThreshold &&
    d[hazardColumn] !== null
  ...
  // Group by admin1 and aggregate
  const grouped = d3.group(admin1Data, d => d.gaul1_code);
  return Array.from(grouped.values()).map(group => {
    const first = group[0];
    const avgReadiness = d3.mean(group, d => d.cis_readiness_index);
    const avgHazard = d3.mean(group, d => d[hazardColumn]);
    const classified = bivariateClassify(avgReadiness, avgHazard, cisReadinessCutoffs, hazardTypeCode);
...
s2DataIndex = {
  ...
  // Also index admin1 data for zoom
  if (s2CurrentAdminLevel === 0) {
    s2Admin1Data.forEach(d => {
      const admin0 = d.admin0_name;
      const admin1 = d.admin1_name;
...
s2BivariateMap = {
  ...
  if (!d3.select("style#bivariateTipStyles").node()) {
    d3.select("head").append("style")
      .attr("id", "bivariateTipStyles")
      .text(`
        .bivariateTip {
  ...
  // Get features and data based on current admin level and clicked state
  let featuresToShow, dataToUse;
  if (s2CurrentAdminLevel === 0 && s2ClickedAdmin0 !== null) {
    // Show admin1 for clicked admin0
    dataToUse = s2Admin1Data.filter(d => d.admin0_name === s2ClickedAdmin0);
    
    // Get admin1 boundaries for clicked country and join with data
    const admin1Features = admin1Boundaries.features
      .filter(f => f.properties.admin0_name === s2ClickedAdmin0)
      .map(feature => {
        const gaulCode = feature.properties.gaul1_code;
        const match = dataToUse.find(d => d.gaul1_code === gaulCode);
        
        // Fallback: try matching by admin names
        const dataMatch = match || dataToUse.find(d => 
          d.admin0_name === feature.properties.admin0_name &&
          d.admin1_name === feature.properties.admin1_name
        );
      ...
      .filter(f => f.properties.bivariateClass); // Only show features with data
    ...
    // Show current level features
    featuresToShow = s2GeoData;
    dataToUse = s2MapData;
  }
  
  // Create lookup map for current data (multiple keys for robustness)
  const dataLookup = new Map();
  dataToUse.forEach(d => {
    // Primary key: GAUL code
    const gaulKey = s2CurrentAdminLevel === 0 && s2ClickedAdmin0 !== null
      ? d.gaul1_code
    ...
    // Secondary key: admin name combination (for fallback matching)
    if (s2CurrentAdminLevel === 0 && s2ClickedAdmin0 !== null) {
      const nameKey = `${d.admin0_name}_${d.admin1_name}`;
      dataLookup.set(nameKey, d);
    }
  });
  ...
  const paths = svg
    ...
    .attr("class", s2CurrentAdminLevel === 0 && s2ClickedAdmin0 !== null ? "admin1-boundary" : "admin0-boundary")
    .attr("fill", (d) => {
      const gaulCodeField = s2CurrentAdminLevel === 0 && s2ClickedAdmin0 !== null
        ? 'gaul1_code'
        : s2CurrentAdminLevel === 0
        ? 'gaul0_code'
        : s2CurrentAdminLevel === 1
      ...
      // Try GAUL code first
      let dataEntry = dataLookup.get(d.properties[gaulCodeField]);
      
      // Fallback: try admin name combination
      if (!dataEntry && s2CurrentAdminLevel === 0 && s2ClickedAdmin0 !== null) {
        const nameKey = `${d.properties.admin0_name}_${d.properties.admin1_name}`;
        dataEntry = dataLookup.get(nameKey);
      }
    ...
    .on("click", (event, d) => {
      if (s2CurrentAdminLevel === 0 && s2ClickedAdmin0 === null) {
        mutable s2ClickedAdmin0 = d.properties.admin0_name;
      }
    });
  ...
  paths
    .on("mousemove", (event, d) => {
      ...
      const regionName = s2CurrentAdminLevel === 0 && s2ClickedAdmin0 !== null
      ...
      const tooltipHtml = `
        <table>
          ...
          <tr><th>CIS Readiness</th><td>${dataEntry.cis_readiness_index != null ? dataEntry.cis_readiness_index.toFixed(2) : "N/A"}</td></tr>
          <tr><th>Readiness Class</th><td>${dataEntry.readinessClass || "N/A"}</td></tr>
          <tr><th>${hazardName}</th><td>${dataEntry.hazardValue != null ? dataEntry.hazardValue.toFixed(1) : "N/A"}</td></tr>
          <tr><th>Hazard Class</th><td>${dataEntry.hazardClass || "N/A"}</td></tr>
          <tr><th>Category</th><td>${dataEntry.bivariateClass || "N/A"}</td></tr>
          ${dataEntry.bivariateClass === "High × High" ? `<tr><th>Priority</th><td>🎯 PRIORITY ZONE</td></tr>` : ""}
        </table>
      `;
  ...
  function bi_legend(selection) {
    ...
    classes.forEach((hazardClass, i) => {
      classes.forEach((readinessClass, j) => {
        const key = `${readinessClass} × ${classes[2 - i]}`;
          ...
          .on("mousemove", (event) => {
            const tooltipHtml = `
              <table>
                ...
                <tr><th>Hazard Severity</th><td>${classes[2 - i]}</td></tr>
    ...
    // Diagonal axis labels (positioned outside the rotated grid)
    // CIS Readiness label (diagonal from bottom-left towards top-right)
    const readinessLabelX = -35;
    const readinessLabelY = totalHeight + 25;
    selection
      .append("text")
      .attr("x", readinessLabelX)
      .attr("y", readinessLabelY)
      .attr("font-size", "11")
      .attr("fill", "#333")
      .attr("font-weight", "600")
      .attr("transform", `rotate(-45, ${readinessLabelX}, ${readinessLabelY})`)
      .text("CIS Readiness");
    
    // Hazard Severity label (diagonal from bottom-right towards top-left)
    const hazardLabelX = totalWidth + 35;
    const hazardLabelY = totalHeight + 25;
    selection
      .append("text")
      .attr("x", hazardLabelX)
      .attr("y", hazardLabelY)
      ...
      .attr("transform", `rotate(45, ${hazardLabelX}, ${hazardLabelY})`)
  ...
  // Position it at bottom-left with enough space for the rotated diamond
  const legendX = 20;
...
// Render map, table, or plot based on toggle
{
  if (s2ViewToggle === "map") {
    yield html`
      <div>
    ...
    `;
  } else if (s2ViewToggle === "plot") {
    // Bivariate plot
    const hazardName = s2HazardType === "drought" ? "Drought (Dry Spell Days)" : "Waterlogging (Days)";
    const hazardTypeCode = s2HazardType === "drought" ? "NDWS-mean" : "NDWL0-mean";
    
    // Prepare plot data
    const plotData = s2MapData.map(d => ({
      x: d.cis_readiness_index,
      y: d.hazardValue,
      bivariateClass: d.bivariateClass,
      readinessClass: d.readinessClass,
      hazardClass: d.hazardClass,
      adminName: s2CurrentAdminLevel === 0 
        ? d.admin0_name 
        : s2CurrentAdminLevel === 1
    ...
    // Get hazard cutoffs for reference lines
    const hazardCutoffs = hazardTypeCode === "NDWS-mean" 
      ? { low: 15, moderate: 20 }
      : { low: 2, moderate: 5 };
    ...
    yield Plot.plot({
      ...
      marks: [
        // Reference lines for CIS Readiness cutoffs
        Plot.ruleX([cisReadinessCutoffs.low], { stroke: "#666", strokeDasharray: "4,3", strokeWidth: 1 }),
        Plot.ruleX([cisReadinessCutoffs.moderate], { stroke: "#666", strokeDasharray: "4,3", strokeWidth: 1 }),
        // Reference lines for Hazard cutoffs
        Plot.ruleY([hazardCutoffs.low], { stroke: "#666", strokeDasharray: "4,3", strokeWidth: 1 }),
        Plot.ruleY([hazardCutoffs.moderate], { stroke: "#666", strokeDasharray: "4,3", strokeWidth: 1 }),
        // Scatter points
        Plot.dot(plotData, {
          x: "x",
          y: "y",
