...
s2ProcessedData = {
  ...
  // If specific admin2 regions are selected, filter to only those
  else {
    filtered = filtered.filter(d => 
      selectedAdmin0.includes(d.admin0_name) &&
      selectedAdmin1.includes(d.admin1_name) &&
      d.admin2_name != null &&
      (selectedAdmin2.length === 0 || selectedAdmin2.includes(d.admin2_name))
    );
  }
...
s2BivariateMap = {
  ...
  function bi_legend(selection) {
    ...
    // Axis labels rotated to run parallel with the diamond edges
    // CIS Readiness label (along the bottom-right edge of the diamond, rotated -45°)
    selection
      .append("text")
      .attr("x", centerX + 60)
      ...
      .attr("transform", `rotate(-45, ${centerX + 60}, ${totalHeight + 20})`)
      .text("CIS Readiness →");
    
    // Hazard Severity label (along the top-right edge of the diamond, rotated +45°)
    selection
      .append("text")
      .attr("x", totalWidth + 60)
      .attr("y", centerY - 30)
      .attr("font-size", "12")
      .attr("fill", "#333")
      .attr("font-weight", "600")
      .attr("text-anchor", "middle")
      .attr("transform", `rotate(45, ${totalWidth + 60}, ${centerY - 30})`)
