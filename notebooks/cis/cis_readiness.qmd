...
// Uses Observable's Inputs.bind to keep selectors in sync with master
createSyncedAdminSelector = function(level, masterView, options, uniqueId, labelString, placeholderString) {
  ...
  // Use Inputs.bind to bind this selector to the master
  // This creates a two-way binding between the selectors
...
## `{ojs} _lang(cis.methodsTitle)` {#methods-sources}

`{ojs} _lang(cis.methodsContent)`

### `{ojs} _lang(cis.dataSourcesTitle)`

`{ojs} md(_lang(cis.dataSourcesList))`
...
```{ojs}
toc_bottom = atlasTOC({
  skip: ["notebook-title", "appendix", "source-code", "methods-sources"], // These should be the section headings in the {# headings}
