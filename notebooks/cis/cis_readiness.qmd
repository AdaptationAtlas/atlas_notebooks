...
```{ojs}
// Import all UI components FIRST before any cells that use them
...
```{ojs}
// Language setup - MUST be defined before includes
...
// This creates a new selector that stays in sync with the master
// Helper function to create synchronized admin selectors
// This creates a new selector that stays in sync with the master (forced reload)
createSyncedAdminSelector = function(level, masterView, options, uniqueId, labelString, placeholderString) {
  // Get initial value from master view
  const initialValue = masterView.value ? masterView.value.slice() : [];

  // Create a new independent selector initialized with master's value
  ...
  selector.addEventListener('input', () => {
    // Check equality to avoid loops
    if (JSON.stringify(masterView.value) !== JSON.stringify(selector.value)) {
      masterView.value = selector.value;
      masterView.dispatchEvent(new Event('input', { bubbles: true }));
    }
  });
  
  // When master changes, update this selector
  masterView.addEventListener('input', () => {
    if (JSON.stringify(selector.value) !== JSON.stringify(masterView.value)) {
      selector.value = masterView.value ? masterView.value.slice() : [];
    }
  });
