...
createSyncedAdminSelector = function(level, masterView, masterValue, options, uniqueId, labelKey, placeholderKey) {
  ...
  const selector = multiSelect({
    ...
    options: options,
    selected: masterValue ? masterValue.slice() : [], // Initialize with current master value
    maxSelections: 10
  });
  ...
  // This creates a reactive dependency on masterValue
  if (masterValue && JSON.stringify(selector.value) !== JSON.stringify(masterValue)) {
