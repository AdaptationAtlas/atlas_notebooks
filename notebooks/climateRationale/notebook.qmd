...
recentChanges_obs = {
  ...
  return html`<div>${status}${chartAndLegendWithDownload}${captionDetails(plotCaption, undefined)}${trendBadge}${ipccQualifier}</div>`;
}
...
recentChangesMap_obs = {
  ...
  return html`<div>${status}${withDownload}${captionDetails(mapCaption, undefined)}</div>`;
}
```
