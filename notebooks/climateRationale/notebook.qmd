...
//| output: false
{
  ...
  [admin0Iso3, admin1Names, futurePeriodSelect, futureScenarioSelect, prod_type, hazardExposureVisible];
...
timeseries_futureProjections = () => {
  ...
  // In absolute mode, expand the y-domain to include the 1995–2014 baseline
  // dot + bar (mean ± interannual σ) so the marker is never clipped — it's
  // drawn at year = yearMin - 1 with y in [mean - sd, mean + sd].
  let yBaseMin = +Infinity;
  let yBaseMax = -Infinity;
  if (!_showAnomaly) {
    for (const a of new Set(_data.map((d) => d.adminName))) {
      const m = baselineMeanByAdmin.get(a);
      const s = baselineStdByAdmin.get(a);
      if (m == null) continue;
      yBaseMin = Math.min(yBaseMin, m - (s ?? 0));
      yBaseMax = Math.max(yBaseMax, m + (s ?? 0));
    }
  }
  ...
  } else if (isFinite(yBaseMin)) {
    // Absolute mode: include the baseline dot+bar bounds.
    yMin = Math.min(yMin, yBaseMin);
    yMax = Math.max(yMax, yBaseMax);
  }
  if (showThresholds) {
  ...
  const renderRow = (rowAdmins, rowIdx, isFirst, isLast) => {
    ...
    // Baseline marker: 1995–2014 mean ± interannual σ, rendered as a dot +
    // vertical error bar per admin at the left edge of the projection x-axis
    // (only in absolute mode — in anomaly mode the baseline is zero by
    // definition and the existing zero ruleY conveys that already).
    // Placed at year = yearMin - 1 so it sits visually just outside the
    // projection cone and reads as "where the recent past sat" without
    // overlapping the first projection point.
    const yearMin = d3.min(rowData, (d) => d.year);
    const baselineX = yearMin != null ? yearMin - 1 : null;
    const baselineMarkers = !_showAnomaly && baselineX != null
      ? rowAdmins
          .map((a) => {
            const m = baselineMeanByAdmin.get(a);
            const s = baselineStdByAdmin.get(a);
            if (m == null) return null;
            return { adminName: a, x: baselineX, y: m, y1: m - (s ?? 0), y2: m + (s ?? 0) };
          })
          .filter((d) => d != null)
      : [];
    const baselineLabels = baselineMarkers
      .filter((d) => d.adminName === lastAdmin);
    return Plot.plot({
      width,
      height: 380,
      marginLeft: 70,
      ...
      marks: [
        ...
        // 1995–2014 baseline marker — dot at the mean + vertical bar at
        // ±1 interannual σ, drawn one year to the left of the projection
        // start so the reader can compare the future trajectory to the
        // recent past at a glance. Only rendered in absolute mode; in
        // anomaly mode the baseline is implicitly zero (the dashed zero
        // line above) and the threshold ±1σ/±2σ lines (when Highlight is
        // ON) already encode the same information.
        Plot.ruleX(baselineMarkers, {
          fx: "adminName",
          x: "x",
          y1: "y1",
          y2: "y2",
