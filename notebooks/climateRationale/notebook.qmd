...
// Key Facts: one client per parquet so the four plots (Pov, Gdp,
// Landuse, Exposure) don't serialise behind each other. Before this
// split, the Key Facts paints were all queued behind crop-livestock_all
// (the exposure parquet, ~85 s to drain on cold start due to absent
// row-group stats — same upstream issue as futureProjections per
// dispatches/2026-05-27_parquet-pushdown-pipeline-ask.md).
...
db ={
  ...
  return await generateDB(
    // Slim db down to just the parquets that don't have their own
    // dedicated client. Excluded:
    //   - futureProjections section parquets (own client: dbFutureHive)
    //   - hazard_exposure                    (own client: dbHazardExposure)
    //   - Key Facts parquets above           (own client: dbPov/Gdp/Landuse/Exposure)
    // What's left in db: historic_climate_timeseries (Recent Changes)
    // and adm0_faostat (Production Trends).
    _cleaned.filter((d) =>
      !d.sections.includes("futureProjections") &&
      d.key !== "hazard_exposure" &&
      d.key !== "poverty" &&
      d.key !== "a0_gdp" &&
      d.key !== "a0_landuse" &&
      d.key !== "exposure"
