...
recentChanges_obs = {
  ...
  // legend together (issue #5: download button sits below legend) and
  // pass a pngOverride that rasterises the wrapper via foreignObject
  // so the exported PNG includes the legend alongside the chart.
  ...
  const chartAndLegendWithDownload = chartDownloadMenu(chartAndLegend, {
    ...
    // Composite PNG = chart SVG + legend HTML via foreignObject.
    pngOverride: () => elementToPngBlob(chartAndLegend, 2),
  });
  ...
  return html`<div>${chartAndLegendWithDownload}${trendBadge}${ipccQualifier}</div>`;
}
