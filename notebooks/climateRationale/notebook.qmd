...
```{ojs}
md`<details class="alert alert-info help-callout" role="note">
<summary>Generic thresholds: known historic-vs-future inconsistency — chart now defaults to Crop-specific (FAO Ecocrop).</summary>

The chart defaults to the **Crop-specific (FAO Ecocrop)** threshold definition because it produces a consistent picture between historic 1995–2014 and future scenario × period panels.

The alternative **Generic thresholds** option (under Advanced controls below) currently has an upstream pipeline bug: the historic panel reports zero exposure under \`heat\`, \`heat+wet\`, and \`wet\` for many countries, while every future scenario × period shows non-zero exposure across all 7 hazard categories — historic and future appear to be categorised by different rules. The Generic option is kept available for testing only; do not draw narrative conclusions from generic-threshold historic-vs-future comparisons until the upstream re-bake lands.
