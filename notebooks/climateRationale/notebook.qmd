...
```{ojs}
md`<details class="alert alert-info help-callout" role="note">
<summary>Heads up — historic and future hazard categories are not directly comparable.</summary>

The historic 1995–2014 panel reports **zero exposure under \`heat\`, \`heat+wet\`, and \`wet\`** for many countries — those occurrences appear bundled into the \`dry+*\` combinations by the historic-baseline pipeline step. Future scenarios use a different categorisation and redistribute the same mass across all 7 hazard combinations, so \`wet\`, \`heat\`, and \`heat+wet\` light up for the same admin × crop where the historic panel shows nothing. Compare **totals per (admin × scenario × timeframe)** rather than reading the per-color split across historic vs future. A pipeline-side fix is pending upstream.
