...
```{ojs}
// SPEI-specific note: explains the controls that disappear and what
// the map shows. Only renders when the active Recent Changes variable
// is SPEI-03 or SPEI-12; for the other 4 observational vars (TAVG,
// TMAX, TMIN, PTOT) this returns an empty md fragment.
{
  const isSPEI = (climateVarSelect.id || "").startsWith("SPEI");
  if (!isSPEI) return md``;
  return md`<details class="alert alert-info help-callout" role="note" open>
<summary>About SPEI in this section</summary>

SPEI is the **Standardised Precipitation–Evapotranspiration Index** — already a z-score against its own 1991–2020 reference window. That means several of the controls and views in this section don't apply:

- **Show as anomaly** is hidden — SPEI values *are* anomalies by construction (a value of -1.5 already means 1.5 standard deviations drier than the 1991–2020 norm).
- **Monthly view** is hidden — SPEI is a rolling-window accumulation pinned to a single month per year (for SPEI-03, the season's last month), so there's nothing finer to show.
- **Observational uncertainty band** is hidden — the band shown for raw climate variables reflects CHIRPS / CHIRTS-ERA5 sampling spread; SPEI is a derived index where the uncertainty is dominated by the calibration window, not the source dataset.
- The **map below** shows SPEI's *interannual variability* (the standard deviation over 1991–2020 in z-score units) rather than the mean, because the mean is ~0 everywhere by construction. Darker reds mark regions where SPEI swings more widely across years (i.e. more variable drought conditions).
