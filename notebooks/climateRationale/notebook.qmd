...
db ={
  ...
  return await generateDB(
    // Future projections + hazard exposure data are big, so split into
    // separate databases (dbFutureHive + dbHazardExposure) that gate
    // their view-registration on IntersectionObserver visibility.
    _cleaned.filter((d) =>
      !d.sections.includes("futureProjections") &&
      !d.sections.includes("hazardExposure")
    ),
  );
}

dbHazardExposure = {
  // Path B-2: gate hazard_exposure parquet registration on the section's
  // visibility. The hazard_exposure parquet is the largest single file
  // in the notebook (~60 M rows); its footer + initial column-chunks
  // accounted for ~6 fetches on initial paint before this gate. The
  // sole consumer (hazardExposure_dataAll) already returns [] while
  // !hazardExposureVisible, so the sentinel is belt-and-braces.
  if (!hazardExposureVisible) return { query: async () => [] };
  const _cleaned = data_obj
    .filter((d) => d.sections.includes("hazardExposure"))
