...
recentChangesMap_obs = {
  ...
  if (mapSmooth_obs) {
    ...
    // fillRect on the display canvas. Bypasses drawImage's upsample,
    // ctx.imageSmoothingEnabled, and CSS image-rendering entirely —
    // a browser cannot smooth explicit fillRect calls. (Setting
    // imageSmoothingEnabled = false alone produced visually identical
    // output on Pete's setup; the rectangle-paint path is the
    // reliable way to guarantee discrete cells.)
    const cellW = W / sw;
    const cellH = H / sh;
    for (let row = 0; row < sh; row++) {
      const cy = row * cellH;
      for (let col = 0; col < sw; col++) {
        ...
        const c = colorRamp_obs(v, activeStops);
        if (!c) continue;
        // Add a small +0.5 px overlap on adjacent cells so rounding
        // doesn't leave thin background-coloured seams between cells.
        dctx.fillStyle = `rgba(${c[0]},${c[1]},${c[2]},${c[3] / 255})`;
        dctx.fillRect(col * cellW, cy, cellW + 0.5, cellH + 0.5);
      }
    }
  }
  dctx.restore();
