...
```{ojs}
// Variable description block — mirrors the climateVarDescription pattern
// used in the Recent Changes section. Renders a one-paragraph,
// user-friendly explanation of the active variable so non-economists can
// read "constant 2014–2016 Int$" without chasing FAOSTAT documentation.
{
...
```{ojs}
viewof anomaly_obs = Inputs.toggle({ label: "Show as anomaly (vs 1991-2020)", value: false });
```
...
//| output: false
observed_obs_raw = {
  if (obsAdmin0Iso3 == null) return [];
  if (!obsVarCodes.includes(climateVarSelect.id)) return [];  // heat-index placeholder branch
  ...
  const isAdmin1 = admin1Names.length === 1;
  const adminName = isAdmin1 ? admin1Names[0] : null;
...
baseline_obs = {
  const baseline = observed_obs_raw.filter((d) => d.year >= 1991 && d.year <= 2020);
...
observed_obs = observed_obs_raw.map((d) => {
  const v = useAnomaly_obs ? d.value_mean - baseline_obs.mean : d.value_mean;
  const z = baseline_obs.sd > 0 ? (d.value_mean - baseline_obs.mean) / baseline_obs.sd : 0;
  let cls = "normal";
  if (isSPEI_obs) {
...
recentChanges_obs = {
  ...
  const stripeColour = (d) => {
    const anom = d.value_mean - baseline_obs.mean;
    const scale = baseline_obs.sd > 0 ? baseline_obs.sd * 2 : 1;
  ...
  // against a 0 baseline. PTOT bars start from 0. Anomaly mode →
  // all bars from 0.
  const barBaseline = useAnomaly_obs ? 0
    : variable === "PTOT" ? 0
    : baseline_obs.mean;

  const meanForBands = useAnomaly_obs ? 0 : baseline_obs.mean;
  // Hazard-gradient background bands. Inside ±1σ stays unshaded.
  // 1σ–2σ tinted amber (matches "unusual" colour); >2σ tinted red
  // (matches "extreme"). Caps at max(±3σ, data extent).
  const _bandUnusual = "rgba(244, 162, 97, 0.10)";
  const _bandExtreme = "rgba(225,  87, 89, 0.14)";
  let bands = [];
  if (!isSPEI_obs && baseline_obs.sd > 0) {
    const dataYMin = d3.min(observed_obs, (d) => d.value_plot);
    const dataYMax = d3.max(observed_obs, (d) => d.value_plot);
    const topCap    = Math.max(dataYMax, meanForBands + 3 * baseline_obs.sd);
    const bottomCap = Math.min(dataYMin, meanForBands - 3 * baseline_obs.sd);
    bands = [
      { y1: meanForBands + 2 * baseline_obs.sd, y2: topCap, fill: _bandExtreme },
      { y1: bottomCap, y2: meanForBands - 2 * baseline_obs.sd, fill: _bandExtreme },
      { y1: meanForBands + baseline_obs.sd, y2: meanForBands + 2 * baseline_obs.sd, fill: _bandUnusual },
      { y1: meanForBands - 2 * baseline_obs.sd, y2: meanForBands - baseline_obs.sd, fill: _bandUnusual },
    ];
  }

  const speiLines = isSPEI_obs ? [
    { y: -2,   stroke: "#e15759" },
    { y: -1.5, stroke: "#e15759" },
    { y: -1,   stroke: "#f4a261" },
    { y:  0,   stroke: "#999"    },
  ...
  // Mark builders.
  const stripesMark = (yMin, yMax) => Plot.rect(observed_obs, {
  ...
  const barMark = Plot.rect(observed_obs, {
  ...
  const lineMarks = [
    Plot.line(observed_obs, { x: xField, y: "value_plot", stroke: "#333", strokeWidth: 1 }),
    Plot.dot(observed_obs, {
  ...
  // Split into bgMarks + dataLayerMarks so the trend CI band + obs
  // uncertainty band can interleave between them (see z-order below).
  let bgMarks = [];
  let dataLayerMarks = [];
  if (isSPEI_obs) {
    bgMarks = speiLines.map((l) => Plot.ruleY([l.y], { stroke: l.stroke, strokeDasharray: "2,3" }));
    dataLayerMarks = [
      Plot.barY(observed_obs, {
    ...
    bgMarks = [
      ...bands.map((b) =>
        Plot.rect([b], {
          x1: d3.min(observed_obs, (d) => d[xField]),
          x2: d3.max(observed_obs, (d) => d[xField]),
    ...
    const yMin = d3.min(observed_obs, (d) => d.value_plot);
    const yMax = d3.max(observed_obs, (d) => d.value_plot);
    bgMarks = [];
  ...
  // Trend layer (Mann-Kendall + Theil-Sen). Skipped when off or for SPEI.
  const trendStats = (!showTrend_obs || isSPEI_obs)
    ? null
    : mannKendall(observed_obs.map((d) => d.value_plot), {
        xValues: observed_obs.map((d) => Number(d.year)),
      });

  const trendStroke = variable === "PTOT" ? "#1f5fa0" : "#b21010";
  const trendMarks = (showTrend_obs && !isSPEI_obs && trendStats && !trendStats.error &&
    ...
    ? trendOverlayMarks(observed_obs, {
        xField: "year",
        yField: "value_plot",
        showCI: true,
        ciOpacity: 0.15,
  ...
  // Suppressed for SPEI and for stripes-only (no data line to wrap).
  const obsBandMarks = (showObsUncertainty_obs && !isSPEI_obs && plotType_obs !== "stripes")
    ? observationalUncertaintyMarks(observed_obs, {
        xField: "year",
        yField: "value_plot",
        variable,
        fill: "#666",
  ...
  // Split trendMarks into [CI band, trend line] for explicit z-order
  // interleaving — CI band under data line, trend line on top.
  ...
  const chart = Plot.plot({
    ...
    // Z-order bottom → top: bg bands + baseline rule, obs uncertainty
    // band, trend CI band, data layer, trend line.
  ...
  if (!showTrend_obs) {
    ...
    const speiStats = mannKendall(observed_obs.map((d) => d.value_plot), {
      xValues: observed_obs.map((d) => Number(d.year)),
    });
    if (speiStats && !speiStats.error) {
      trendBadge = html`<div style="font-size: 0.9em; padding: 6px 10px; background: #f6f6f6; border-left: 3px solid #888; margin-bottom: 6px;">
        <strong>${variable} trend:</strong> Mann-Kendall p ${fmtP(speiStats.pValue)} · n = ${speiStats.n} years (${yearSpan})${speiStats.usedTFPW ? " · TFPW applied" : ""}.
  ...
  // chartDownloadMenu handles PNG / SVG / CSV exports.
  ...
  const showsBands      = !isSPEI_obs && (plotType_obs === "line" || plotType_obs === "bars") && baseline_obs.sd > 0;
...
recentChanges_obs_caption = {
  ...
  const region = admin1Names.length === 1
    ...
    : admin1Names.length > 1
      ? `${obsAdmin0Name} (national — ${admin1Names.length} admin1 regions selected)`
      : `${obsAdmin0Name} (national)`;
  const baselineCopy = baseline_obs.n > 0
    ? `Baseline (1991-2020): μ = ${baseline_obs.mean.toFixed(2)}, σ = ${baseline_obs.sd.toFixed(2)}, n = ${baseline_obs.n} years.`
  ...
  const multiCountryNote = admin0Iso3.length > 1
    ...
    : "";
  return md`
*Region: **${region}** · Variable: **${variable}** · Season: **${season}** · View: **${viewType_obs}** · Plot: **${plotType_obs}**${useAnomaly_obs ? " · **anomaly mode**" : ""}${showTrend_obs ? " · **trend layer on**" : ""}${showObsUncertainty_obs && !isSPEI_obs ? " · **obs uncertainty band on**" : ""}*

${baselineCopy} ${speiCopy}${multiCountryNote}
  `;
}
```
