...
recentChanges_obs = {
  ...
  const nRows = observed_obs.length;
  const status = html`<div style="font-size: 0.82em; color: #666; margin-bottom: 4px;">
    data fetch ${fetchSec}s · ${nRows} rows · ${observed_obs_raw.nQueries} ${observed_obs_raw.nQueries === 1 ? "query" : "queries"} · source: ${observed_obs_raw.source}
  </div>`;
