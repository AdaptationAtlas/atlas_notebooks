...
db ={
  ...
  return await generateDB(
    // Future projections (gated via dbFutureHive) and the big
    // hazard_exposure parquet (gated via dbHazardExposure below) are
    // pulled out so their view-registration can defer until the user
    // scrolls toward their sections. We filter the hazard_exposure
    // dataset by KEY rather than section membership because the
    // sibling `exposure` parquet (crop+livestock VoP) is in BOTH the
    // keyFacts and hazardExposure sections — Key Facts needs it on
    // first paint, so it must stay in `db`.
    _cleaned.filter((d) =>
