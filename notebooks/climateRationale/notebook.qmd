...
recentChanges_obs = {
  ...
  // (issue #5 part a, Pete 2026-05-21). Declared AFTER `legend` to
  // avoid the TDZ ReferenceError Pete hit 2026-05-22.
  //
  // TODO: include the legend in the PNG export too (issue #5 part c).
  // The earlier pngOverride approach (elementToPngBlob via
  // foreignObject) failed silently in some browsers — the nested SVG
  // inside an XHTML foreignObject inside an SVG envelope appears to
  // taint the canvas, so canvas.toBlob returned null. Reverted to the
  // default chart-SVG-only PNG so the button keeps working; PNG-with-
  // legend revisit needs a different approach (probably rebuild the
  // legend as inline SVG marks via Plot.legend or hand-build a
  // composite canvas pipeline that rasterises each layer separately).
