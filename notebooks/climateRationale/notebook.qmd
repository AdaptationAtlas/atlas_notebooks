...
viewof includeAdmin0 = {
  ...
  // Native browser `title` tooltips have a ~1 s hover delay — Pete read
  // that as the tooltip being broken. Replace with a CSS-only instant
  // tooltip: a visible ⓘ icon with a bubble that pops up on :hover via
  // visibility/opacity, no JS, no delay. Keep `title` as an a11y fallback
  // for screen-readers and for users who hover-and-wait.
  const tip = "When ON, plots include country-level (admin0) values alongside any selected admin1 regions. When OFF (default), only the selected admin1 regions are shown. Toggle ON when you want a national reference line, or when no admin1 regions are selected and you want to see country-level data anyway.";
  const label = toggle.querySelector("label");
  if (label) {
    const wrap = html`<span class="cr-tooltip-wrap" style="position: relative; display: inline-block; margin-left: 4px;">
      <span class="cr-tooltip-icon" aria-label="${tip}" title="${tip}" style="cursor: help; color: #2563eb; font-size: 1.05em;">ⓘ</span>
...
inputTemplate()([globalA0, globalA1, viewof includeAdmin0]);
```
...
`{ojs} downloadButton(keyFacts_combinedData, "keyFacts", "Download demographic + economic")`
...
//| output: false
{
  renderToDiv("plotProductionTrends", () => {
    if (viewProductionTrends === "table") return dataTable(productionTrends_data);
    return chart_productionTrends();
  });
}
```

`{ojs} downloadButton(productionTrends_data, "productionTrends")`
...
`{ojs} downloadButton(exposure_plotData, "agProductionBySector", "Download production value by sector")`
...
//| output: false
{
  // Re-show loader when data-driving selectors change, so users see feedback while the next query runs.
  [admin0Iso3, admin1Names, climateVarSelect, seasonSelect, showRecentAnomaly, highlightExtremes, paletteRecent, viewRecentChanges];
...
//| output: false
{
  [admin0Iso3, admin1Names, climateVarSelect, seasonSelect, futurePeriodSelect, futureScenarioSelect, showFutureUncertainty, showFutureAnomaly, highlightExtremesFuture, paletteFuture, viewFutureChanges];
...
//| output: false
{
  [admin0Iso3, admin1Names, futurePeriodSelect, futureScenarioSelect, prod_type, viewHazardExposure];
...
//| output: false
{
  renderToDiv("plotHazardExposure", () => {
    if (viewHazardExposure === "table") return dataTable(hazardExposure_plotData);
    return stackbars_hazardExposure();
  });
}
```

`{ojs} downloadButton(hazardExposure_plotData, "hazardExposure")`
...
// admin-faceted plot.
captionDetails = (caption, summary = "About this plot") => html`
...
exposureBars_keyFacts = function () {
  ...
  // Grid layout: up to 3 admin panels per row, wrap to additional rows.
  // Each row is its own Plot.plot so heights stay bounded.
  const cols = Math.min(3, new Set(_data.map((d) => d.adminName)).size);
  ...
  const renderRow = (rowAdmins, rowIdx) => {
    ...
    return Plot.plot({
      ...
      // (admin names, often two lines after wrapTickLabel).
...
barplot_recentChanges = () => {
  ...
  const { chunks, cols } = adminGridSplit(_data);
...
stackbars_hazardExposure = () => {
  ...
  const { chunks, cols } = adminGridSplit(dataForAggregation);
