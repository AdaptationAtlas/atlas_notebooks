...
```{ojs}
viewof viewAgProduction = Inputs.select(["bars", "treemap"], {
  label: "View Type",
  value: "bars",
...
treemap_agProduction = () => {
  ...
  // Same sequential palette family as the bars view.
  // paletteAgProd is a sequential palette name (set by
  // sequentialPaletteSelector) — its interpolators live in
  // sequentialPaletteInterpMap, NOT the diverging paletteInterpMap.
  const interp =
    sequentialPaletteInterpMap[paletteAgProd] || d3.interpolateYlGn;
  const maxVal = d3.max(rows, (d) => d.value) || 1;
  // Square-root scaling so smaller values aren't washed out — same
  // trick the bars view uses implicitly via Plot's linear scale on
  // the extents.
  const colorOf = (v) => interp(Math.sqrt(v / maxVal));
  ...
  const buildOne = (admin) => {
    ...
    const leafEls = root.leaves().map((leaf) => {
      ...
      const showLabel = w > 56 && h > 22;
      const showValue = w > 70 && h > 36;
      const label =
        _lang(cropTranslations[leaf.data.crop]) || leaf.data.crop;
      const tipText = `${label}: ${fmtUSD(leaf.data.value)}`;
    ...
    return html`<div style="display: inline-block; vertical-align: top; margin-right: ${gap}px; margin-bottom: ${gap}px;">
  ...
  const caption = multiLineText(
    [
      `Data is from ${exposure_plotData[0]?.value_year ?? "?"}, measured in nominal ${exposure_plotData[0]?.unit?.split("-").pop() ?? "?"} US dollars.`,
      "Box area is proportional to value of production. Boxes are sorted (largest top-left); colour is also scaled to value (square-root) so smaller crops stay distinguishable. Hover for full crop name + value when the label doesn't fit.",
      html`Source: <a href="https://cgspace.cgiar.org/items/5736e3ff-aadc-4999-8922-8bac50b1c916">MapSPAM 2020 v1r2 SSA</a> (Adaptation Atlas commissioned variant). Atlas-hosted data: <a href="https://radiantearth.github.io/stac-browser/#/external/digital-atlas.s3.amazonaws.com/stac/public_stac/catalog.json">STAC catalog</a>.`,
    ],
    "atlasFigCaption",
  );
...
treemap_productionTrends = () => {
  ...
  const buildOne = (admin) => {
    ...
    const leafEls = root.leaves().map((leaf) => {
      ...
      const tipText = `${label} (${leaf.data.year}): ${fmtVal(leaf.data.value)}`;
      return svg`<g transform="translate(${leaf.x0}, ${leaf.y0})">
        <rect
          class="cr-treemap-cell"
          data-tip="${tipText}"
          width="${w}"
          height="${h}"
          fill="${colorByCommodity.get(label)}"
          stroke="#999"
          stroke-width="0.75"
        ><title>${tipText}</title></rect>
        ${
  ...
  const caption = multiLineText(
    [
      ...
      html`<strong>Source.</strong> <a href="https://www.fao.org/faostat/en/#data/QV">FAOSTAT QV</a> + <a href="https://www.fao.org/faostat/en/#data/QCL">FAOSTAT QCL</a>. Built by <a href="https://github.com/AdaptationAtlas/hazards_prototype/blob/develop/R/0.4.5_create_faostat_long.R">AdaptationAtlas/hazards_prototype</a>.`,
      "Commodities filtered to those representing ≥ 0.25 % of national vop_intd15 averaged over the last 5 years; FAO aggregate rollups and residual “n.e.c.” categories excluded; spice items combined into a single “Spices” entry.",
