...
//| output: false
{
  ...
  [admin0Iso3, admin1Names, climateVarSelectFuture, seasonSelect, futurePeriodSelect, futureScenarioSelect];
...
//| output: false
{
  renderToDiv("plotFutureProjections", () => {
    ...
    // Keep showing the loader spinner until the gate flips.
    if (!futureProjectionsVisible) return loaderContent();
...
//| output: false
{
  renderToDiv("plotHazardExposure", () => {
    ...
    // yet), hazardExposure_dataAll returns []. Show the loader spinner
    // rather than fall through to "No data available" downstream.
    if (!hazardExposureVisible) return loaderContent();
