...
// (CHIRPS / CHIRTS-ERA5) and only supports the 6 observational vars;
// Future + Extreme use CMIP6 projections and support all 12 (incl. heat
// indices). Keeping the two selectors disconnected lets a user explore
// e.g. observed PTOT in Recent and a heat index (HSH-max) in Future
// without one selection clobbering the other. Lives in its own cell
// because Observable cells allow only one top-level binding.
viewof climateVarSelectFuture = Inputs.select(hazards_obj, {
  format: (d) => d.id,
  value: hazards_obj.find((d) => d.id === "PTOT") ?? hazards_obj[0],
});
```
