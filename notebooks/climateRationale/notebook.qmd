...
recentChanges_obs = {
  ...
  // (issue #5 part a). Composite PNG export rasterises the chart SVG
  // via the proven SVG → Image path, then rasterises the legend HTML
  // separately via foreignObject, then stacks them vertically on a
  // single canvas (issue #5 part c). The legend step is in a try/catch
  // so the button still works even if foreignObject silently taints
  // the canvas in some browser — falls back to chart-only PNG.
  ...
  const compositeChartLegendPng = async () => {
    const dpr = 2;
    const loadImg = (url) => new Promise((res, rej) => {
      const im = new Image();
      im.onload  = () => res(im);
      im.onerror = (e) => rej(new Error("image load failed"));
      im.src = url;
    });

    // Layer 1 — chart SVG. Plot.plot returns an SVG, but without
    // xmlns / xmlns:xlink / explicit width / height attributes the
    // Image rasteriser can't decode the serialised string. Mirror the
    // chartDownloadMenu helper's _serializeSvg pattern (clone +
    // backfill those attributes on the clone) so the rasteriser
    // succeeds. The earlier omission silently failed every PNG export
    // and made Pete's button look broken on 2026-05-22.
    const chartRect = chart.getBoundingClientRect();
    const chartW = Math.max(1, Math.ceil(chartRect.width));
    const chartH = Math.max(1, Math.ceil(chartRect.height));
    const chartClone = chart.cloneNode(true);
    if (!chartClone.getAttribute("xmlns"))       chartClone.setAttribute("xmlns",       "http://www.w3.org/2000/svg");
    if (!chartClone.getAttribute("xmlns:xlink")) chartClone.setAttribute("xmlns:xlink", "http://www.w3.org/1999/xlink");
    if (!chartClone.getAttribute("width"))       chartClone.setAttribute("width",  chartW);
    if (!chartClone.getAttribute("height"))      chartClone.setAttribute("height", chartH);
    const chartSvgString = new XMLSerializer().serializeToString(chartClone);
    const chartBlob = new Blob([chartSvgString], { type: "image/svg+xml;charset=utf-8" });
    const chartUrl = URL.createObjectURL(chartBlob);

    // Layer 2 — legend HTML (skip when no legend items rendered).
    // Wrap in foreignObject envelope so Image can rasterise the live
    // HTML node. Catches taint / parse errors so the button still
    // works in the chart-only fallback path.
    let legendUrl = null;
    let legendW = 0, legendH = 0;
    if (legend && typeof legend.getBoundingClientRect === "function") {
      try {
        const lRect = legend.getBoundingClientRect();
        legendW = Math.max(1, Math.ceil(lRect.width));
        legendH = Math.max(1, Math.ceil(lRect.height));
        const lClone = legend.cloneNode(true);
        const xhtml = document.createElement("div");
        xhtml.setAttribute("xmlns", "http://www.w3.org/1999/xhtml");
        xhtml.style.background = "#ffffff";
        xhtml.appendChild(lClone);
        const fo = document.createElementNS("http://www.w3.org/2000/svg", "foreignObject");
        fo.setAttribute("x", "0"); fo.setAttribute("y", "0");
        fo.setAttribute("width", String(legendW));
        fo.setAttribute("height", String(legendH));
        fo.appendChild(xhtml);
        const svgEnv = document.createElementNS("http://www.w3.org/2000/svg", "svg");
        svgEnv.setAttribute("xmlns", "http://www.w3.org/2000/svg");
        svgEnv.setAttribute("width", String(legendW));
        svgEnv.setAttribute("height", String(legendH));
        svgEnv.appendChild(fo);
        const lXml = new XMLSerializer().serializeToString(svgEnv);
        const lBlob = new Blob([lXml], { type: "image/svg+xml;charset=utf-8" });
        legendUrl = URL.createObjectURL(lBlob);
      } catch (e) {
        console.warn("Composite PNG: legend rasterise prep failed, exporting chart only:", e);
        legendUrl = null;
      }
    }

    // Load images. Legend may fail at decode time → fall back to chart-only.
    const chartImg = await loadImg(chartUrl);
    let legendImg = null;
    if (legendUrl) {
      try { legendImg = await loadImg(legendUrl); }
      catch (e) { console.warn("Composite PNG: legend image decode failed, exporting chart only:", e); legendImg = null; }
    }

    // Composite onto a single canvas.
    const compW = Math.max(chartW, legendW);
    const compH = chartH + (legendImg ? legendH : 0);
    const canvas = document.createElement("canvas");
    canvas.width  = compW * dpr;
    canvas.height = compH * dpr;
    const ctx = canvas.getContext("2d");
    ctx.fillStyle = "#ffffff";
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.drawImage(chartImg, 0, 0, chartW * dpr, chartH * dpr);
    if (legendImg) ctx.drawImage(legendImg, 0, chartH * dpr, legendW * dpr, legendH * dpr);

    URL.revokeObjectURL(chartUrl);
    if (legendUrl) URL.revokeObjectURL(legendUrl);

    return await new Promise((resolve, reject) => {
      canvas.toBlob((b) => b ? resolve(b) : reject(new Error("canvas.toBlob returned null (canvas tainted?)")), "image/png");
    });
  };
  ...
  const chartAndLegendWithDownload = chartDownloadMenu(chartAndLegend, {
    ...
    pngOverride: compositeChartLegendPng,
  });
...
cogURL_for_obs = ({ variable, season, clim = "1991-2020", stat = "mean" }) => {
  ...
  // metadata. Brayden's 2026-05-21 re-bake moved the physical
  // directory from `period=AMJ` (the old random-token workaround) to
