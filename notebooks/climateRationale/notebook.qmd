...
dbHazardExposure = {
  ...
  const _cleaned = data_obj
    .filter((d) => d.key === "hazard_exposure")
    .map((item) => ({ ...item, s3_path: patchWindowsCache(item.s3_path) }));
...
summary_futureProjections = () => {
  ...
  const summary = [];
  // Baseline row per admin: 1995–2014 mean ± interannual σ. In anomaly
  // mode the centre is zero by definition; in absolute mode it sits at the
  // per-admin baselineMean. Rendered as a distinct grey whisker above the
  // SSP rows so the future projections can be compared to "where the
  // recent past sat" inside the same chart.
