...
observed_obs_raw = {
  ...
  const baseURL    = useAdm1 ? monthlyAdm1URL : monthlyAdm0URL;
  const periodsURL = useAdm1 ? periodsAdm1URL : periodsAdm0URL;
  const variable = climateVarSelect.id;
  const season = seasonSelect.season;

  // Build SELECT'd adminName: literal national label when no admin1,
  // else admin1_name column. SQL-quote admin1 names defensively.
  const adminSelect = useAdm1
    ? "admin1_name AS adminName"
    : `'${obsAdmin0Name.replace(/'/g, "''")}' AS adminName`;
  const admin1Filter = useAdm1
...
baselines_obs = {
  ...
  for (const row of observed_obs_raw) {
...
adminNames_obs = {
  ...
  return [...new Set(observed_obs_raw.map((d) => d.adminName))];
}
...
recentChanges_obs = {
  ...
  for (const adminName of adminNames_obs) {
    ...
    if (isSPEI_obs) {
      ...
      bandsData.push(
        { adminName, y1: 1.5,        y2: topCap,    fill: _bandExtreme },
        { adminName, y1: bottomCap,  y2: -1.5,      fill: _bandExtreme },
        { adminName, y1: 1.0,        y2: 1.5,       fill: _bandUnusual },
        { adminName, y1: -1.5,       y2: -1.0,      fill: _bandUnusual },
      );
    } else {
      ...
      bandsData.push(
        { adminName, y1: mFb + 2 * base.sd, y2: topCap,                   fill: _bandExtreme },
        { adminName, y1: bottomCap,         y2: mFb - 2 * base.sd,        fill: _bandExtreme },
        { adminName, y1: mFb + base.sd,     y2: mFb + 2 * base.sd,        fill: _bandUnusual },
        { adminName, y1: mFb - 2 * base.sd, y2: mFb - base.sd,            fill: _bandUnusual },
      );
    }
  }
  ...
  const speiRuleData = isSPEI_obs
    ? adminNames_obs.flatMap((adminName) =>
        [-2, -1.5, -1, 0, 1, 1.5, 2].map((y) => ({ adminName, y })))
    : [];
  ...
  const baselineRuleData = isSPEI_obs ? [] : adminNames_obs
    .map((adminName) => {
      const base = baselines_obs.get(adminName);
      if (!base) return null;
      return { adminName, y: useAnomaly_obs ? 0 : base.mean };
    })
    .filter((r) => r != null);

  // Mark builders.
  const stripesMark = Plot.rect(observed_obs, {
    x1: (d) => d[xField] - stripeWidth/2,
    x2: (d) => d[xField] + stripeWidth/2,
    // Per-facet yMin / yMax — fall back to global if single facet.
    y1: (d) => d3.min(observed_obs.filter((x) => x.adminName === d.adminName), (x) => x.value_plot),
    y2: (d) => d3.max(observed_obs.filter((x) => x.adminName === d.adminName), (x) => x.value_plot),
    fill: stripeColour,
    stroke: null,
    fy: isFaceted ? "adminName" : undefined,
  ...
  // Hazard-band mark (per-facet via fy if faceted). Spans the full x
  // domain inside each facet.
  ...
  // SPEI reference lines as data-driven ruleY (per-facet via fy).
  ...
  const facetCount = isFaceted ? nFacets : 1;
  const perFacetHeight = isFaceted ? 200 : 380;
  const chart = Plot.plot({
    width: 1200,
    height: facetCount * perFacetHeight + (isFaceted ? 30 : 0),
    marginLeft: 70,
    marginBottom: 50,
    marginTop: 30,
    style: {
    ...
    x: {
      ...
      // tickSpacing controls density at default ticks (issue #7) — for
      // 45-year ranges we want ~9 ticks (every 5 years), and Plot's
      // tickSpacing arg accepts pixels-per-tick which scales with width.
      ticks: Math.ceil((xDomain[1] - xDomain[0]) / 5),
    },
    y: { label: yLabel, grid: true, nice: true },
    fy: isFaceted
      ? { label: null, padding: 0.08 }
      : undefined,
...
//| output: false
{
  ...
  [admin0Iso3, admin1Names, climateVarSelect, seasonSelect, monthly_obs, plotType_obs, anomaly_obs];
...
### Map — observational climatology (1991–2020 mean)
...
cogURL_for_obs = ({ variable, season, clim = "1991-2020", stat = "mean" }) => {
  ...
  // The path pattern is brittle until the pipeline fix lands (4 PTOT
  // COGs are dispatched as broken — see
...
recentChangesMap_obs = {
  ...
  const globalStops = ramps_obs[climateVarSelect.id] ?? ramps_obs.TAVG;
  ...
  // Download button — composites canvas + SVG overlay into one PNG.
  ...
  dlBtn.onclick = async () => {
    ...
    composite.toBlob((blob) => {
      ...
      a.download = `obs_${climateVarSelect.id}_${seasonSelect.season}_${mapStat_obs}_${slug}.png`;
      a.click();
  ...
  setStatus(`${climateVarSelect.id} ${seasonSelect.season} (${statLabel}) for ${renderLabel} · country fetch ${fetchSec}s · sub-window ${sw}×${sh} → ${W}×${H} · ${rangeStr} · ramp: ${rampLabel}`);
