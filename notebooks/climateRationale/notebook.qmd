...
import { chartDownloadMenu, chartDownloadButton, elementToPngBlob }   from "/helpers/chartDownloadMenu.ojs";
...
<style>
  ...
  .plot-footer-row > .plot-caption-details {
    margin: 0;
    /* flex-grow to fill remaining row width; min-width: 0 overrides the
       default min-width: auto so the details element can shrink below
       its intrinsic content size, allowing the body text to wrap rather
  ...
  /* Each paragraph inside an "About this plot" body becomes its own
     block so the text wraps within the page-content width rather than
     extending past the chart's SVG width. multiLineText produces
     inline spans with <br> separators (so the captions can be used as
     flow-text elsewhere), so we collapse the <br>s here and override
     the inline default to block-with-margin. */
  .plot-caption-body .atlasFigCaption {
    display: block;
    margin-bottom: 0.4em;
    max-width: 100%;
  }
  .plot-caption-body .atlasFigCaption + br {
    display: none;
  }
  ...
  /* Inline help disclosure shown in the byproducts-toggle slot when the
     toggle is hidden because the active variable is FAOSTAT VoP (which
     has no processed forms by methodology). Sits in the controls row,
     compact when closed. */
  details.byproducts-vop-hint {
    font-size: 0.82em;
    color: #6b4d1a;
    max-width: 26em;
    margin: 0;
  }
  details.byproducts-vop-hint > summary {
    cursor: pointer;
    list-style: none;
    padding: 0;
    color: #2a7a2a;
  ...
  details.byproducts-vop-hint > summary::-webkit-details-marker {
    display: none;
  }
  details.byproducts-vop-hint > summary::before {
    content: "▸ ";
    display: inline-block;
    color: #2a7a2a;
  }
  details.byproducts-vop-hint[open] > summary::before {
    content: "▾ ";
  }
  details.byproducts-vop-hint > .byproducts-vop-hint-body {
    margin-top: 0.4rem;
    line-height: 1.4;
    color: #555;
  }
  details.byproducts-vop-hint > .byproducts-vop-hint-body p {
    margin: 0;
  }
  ...
  /* Hide the inter-model ribbon toggle when Future Projections is in
  ...
  /* Hazard exposure "Advanced controls" disclosure — neutral settings-panel
     look, distinct from the blue alert-style help callouts above. Reuses
     help-callout's chevron behaviour without the alert-info colouring. */
  details.advanced-hazard-controls {
  ...
  details.advanced-hazard-controls > summary {
    cursor: pointer;
    font-weight: 600;
    list-style: none;
    color: #444;
  ...
  details.advanced-hazard-controls > summary::marker,
  details.advanced-hazard-controls > summary::-webkit-details-marker {
    display: none;
  }
  details.advanced-hazard-controls > summary::before {
    content: "▸";
    display: inline-block;
    width: 1.1em;
    transition: transform 0.15s;
  }
  details.advanced-hazard-controls[open] > summary::before {
    transform: rotate(90deg);
  }
  details.advanced-hazard-controls[open] > summary {
    margin-bottom: 0.5rem;
  }
  details.advanced-hazard-controls .advanced-hazard-help {
    font-size: 0.85rem;
    color: #555;
    margin: 0 0 0.65rem;
    line-height: 1.4;
  }
  details.advanced-hazard-controls .advanced-hazard-row {
    display: flex;
    flex-wrap: wrap;
    gap: 1.5rem;
    align-items: flex-end;
  }
  /* Force each Inputs.select to size to its content rather than stretch
     to the full row width. Observable's Inputs CSS sets max-width: 640px
     and width: 100% on the wrapping <form>, plus min-width: 240px on the
     <select> itself. Need to override BOTH so two selects fit side-by-side
     on the same flex row. */
  details.advanced-hazard-controls .advanced-hazard-row > form {
    flex: 0 0 auto;
    width: auto !important;
  ...
  details.advanced-hazard-controls .advanced-hazard-row > form > label {
    width: auto;
    margin-right: 0;
  }
  details.advanced-hazard-controls .advanced-hazard-row > form select {
    min-width: 240px;
    width: auto;
  }

  /* Contextual warning shown only when the user picks the "Generic
     thresholds" option from the Advanced-controls dropdown. Sits between
     the controls and the chart so it stays in view while the user reads
     the (potentially misleading) bars. Amber/yellow alert palette to read
     as a caution, not a hard error. */
  .generic-threshold-warning {
  ...
  .generic-threshold-warning-title {
    font-weight: 700;
    margin-bottom: 0.3rem;
  }
  .generic-threshold-warning-body {
    font-size: 0.9rem;
    line-height: 1.45;
  }
  .generic-threshold-warning-body p {
    margin: 0;
  }
  .generic-threshold-warning-body code {
...
```{ojs}
globalA0 = renderA0Multi({ maxSelections: 1, requireAtLeastOne: true });
...
```{ojs}
// Three Key-Facts charts (poverty + GDP + land use) in ONE wrapper so a
// single image download can capture all three. renderToDiv below still
// targets each inner div by id (getElementById finds nested nodes).
keyFactsCharts = html`<div>
  ${loaderDiv("plotPov")}
  ${loaderDiv("plotGdp")}
...
captionDetails(
  ...
  chartDownloadButton(keyFactsCharts, { filename: "keyFacts", data: keyFacts_combinedData })
);
```
...
```{ojs}
// Hint shown in the slot where the byproducts toggle would be when
// it's hidden for VoP variables. Surfaces the WHY at the moment of
// confusion — users who look for the toggle and see nothing get a
// brief explanation + link to Methods. Scoped to VoP only because
// the physical-quantity case (production/yield/export_quantity/
// import_quantity) is already explained in those variables' own
// description text. See dispatches/2026-05-25_faostat-trade-data-audit.md
// "Aside — why no byproducts in VoP".
{
  const isVoP = productionVar.id === "vop_intd15" ||
                productionVar.id === "vop_usd15";
  if (!isVoP) return html``;
  return html`<details class="byproducts-vop-hint">
    <summary>Why no byproducts for Value of production?</summary>
    <div class="byproducts-vop-hint-body">${md`${_lang(nbText.sections.productionTrends.byproductsVoPHint)}`}</div>
  </details>`;
}
```
...
viewof productionYearStart = Inputs.range(
  ...
  {
    step: 1,
    // F-4 (dispatch 2026-05-25_faostat-trade-data-audit.md) — soft default
    // of 2015 for trade variables (sidesteps pre-2015 reporter-country
    // anomalies + aligns with FAOSTAT's deflator reference period); 2010
    // for everything else. Slider still allows dragging earlier.
    value: Math.max(
      productionAvailableYears.min,
      /^(export|import)/.test(productionVar.id) ? 2015 : 2010
    ),
    label: "From year",
  },
);
...
viewof anomaly_obs = {
  // Hide the toggle when SPEI is selected — SPEI is already a
  // standardised z-score against its own baseline, so "show as anomaly"
  // has no meaning. The toggle's value is still consumed downstream as
  // `useAnomaly_obs = anomaly_obs && !isSPEI_obs` so the chart code
  // path is unaffected. We hide the entire Quarto cell wrapper (not
  // just the toggle) so the cols-3 grid reflows and the remaining
  // controls fill the row, rather than leaving an empty slot.
  const isSPEI = (climateVarSelect.id || "").startsWith("SPEI");
  const toggle = Inputs.toggle({ label: `Show as anomaly (vs ${baselinePeriod_obs})`, value: true });
  // setTimeout(0) rather than queueMicrotask — the toggle isn't in the
  // DOM yet when this cell evaluates; Observable inserts it after the
  // current synchronous task, so we have to defer past that.
  setTimeout(() => {
    const cell = toggle.closest('.cell');
    if (cell) cell.style.display = isSPEI ? 'none' : '';
  }, 0);
  return toggle;
}
```
...
```{ojs}
// Baseline period for the observed-record anomaly framing. WMO's
// standard climatological normal is 1991-2020, used everywhere by
// default. The Atlas's CMIP6 future-projection products are calibrated
// against 1995-2014, however, so a "matches Future Projections"
// option is offered for users who want a like-for-like comparison
// between observed anomalies and projected anomalies. Hidden for SPEI
// — the SPEI values in the parquet are already standardised against
// their own internal reference window, so an external baseline-period
// choice doesn't apply.
viewof baselinePeriod_obs = {
  const isSPEI = (climateVarSelect.id || "").startsWith("SPEI");
  const select = Inputs.select(["1991-2020", "1995-2014"], {
    label: "Baseline period (anomalies)",
    value: "1991-2020",
  });
  setTimeout(() => {
...
```{ojs}
//| output: false
// Parse the selected baseline period string ("YYYY-YYYY") into a
// {start, end} year range used by baselines_obs filter + all dynamic
// "(period)" labels in the chart, table, and caption.
baselineYears_obs = {
  const [start, end] = baselinePeriod_obs.split("-").map(Number);
  return { start, end };
}
```
...
viewof monthly_obs = {
  ...
  // SPEI pins to a single month per year (the season's last month), so a
  // "monthly resolution" toggle has no meaning. Hide the entire Quarto
  // cell wrapper (not just the toggle) so the cols-3 grid reflows and
  // the remaining controls fill the row.
  // setTimeout(0) rather than queueMicrotask — the toggle isn't in the
  // DOM yet when this cell evaluates; Observable inserts it after the
  // current synchronous task, so we have to defer past that.
  setTimeout(() => {
...
viewof showObsUncertainty_obs = {
  // Hide for SPEI — the uncertainty band reflects CHIRPS / CHIRTS-ERA5
  // sampling spread on raw variables; SPEI is a derived index where
  // calibration-window choice dominates the uncertainty story, not
  // source-dataset sampling. See the "About SPEI in this section"
  // callout for the user-facing explanation.
  const isSPEI = (climateVarSelect.id || "").startsWith("SPEI");
  const toggle = Inputs.toggle({ label: "Show observational uncertainty band (indicative)", value: false });
  setTimeout(() => {
    const cell = toggle.closest('.cell');
    if (cell) cell.style.display = isSPEI ? 'none' : '';
  }, 0);
...
// admin0Iso3 is an array of length 0 or 1 (renderA0Multi maxSelections=1
// per the CR-089/L7 cold-start perf fix). Observational query is single-
// country; take the first. Same fallback name lookup.
...
observed_obs_raw = {
  ...
  const rows = Array.from(await dbObservational.query(buildQuery(useAdm1 ? "adm1" : "adm0")));
  let nQueries = 1;
  ...
  if (useAdm1 && includeAdmin0) {
    const nationalRows = Array.from(await dbObservational.query(buildQuery("adm0")));
...
// Per-admin baseline {mean, sd, n} keyed by adminName. The window is
// user-selectable via viewof baselinePeriod_obs (defaults to the WMO
// 1991-2020 standard normal; 1995-2014 also offered to match the
// Atlas's CMIP6 future-projection products). Computed independently
// per admin1 so each facet's hazard bands + z-scores reference the
// right baseline.
baselines_obs = {
  const byAdmin = new Map();
  const { start, end } = baselineYears_obs;
...
recentChanges_obs = {
    ...
    return html`<div>
      ${dataTable(observed_obs, {
        ...
        header: {
          ...
          z:           `z (σ vs ${baselinePeriod_obs} baseline)`,
          cls:         "Classification",
        },
  ...
  if (isSPEI_obs) {
    ...
    dataLayerMarks = [
      // Use Plot.rect with explicit x1/x2 + y1: 0 / y2: value — matches
      // the non-SPEI barMark pattern. Plot.barY with quantitative x
      // (year as integer) renders zero-width bars in Plot 0.6.13+
      // because the auto bandwidth calc collapses to the data step.
      // Explicit rect bounds avoid the issue entirely.
      Plot.rect(observed_obs_aug, {
        x1: (d) => d[xField] - stripeWidth * 0.4,
        x2: (d) => d[xField] + stripeWidth * 0.4,
        y1: 0,
        y2: "value_plot",
  ...
  // SPEI included: although SPEI is already a standardised z-score, a
  // monotonic trend in drought severity is still meaningful (e.g.
  // increasing dry / decreasing wet decades) and the Theil-Sen line
  // makes that direction visible at a glance.
  ...
  const trendMarks = (showTrend_obs && trendStats && !trendStats.error &&
                      (isSPEI_obs || plotType_obs === "line" || plotType_obs === "line+stripes" || plotType_obs === "bars"))
  ...
  const showsTrendOverlay = showTrend_obs && trendStats && !trendStats.error &&
                            (isSPEI_obs || plotType_obs === "line" || plotType_obs === "line+stripes" || plotType_obs === "bars");
  ...
  if (showsBaselineRule) {
    legendItems.push({ kind: "rule", color: "#4682b4", dasharray: "3 2", label: `${baselinePeriod_obs} baseline mean` });
  }
  if (isSPEI_obs) {
  ...
  // legend together in a single <div>; chartDownloadButton auto-detects
  // the multi-element wrapper and routes PNG/SVG through html-to-image
  // (compositePng / compositeSvg), so the LEGEND is captured in the
  // export alongside the chart. (Earlier foreignObject attempts that
  // tainted the canvas were superseded by the html-to-image path.)
  ...
  // The download split-button moves into the captionDetails plot-footer-row
  // (inline with "About this plot") rather than sitting in its own row
  // beneath the chart.
  const downloadBtn = chartDownloadButton(chartAndLegend, {
    filename,
    data: observed_obs,
  ...
  // "About this plot" disclosure — methods and reading guidance moved
  // from the static callouts that used to live above the controls.
  // For SPEI, an extra section explains why several controls disappear
  // and what to expect of the trend line + zone shading.
  const plotCaptionParas = [
    html`Values are shown as <strong>anomalies</strong> relative to the <strong>${baselinePeriod_obs.replace("-", "–")}</strong> baseline (${baselinePeriod_obs === "1991-2020" ? "WMO standard climatological normal" : "matches the Atlas's CMIP6 future-projection calibration window"}). The zero line represents the ${baselinePeriod_obs.replace("-", "–")} average for the selected season and variable. Values above zero indicate years wetter / warmer than the baseline; values below zero indicate the opposite. Use the <em>Baseline period</em> selector above the chart to switch between the two windows — the Atlas-convention 1995–2014 is useful when comparing observed anomalies directly against Future Projections, which are calibrated against the same window.`,
    html`The optional <strong>trend layer</strong> overlays a Mann-Kendall significance test + Theil-Sen slope estimator (Sen-slope) on the annual series. TFPW (trend-free pre-whitening) is applied automatically when the residual series shows positive autocorrelation, to avoid inflating the test against AR(1) noise.`,
  ];
  if (isSPEI_obs) {
    plotCaptionParas.push(
      html`<strong>About SPEI in this section.</strong> SPEI is the Standardised Precipitation–Evapotranspiration Index — already a z-score against its own 1991–2020 reference window. That means several of the standard controls don't apply: <em>Show as anomaly</em> is hidden (SPEI values <em>are</em> anomalies by construction), <em>Monthly view</em> is hidden (SPEI is a rolling-window accumulation pinned to a single month per year), and the <em>Observational uncertainty band</em> is hidden (calibration-window choice dominates SPEI's uncertainty, not source-dataset sampling). The shaded ±1, ±1.5, ±2 reference lines mark the WMO drought / wet categories.`,
    );
  }
  plotCaptionParas.push(
    html`Source: <a href="#methods-climate-data">Methods → Climate data sources</a> and <a href="#methods-trend-estimation">Methods → Trend estimation</a> for the underlying products, baselines, and statistical caveats.`,
  );
  const plotCaption = multiLineText(plotCaptionParas, "atlasFigCaption");

  return html`<div>${status}${chartAndLegend}${captionDetails(plotCaption, undefined, downloadBtn)}${trendBadge}${ipccQualifier}</div>`;
}
...
recentChanges_obs_caption = {
  ...
  if (adminNames_obs.length === 1) {
    ...
    baselineCopy = (b && b.n > 0)
      ? `Baseline (${baselinePeriod_obs}): μ = ${b.mean.toFixed(2)}, σ = ${b.sd.toFixed(2)}, n = ${b.n} years.`
      : `Baseline (${baselinePeriod_obs}): insufficient data.`;
  } else {
    ...
    baselineCopy = `Per-admin baselines (${baselinePeriod_obs}): ${lines.join(" · ")}.`;
  }
  ...
  return md`${baselineCopy} ${speiCopy}`;
}
```
...
```{ojs}
// Classify the continuous ramp into N equal-interval value ranges →
// stepped / banded (choropleth-style) map instead of a smooth gradient.
// "Continuous" keeps the existing smooth interpolation. Classification
// snaps each pixel to its class-centre colour and switches the legend
// to discrete blocks with break labels. Equal-interval is computed over
// the ACTIVE ramp domain, so it respects the "Lock map ramp" /
// dynamic-rescale toggle above.
viewof mapClasses_obs = Inputs.select(
  ["Continuous (smooth ramp)", "3", "4", "5", "6", "7", "8", "10"],
  { label: "Classify into value ranges (stepped map)", value: "Continuous (smooth ramp)" }
);
```
...
admin0_feature_obs = {
  ...
  // A country can ship as several admin0 units sharing one iso3 —
  // disputed regions carry the host country's iso3 as their own gaul0
  // unit (CR-115 option2 convention: KEN = Kenya + Ilemi Triangle;
  // also EGY, SDN, SSD). Merge all matches into one MultiPolygon so
  // the COG fetch bbox and mask cover the whole country, not just
  // whichever unit happens to sit first in the topojson.
  const matches = admin0_geo_obs.features.filter((f) => f.properties && (f.properties.iso3 === iso3 || f.properties.ISO3 === iso3 || f.properties.iso_3 === iso3));
  ...
  const primary = matches.reduce((a, b) =>
    (b.properties.gaul0_code ?? 0) > (a.properties.gaul0_code ?? 0) ? b : a
  );
  return {
    type: "Feature",
...
buildLegend_obs = (stops, variable, stat, classInfo) => {
  const W = 600;
  const barH = 14;
  ...
  const fmtVal = (v) => Math.abs(v) >= 100 ? Math.round(v) : (Math.abs(v) >= 10 ? v.toFixed(0) : v.toFixed(1));

  let defsMarkup = "";
  let barMarkup;
  let ticks;
  if (classInfo) {
    // Stepped legend: one discrete block per class + N+1 equal-interval
    // break labels (matches the map's class boundaries exactly).
    const { nClasses, lo, hi, colors } = classInfo;
    const span = (hi - lo) || 1;
    const blockW = (W - 2 * padX) / nClasses;
    barMarkup = colors.map((c, k) =>
      `<rect x="${(padX + k * blockW).toFixed(2)}" y="${padTop}" width="${blockW.toFixed(2)}" height="${barH}" fill="rgb(${c.slice(0, 3).join(",")})" stroke="#333" stroke-width="0.4"/>`
    ).join("");
    ticks = Array.from({ length: nClasses + 1 }, (_, k) => {
      const v = lo + (k / nClasses) * span;
      const x = padX + k * blockW;
      return `
        <line x1="${x.toFixed(2)}" x2="${x.toFixed(2)}" y1="${padTop}" y2="${padTop + barH + 3}" stroke="#333" stroke-width="0.8"/>
        <text x="${x.toFixed(2)}" y="${padTop + barH + 16}" font-size="10" fill="#333" text-anchor="middle" font-family="system-ui, sans-serif">${fmtVal(v)}</text>
      `;
    }).join("");
  } else {
    ...
    defsMarkup = `<defs><linearGradient id="${gradientId}" x1="0" x2="1" y1="0" y2="0">${gradientStops}</linearGradient></defs>`;
    barMarkup = `<rect x="${padX}" y="${padTop}" width="${W - 2 * padX}" height="${barH}" fill="url(#${gradientId})" stroke="#333" stroke-width="0.6"/>`;
    ticks = stops.map((s) => {
      const x = xForVal(s.at);
      return `
        <line x1="${x}" x2="${x}" y1="${padTop}" y2="${padTop + barH + 3}" stroke="#333" stroke-width="0.8"/>
        <text x="${x}" y="${padTop + barH + 16}" font-size="10" fill="#333" text-anchor="middle" font-family="system-ui, sans-serif">${fmtVal(s.at)}</text>
      `;
    }).join("");
  }

  const unit = variable === "PTOT" ? "mm" : variable.startsWith("SPEI") ? (stat === "sd" ? "σ (z)" : "z") : "°C";
  const titleText = variable.startsWith("SPEI") && stat === "sd"
    ? `${variable} — interannual variability (σ across 1991–2020, z-score units)`
    : `${variable} (${stat}) — ${unit}`;
...
recentChangesMap_obs = {
  ...
  // Optional classification: snap the continuous ramp to N equal-
  // interval value ranges for a stepped (choropleth-style) map. Domain
  // is the ACTIVE ramp extent, so classes track the lock/dynamic toggle.
  // pixelColor() is the single colour accessor used by BOTH paint paths
  // and the legend below — continuous falls through to colorRamp_obs.
  const nClasses_obs =
    (mapClasses_obs && mapClasses_obs !== "Continuous (smooth ramp)")
      ? +mapClasses_obs : null;
  const rampLo = activeStops[0].at;
  const rampHi = activeStops[activeStops.length - 1].at;
  let classInfo = null;
  let pixelColor = (v) => colorRamp_obs(v, activeStops);
  if (nClasses_obs) {
    const span = (rampHi - rampLo) || 1;
    // One representative colour per class, sampled at the class centre.
    const classColors = [];
    for (let k = 0; k < nClasses_obs; k++) {
      const t = (k + 0.5) / nClasses_obs;
      classColors.push(colorRamp_obs(rampLo + t * span, activeStops));
    }
    pixelColor = (v) => {
      if (v == null || !isFinite(v)) return null;
      let k = Math.floor((v - rampLo) / span * nClasses_obs);
      if (k < 0) k = 0;
      if (k >= nClasses_obs) k = nClasses_obs - 1;
      return classColors[k];
    };
    classInfo = { nClasses: nClasses_obs, lo: rampLo, hi: rampHi, colors: classColors };
    rampLabel += `, ${nClasses_obs} classes`;
  }
  ...
  const legend = buildLegend_obs(activeStops, climateVarSelect.id, mapStat_obs, classInfo);
  ...
  const statLabel = mapStat_obs === "mean"
    ? "1991-2020 mean"
    : mapStat_obs === "sd"
      ? "1991-2020 interannual variability"
      : `1991-2020 ${mapStat_obs}`;
  ...
  // Download button below the map, styled identically to the chart's
  // split-button. The map is a composite (canvas raster + SVG boundary
  // overlay + HTML legend), so we pass the whole `mapAndLegend` wrapper
  // and let chartDownloadButton auto-detect it: PNG routes through
  // html-to-image (captures canvas + overlay + legend in one pass) and
  // SVG through compositeSvg (foreignObject envelope). This matches the
  // sandbox download path and includes the legend — replacing the older
  // manual canvas-composite that excluded the legend and disabled SVG.
  ...
  // Split-button moves into the captionDetails plot-footer-row alongside
  // the "About this plot" disclosure.
  const downloadBtn = chartDownloadButton(mapAndLegend, { filename });

  // "About this plot" — methods notes that used to live in a static
  // callout above the map. Same format as the chart captions elsewhere
  // in the notebook (multiLineText + captionDetails). SPEI gets an
  // extra paragraph explaining the SD-not-mean choice.
  const mapCaptionParas = [
    html`Source data: <strong>CHIRPS v3</strong> for precipitation and <strong>CHIRTS-ERA5</strong> for temperature, downsampled to a Cloud-Optimized GeoTIFF (COG) of the 1991–2020 mean per (variable × season). Boundary polygons are GAUL 2024 admin1 simplified to a vector tile-friendly resolution.`,
    html`See <a href="#methods-climate-data">Methods → Climate data sources</a> and <a href="#methods-observational-uncertainty">Methods → Observational uncertainty</a> for product details and known caveats.`,
  ];
  if (isSPEI_obs) {
    mapCaptionParas.unshift(
      html`For SPEI the map shows the <em>standard deviation</em> of the annual SPEI series over 1991–2020 (the SPEI mean is ~0 by construction, since SPEI is standardised against its own reference window). Darker reds mark regions where SPEI swings more widely across years — i.e. more variable drought conditions.`,
    );
  }
  const mapCaption = multiLineText(mapCaptionParas, "atlasFigCaption");

  return html`<div>${status}${mapAndLegend}${captionDetails(mapCaption, undefined, downloadBtn)}</div>`;
}
```
...
<a id="whyTwoDatasets"></a>

## `{ojs} _lang(nbText.sections.whyTwoDatasets.heading)`
...
<a id="futureProjections-anchor"></a>
...
```{ojs}
Inputs.bind(
  Inputs.select(futureHazards, {
    format: (d) => _lang(d.name),
    label: _lang(general_translations.climateVar),
  }),
  viewof climateVarSelectFuture,
);
```
...
viewof showFutureUncertainty = (() => {
  const el = Inputs.toggle({ label: "Show inter-model 17–83% range", value: true });
...
// view-incompatible controls (currently: the inter-model ribbon toggle when
...
//| output: false
{
  ...
  // must not trip the loader (race with render cell). The visibility
  // flag is included so the loader updates from "Waiting for scroll…"
  // to "Loading data…" the moment the section-gate flips.
  [admin0Iso3, admin1Names, climateVarSelectFuture, seasonSelect, futurePeriodSelect, futureScenarioSelect, futureProjectionsVisible];
  const div = document.getElementById("plotFutureProjections");
  const stage = futureProjectionsVisible ? "Loading data…" : "Waiting for scroll…";
  if (div) div.replaceChildren(loaderContent(stage));
}
```
...
//| output: false
{
  renderToDiv("plotFutureProjections", () => {
    // While section-gated (user hasn't scrolled near Future Projections
    // yet), futureProjections_dataAll returns []. Without this guard,
    // the chart cells fall through to noDataPlaceholder() — which reads
    // to the user as "the data is empty" rather than "still loading".
    // Keep showing the loader until the gate flips.
    if (!futureProjectionsVisible) return loaderContent("Waiting for scroll…");
...
${
  (climateVarSelectFuture.tails ?? "both") === "high-only"
...
//| output: false
{
  // Visibility flag bundled so the loader stage updates the moment the
  // futureProjections gate flips (same upstream as Future Projections).
  [admin0Iso3, admin1Names, climateVarSelectFuture, seasonSelect, futurePeriodSelect, futureScenarioSelect, futureProjectionsVisible];
...
//| output: false
{
  renderToDiv("plotExtremeEvents", () => {
    // Extreme Events derives from futureProjections_data, which is
    // gated on futureProjectionsVisible. Same loader-vs-no-data
    // disambiguation as the Future Projections renderToDiv above.
    if (!futureProjectionsVisible) return loaderContent("Waiting for scroll…");
...
```{ojs}
// Advanced hazard-exposure controls — collapsed by default. Folds two
// power-user toggles below the standard Production-Type / View-Type row:
//   - Severity tier: which day-count threshold to count as "exposed".
//     Only `severe` is currently on S3; `moderate` / `extreme` are listed
//     so the UI affordance is visible but explicit about being pending
//     pipeline publish.
//   - Threshold definition: generic (uniform across crops) vs FAO Ecocrop
//     crop-specific (per-crop precipitation + heat-for-crops thresholds).
//     Both composites are already in the canonical parquet — fully wired.
// Wraps a combined Inputs.form in a <details> so the compound viewof can
// expose {severity, style} to downstream cells in one reactive value.
viewof advancedHazardControls = (() => {
  const tierKeys = ["severe", "moderate", "extreme"];
  const styleKeys = ["generic", "ecocrop"];
  const a = nbText.sections.hazardExposure.advancedControls;
  // Build the two selects independently (not via Inputs.form) so the
  // wrapper layout is under our control — putting them on a single flex
  // row instead of the stacked form-element default.
  const severityInput = Inputs.select(tierKeys, {
    label: _lang(a.severityLabel),
    value: "severe",
    format: (d) => _lang(a.severityOptions[d]),
  });
  const styleInput = Inputs.select(styleKeys, {
    label: _lang(a.styleLabel),
    value: "ecocrop",
    format: (d) => _lang(a.styleOptions[d]),
  });
  const wrapper = html`<details class="advanced-hazard-controls">
    <summary>${_lang(a.summary)}</summary>
    <div class="advanced-hazard-help">${md`${_lang(a.help)}`}</div>
    <div class="advanced-hazard-row">${severityInput}${styleInput}</div>
  </details>`;
  Object.defineProperty(wrapper, "value", {
    get: () => ({ severity: severityInput.value, style: styleInput.value }),
  });
  const bubble = () =>
    wrapper.dispatchEvent(new Event("input", { bubbles: true }));
  severityInput.addEventListener("input", bubble);
  styleInput.addEventListener("input", bubble);
  return wrapper;
})();
```

```{ojs}
hazardSeverity = advancedHazardControls.severity;
hazardThresholdStyle = advancedHazardControls.style;
```

```{ojs}
// Contextual warning: renders only when the user has actively switched
// the Threshold definition to "Generic" (which currently has the upstream
// historic-vs-future categorisation bug). Stays visible while reading
// the chart so the suspect data is flagged in-context. Returns an empty
// node when Ecocrop (the default) is active.
{
  if (hazardThresholdStyle !== "generic") return html`<div></div>`;
  const w = nbText.sections.hazardExposure.genericWarning;
  return html`<div class="alert alert-warning generic-threshold-warning" role="alert">
    <div class="generic-threshold-warning-title">⚠ ${_lang(w.title)}</div>
    <div class="generic-threshold-warning-body">${md`${_lang(w.body)}`}</div>
  </div>`;
}
```
...
//| output: false
{
  ...
  // not trip the loader (race with render cell). Visibility flag bundled
  // so the loader stage updates the moment the gate flips.
  [admin0Iso3, admin1Names, futurePeriodSelect, futureScenarioSelect, prod_type, hazardExposureVisible, hazardSeverity, hazardThresholdStyle];
  const div = document.getElementById("plotHazardExposure");
  const stage = hazardExposureVisible ? "Loading data…" : "Waiting for scroll…";
  if (div) div.replaceChildren(loaderContent(stage));
}
```
...
//| output: false
{
  renderToDiv("plotHazardExposure", () => {
    // While section-gated (user hasn't scrolled near Hazard Exposure
    // yet), hazardExposure_dataAll returns []. Show the loader rather
    // than fall through to "No data available" downstream.
    if (!hazardExposureVisible) return loaderContent("Waiting for scroll…");
...
// Per-admin standard deviation of the 1995–2014 baseline `mean` for the
// CURRENT Future Projections variable, used to draw z-score threshold
// lines (|z| = 1 unusual, |z| = 2 extreme) on the anomaly plots. Must
// filter on climateVarSelectFuture.id (not climateVarSelect.id) — the
// Future Projections selector is independent of the Recent Changes one,
// and a mismatch leaks another hazard's SD into both the threshold
// lines and the y-axis expansion (e.g. TAVG ribbon appears flatlined at
// 0 °C because the axis got blown out to ±2σ of PTOT ≈ ±160 mm).
baselineStdByAdmin = d3.rollup(
  recentChanges_data.filter((d) => d.hazard === climateVarSelectFuture.id),
...
yTickWithUnit = (d) => {
  const u = _lang(climateVarSelectFuture.unit);
...
// iso3 → English admin0_name (e.g. "AGO" → "Angola"). Lets DuckDB
// queries drop `admin0_name` from their SELECT lists — the value is a
// small fixed per-iso3 string already shipped in atlas_countries.json,
// so fetching it from a multi-MB parquet column chunk on every cold
// query is wasted bandwidth. Consumers attach the name in JS after
// the query returns.
iso3ToAdmin0Name = new Map(
  adminNames.map(({ iso3c, admin0_name }) => [iso3c, admin0_name]),
);
```
...
// read_parquet(URL) directly via `dbObservational` — no view to
// register, just a dedicated client so the URL queries don't queue
// behind whatever else is running.
...
//
// CR-089 fix: this was an L2-shape GROUP-BY query against the ~50 MB
// adm1_obs.parquet at page load — 15-30 s of cold-start cost for a
// 53-row result that never changes within a session. Now loaded from
// a precomputed static JSON. Regenerate with
// `scripts/precompute_mainGaul.sh` when the GAUL boundaries change.
mainGaul = {
  const obj = await FileAttachment("/data/climateRationale/mainGaul.json").json();
  const map = new Map();
  for (const [iso3, gaul0_code] of Object.entries(obj)) map.set(iso3, gaul0_code);
  return map;
}

// Per-key DuckDBClient factory. generateDB() creates one DuckDBClient
// with a CREATE VIEW for every entry passed in; passing a single-entry
// array gives that parquet its own dedicated client (and its own
// connection — DuckDB-WASM serialises queries within a connection, so
// per-plot clients let Key Facts queries fire in parallel instead of
// queueing behind the slow exposure scan).
singleDB = (key) => {
  const entry = data_obj.find((d) => d.key === key);
  if (!entry) return null;
  return generateDB([{ ...entry, s3_path: patchWindowsCache(entry.s3_path) }]);
};

// One client per parquet so the queries don't serialise behind each
// other on a single DuckDB-WASM connection. Before this split, every
// first-paint query queued behind crop-livestock_all (the exposure
// parquet, ~85 s to drain on cold start due to absent row-group stats
// — same upstream issue as futureProjections per
// dispatches/2026-05-27_parquet-pushdown-pipeline-ask.md). Recent
// Changes + Production Trends were similarly stuck behind the climate
// timeseries fetch.
dbPov              = singleDB("poverty");
dbGdp              = singleDB("a0_gdp");
dbLanduse          = singleDB("a0_landuse");
dbExposure         = singleDB("exposure");
dbRecentChanges    = singleDB("historic_climate_timeseries");
dbProductionTrends = singleDB("production_timeseries");

// Dedicated client for the lifted Recent Changes observational query
// (adm0_obs / adm1_obs / monthly variants). These cells use
// `FROM read_parquet('<URL>')` directly so there's no view to register
// — we just need an isolated DuckDBClient so the query doesn't queue
// behind dbRecentChanges' historic_climate_timeseries scan.
dbObservational = DuckDBClient.of();

dbHazardExposure = {
  // Path B-2: gate the hazard_exposure parquet registration on the
  // section's visibility. The hazard_exposure parquet is the largest
  // single file in the notebook (~60 M rows); its footer + initial
  // column-chunks accounted for ~6 fetches on initial paint before
  // this gate. The sole consumer (hazardExposure_dataAll) already
  // returns [] while !hazardExposureVisible, so the sentinel is
  // belt-and-braces.
  //
  // CR-091: three severity-tier parquets are now on S3. Pick the path
  // matching the current hazardSeverity from s3_paths; fall back to
  // the legacy s3_path field for forward-compatibility.
  if (!hazardExposureVisible) return { query: async () => [] };
  const _item = data_obj.find((d) => d.key === "hazard_exposure");
  const _path = (_item.s3_paths?.[hazardSeverity]) ?? _item.s3_path;
  const _cleaned = [{ ..._item, s3_path: patchWindowsCache(_path) }];
  return await generateDB(_cleaned);
}
```

```{ojs}
// Section-gate heavy data fetches. Each gate is a Generators.observe
// cell that yields `false` until the section's anchor enters the
// viewport (with 200px lead time), then yields `true` once and stays
// true for the rest of the session. Heavy data cells reference these
// gates as a dep, so their cold S3 fetch doesn't fire until the user
// actually scrolls toward the section. Initial-paint time for sections
// above (Recent Changes, Production Trends, Key Facts) is no longer
// blocked by Future Projections + Hazard Exposure data work.
//
// Anchors: <a id="futureProjections-anchor"></a> and
// <a id="hazardExposure-anchor"></a> are placed immediately before
// each section heading. If a heading's anchor element is missing
// (e.g. headless renders), the helper fails open and returns true so
// the cell still fires.
sectionVisible = (anchorId) => Generators.observe((notify) => {
  notify(false);
...
futureProjectionsVisible = sectionVisible("futureProjections-anchor");
hazardExposureVisible = sectionVisible("hazardExposure-anchor");
```

```{ojs}
dbFutureHive = {
    // Path B-1: gate the view-registration itself on the section's
    // visibility. Without this, DuckDB-WASM fetches the parquet footer
    // for every file in `parquet_scan([...])` at view-creation time
    // (5 footer fetches on init for the 5 future parquets). With the
    // gate, the view isn't created until the user scrolls toward
    // Future Projections. The sole consumer (futureProjections_dataAll)
    // already returns [] while !futureProjectionsVisible, so the
    // sentinel here is belt-and-braces in case a future caller forgets
    // to gate itself.
    if (!futureProjectionsVisible) return { query: async () => [] };
    ...
    // Keep hive_partitioning=1 — every attempt to replace it crashed
    // DuckDB-WASM with `[object WebAssembly.Exception]` even with SQL
    // that ran fine in standalone DuckDB (regexp_extract, split_part
    // on filename, UNION ALL with literal timeperiod). Appears to be
    // a DuckDB-WASM-specific limitation on multi-file parquet_scan
    // without hive_partitioning. The row-group pushdown win we saw
    // in Option-C testing came predominantly from the IN→= predicate
    // change in futureProjections_dataAll, not from removing hive.
    await _db.query(`
...
gdp_plotData = {
  const resp = await dbGdp.query(`
    ...
    FROM a0_gdp
    WHERE ${admin0Iso3.length === 1 ? `iso3 = '${admin0Iso3[0]}'` : `iso3 in ${sqlList(admin0Iso3)}`}
    AND sector != 'total'
    AND year = 2022
  `)
  return resp
...
landuse_plotData = {
  const resp = await dbLanduse.query(`
    SELECT iso3,
      sub_group,
      "group",
      year,
...
pov_plotData = {
  const resp = await dbPov.query(`
    ...
    WHERE 1=1
    AND ${admin0Iso3.length === 1 ? `iso3 = '${admin0Iso3[0]}'` : `iso3 in ${sqlList(admin0Iso3)}`}
    AND (
...
```{ojs}
exposure_plotData = {
  const resp = await dbExposure.query(`
  SELECT
    iso3,
    admin1_name,
    crop,
...
productionYearBounds = {
  const resp = await dbProductionTrends.query(`
...
recentChanges_data = {
  ...
  const resp = await dbRecentChanges.query(`
    SELECT
      iso3,
      admin0_name,
      admin1_name,
...
```{ojs}
// Pulls all 4 future timeperiods for the selected country in ONE
// query (no timeperiod predicate). Switching the timeperiod in the UI
// becomes a JS-only filter against this in-memory result — no
// re-fetch, instant switches. Cold-fetch cost is ~3-4× a single-
// timeperiod fetch (we touch 4 hive-partitioned files instead of 1)
// but it happens once per (country / scenarios / hazards / season /
// admin1) combo and amortises over the rest of the session.
futureProjections_dataAll = {
  // Section-gate: don't fire the cold S3 fetch until the user scrolls
  // near the Future Projections section. Returns an empty array
  // initially so downstream cells render gracefully (the chart shows
  // empty/loading state until visibility flips).
  if (!futureProjectionsVisible) return [];

  const _season = seasonSelect.season;

  // Hazards we actually need: the user-selected one (for the main chart
  // + table + downloads) plus HSH-max (temperature paragraph) and PTOT
  // (precipitation paragraph) which the Quick Insights builder at
  // `climateProjectionInsight` always renders regardless of selection.
  // Deduped because the selected hazard is often HSH-max or PTOT.
  const requiredHazards = [...new Set([climateVarSelectFuture.id, "HSH-max", "PTOT"])];

  // When no admin1 is selected we only want the country aggregate
  // (admin1_name IS NULL). The OR-IS-NULL form with an empty IN list
  // (`admin1_name in ('')`) still forces DuckDB to evaluate the IN
  // arm against candidate row groups — wasted work, no matches.
  // Single-arm IS NULL is faster and lets the row-group skip fire on
  // the admin1_name stats if they exist.
  const admin1Predicate = admin1Names.length > 0
    ? `AND (admin1_name in ${sqlList(admin1Names)} OR admin1_name IS NULL)`
    : `AND admin1_name IS NULL`;

  // Test 2 (2026-05-26): IN-with-single-value seems to defeat parquet
  // row-group pruning in DuckDB-WASM. Use `=` for single-value cases
  // to give the optimizer a fighting chance.
  const iso3Predicate = admin0Iso3.length === 1
    ? `AND iso3 = '${admin0Iso3[0]}'`
    : `AND iso3 in ${sqlList(admin0Iso3)}`;
  const scenarioList = futureScenarioSelect.map((d) => d.toLowerCase());
  const scenarioPredicate = scenarioList.length === 1
    ? `AND scenario = '${scenarioList[0]}'`
    : `AND scenario in ${sqlList(scenarioList)}`;
  const hazardPredicate = requiredHazards.length === 1
    ? `AND hazard = '${requiredHazards[0]}'`
    : `AND hazard in ${sqlList(requiredHazards)}`;
  ...
  SELECT
    ...
    sd,
    sd_anomaly,
    -- CR-060 inter-model ribbon columns, restored 2026-06-12 after the
    -- pipeline republished the canonical with iso3-first row-group sort +
    -- q17/q83 (CR-119). They were dropped 2026-06-05 during the rebake
    -- regression (iso3 dropped + thrift corruption); the ribbon nullish
    -- fallback now resolves to the real inter-model percentile bounds.
    q17,
    q83,
    q17_anomaly,
    q83_anomaly,
  ...
  // admin0_name is dropped from the SELECT (one fewer column-chunk
  // fetch per cold query). Reattach from the iso3 lookup before
  // returning — downstream consumers (extremeEvents, Quick Insights,
  // narrative builders) read d.admin0_name as a stable English name.
  return withAdminName(resp).map((r) => ({
    ...r,
    admin0_name: iso3ToAdmin0Name.get(r.iso3) ?? r.iso3,
  }));
}

// JS-only filter on the timeperiod the user selected. Updates
// instantly because all 4 timeperiods are already in memory.
// Consumers (climateProjectionInsight, futureProjections_plotData,
// extremeEvents_plotData, downloads) read this unchanged.
futureProjections_data = futureProjections_dataAll.filter(
  (d) => d.timeperiod === futurePeriodSelect,
)

futureProjections_data;

futureProjections_plotData = futureProjections_data.filter((d) => d.hazard === climateVarSelectFuture.id)
```
...
```{ojs}
// Pulls historical + all 4 future timeframes for the selected
// country in ONE query (no timeframe predicate against the user's
// selection). Switching the future timeperiod becomes a JS-only
// filter against this in-memory result — no re-fetch, instant
// switches. Cost: ~2-3× a single-timeframe fetch (5 timeframes
// instead of 2) but amortised over the rest of the session.
// admin1 + scenario + hazard_vars + exposure_unit + crop filters
// stay at SQL level so the row-group skip on iso3 still fires
// against a focused result set.
hazardExposure_dataAll = {
  // Section-gate: don't fire the cold S3 fetch until the user scrolls
  // near the Hazard Exposure section. The hazard_exposure parquet is
  // the biggest single file in the notebook (~60M rows), so gating
  // it off-screen is the highest-leverage section-gate.
  if (!hazardExposureVisible) return [];

  const admin1Predicate = admin1Names.length > 0
    ? `AND (admin1_name in ${sqlList(admin1Names)} OR admin1_name IS NULL)`
    : `AND admin1_name IS NULL`;

  // Advanced-controls: swap the hazard_vars composite set based on
  // the threshold-definition toggle. `generic` is the chart default
  // (uniform per-day-count thresholds for all crops + livestock);
  // `ecocrop` switches in the crop-specific Ecocrop-derived precip
  // + crop-heat thresholds (NDWS / NDWL0 / THI-max stay generic — no
  // per-crop Ecocrop analogue for day-count summaries).
  const _hazardVars = hazardThresholdStyle === "ecocrop"
    ? ['PTOT-L+NTxS+PTOT-G', 'PTOT-L+THI-max+PTOT-G']
    : ['NDWS+NTx35+NDWL0', 'NDWS+THI-max+NDWL0'];

  const resp = await dbHazardExposure.query(`
  SELECT
    iso3,
    admin0_name,
    admin1_name,
    ...
    AND hazard_vars in ${sqlList(_hazardVars)}
    AND severity = '${hazardSeverity}'
    AND exposure_unit = 'nominal-usd-2021'
    AND crop != 'generic-crop'
    AND iso3 in ${sqlList(admin0Iso3)}
    ${admin1Predicate}
    AND timeframe in ('1995-2014', '2021-2040', '2041-2060', '2061-2080', '2081-2100')
...
// JS-only filter: historical baseline + the user-selected future
// timeframe. Consumers (chart, table, downloads) read this name
// unchanged.
hazardExposure_plotData = hazardExposure_dataAll.filter(
  (d) => d.timeframe === '1995-2014' || d.timeframe === futurePeriodSelect,
)
...
```{ojs}
//| output: false
// Future Projections + Extreme Events use the CMIP6 ensemble, which
// does NOT carry the SPEI drought indices (SPEI is a derived
// precipitation-evapotranspiration index produced from observed
// CHIRPS / CHIRTS-ERA5 and is not part of the projection pipeline).
// Filter SPEI out of the variables offered in the Future / Extreme
// selectors. Recent Changes keeps its full obsHazards list which
// includes SPEI-03 / SPEI-12.
futureHazards = hazards_obj.filter((h) => !h.id.startsWith("SPEI"));
```

```{ojs}
// Independent climate-variable state for the Future Projections and
// Extreme Events sections. Recent Changes uses observational data
// (CHIRPS / CHIRTS-ERA5) and supports the 6 observational vars
// including SPEI; Future + Extreme use CMIP6 projections and support
// the 10 non-SPEI hazards (incl. heat indices). Keeping the two
// selectors disconnected lets a user explore e.g. observed PTOT in
// Recent and a heat index (HSH-max) in Future without one selection
// clobbering the other.
viewof climateVarSelectFuture = Inputs.select(futureHazards, {
  format: (d) => d.id,
  value: futureHazards.find((d) => d.id === "PTOT") ?? futureHazards[0],
});
```
...
exposureBars_keyFacts = function () {
  ...
  const chartBody = html`<div>${rowChunks.map((r, idx) => renderRow(r, idx))}</div>`;
  return html`<div>
    ${chartBody}
    ${captionDetails(caption, undefined, chartDownloadButton(chartBody, { filename: "agProductionBySector", data: exposure_plotData }))}
  </div>`;
};
```
...
treemap_agProduction = () => {
  ...
  const treemapGrid = html`<div style="display: flex; flex-wrap: wrap; align-items: flex-start; gap: ${gap}px;">
      ${admins.map(buildOne)}
    </div>`;
  const wrap = html`<div>
    ...
    ${treemapGrid}
    ${captionDetails(caption, undefined, chartDownloadButton(treemapGrid, { filename: "agProductionBySector_treemap", data: exposure_plotData }))}
  </div>`;
...
productionTradeDataCaveat = () => {
  ...
  return html`<div style="display: block; font-size: 0.82em; color: #6b4d1a; margin: 0.4em 0 0.6em 0; padding: 0.45em 0.7em; background: #fff7e6; border-left: 3px solid #d4a017; border-radius: 3px; line-height: 1.4;">
    <strong>Trade data caveat</strong> — Export / Import series come from FAOSTAT's Trade Matrix bulk download. Coverage varies by country / commodity (often starting later than 1961); some pre-2017 series show known reporter-country anomalies (e.g. Angola palm oil); some processed exports may be lumped into FAOSTAT's "Juice of fruits n.e.c." or similar generics. Wine and the concentrated juices (apple, orange, grape, pineapple) now roll up correctly under their raw parents (canonical parquet republished 2026-05-27); a small tier-2 sibling audit of niche items remains open. Cross-check sensitive findings against FAOSTAT's <a href="https://www.fao.org/faostat/en/#data/TM">online portal</a> or <a href="https://oec.world/">OEC</a>. See <a href="#methods-production">Methods</a> for details.
  </div>`;
}
```
...
chart_productionTrends = () => {
  ...
  const chartBody = html`<div>${plot}${productionByproductsLegend()}</div>`;
  return html`<div>
    ${productionTradeDataCaveat()}
    ${chartBody}
    ${captionDetails(caption, undefined, chartDownloadButton(chartBody, { filename: "productionTrends", data: productionTrends_data }))}
  </div>`;
};
```
...
treemap_productionTrends = () => {
  ...
  const wrap = html`<div>
    ...
    ${treemapGrid}
    ${captionDetails(caption, undefined, chartDownloadButton(treemapGrid, { filename: "productionTrends_treemap", data: productionTrends_data }))}
  </div>`;
...
timeseries_futureProjections = () => {
  ...
  const _variableName = _lang(climateVarSelectFuture.name);
  const _variableAxis = _lang(climateVarSelectFuture.labelAxis);
  ...
  // CR-060: ribbon uses the inter-model 17–83 % range (AR6 'likely')
  // baked into the parquet, not a synthetic mean ± 1 σ. Anomaly toggle
  // routes to anomaly-percentile columns; raw mode uses absolute
  // percentile columns. Falls back to mean if the columns aren't
  // present (defensive — they SHOULD be there per CR-060 canonical).
  const plotValue_q17 = _showAnomaly ? "q17_anomaly" : "q17";
  const plotValue_q83 = _showAnomaly ? "q83_anomaly" : "q83";
  ...
  // Ribbon bounds: inter-model 17 % / 83 % percentiles across the
  // ensemble (n_models per row, varies slightly across periods).
  const ribbonUpper = (d) => d[plotValue_q83] ?? d[plotValue_mean];
  const ribbonLower = (d) => d[plotValue_q17] ?? d[plotValue_mean];

  const yDataMin = d3.min(_data, ribbonLower);
  const yDataMax = d3.max(_data, ribbonUpper);
  // Only expand the y-axis to include the ±1σ / ±2σ threshold lines when
  // those lines are actually drawn (anomaly mode AND highlight toggle on).
  // Earlier code inflated the domain on _showAnomaly alone, which left the
  // axis at ±maxStd2 even with the threshold lines hidden — making the
  // data look flat-line near zero because the visible range was 10–100×
  // wider than the data range.
  const showThresholds = _showAnomaly && highlightExtremesFuture;
  const maxStd2 = showThresholds
    ? 2 * (d3.max([...baselineStdByAdmin.values()]) ?? 0)
    : 0;
  let yMin = yDataMin;
  let yMax = yDataMax;
  if (_showAnomaly) {
    // Keep the zero baseline line visible in anomaly mode even if the data
    // is entirely above or below zero (e.g. consistent warming).
    yMin = Math.min(yMin, 0);
    yMax = Math.max(yMax, 0);
  }
  if (showThresholds) {
    yMin = Math.min(yMin, -maxStd2 * 1.05);
    yMax = Math.max(yMax, maxStd2 * 1.05);
  }
  // 5 % padding for breathing room around the data; floor to avoid a zero
  // pad when min==max (single-point edge case).
  const yPad = Math.max((yMax - yMin) * 0.05, 0.01);
  const yExtent = [yMin - yPad, yMax + yPad];
  ...
  const _tails = climateVarSelectFuture.tails ?? "both";
        ...
        Plot.dot(rowData, {
          ...
          tip: {
            channels: {
              ...
              // CR-060 inter-model percentile band — shows the AR6 'likely'
              // 17–83 % range alongside the mean. Hidden when the columns
              // aren't present so older parquets don't break the tooltip.
              q17_83: {
                label: "Inter-model 17–83 %",
                value: (d) => {
                  const lo = d[plotValue_q17];
                  const hi = d[plotValue_q83];
                  return lo == null || hi == null ? "—" :
                    `${d3.format(",.2f")(lo)} to ${d3.format(",.2f")(hi)}`;
                },
              },
              n_models: {
                label: "Ensemble size",
  ...
  // CR-060 caption update: inter-model ribbon is now the calibrated
  // 17–83 % percentile band straight from the parquet (no Gaussian
  // approximation). The ensemble size varies slightly across periods
  // because not every GCM produced every (scenario × variable ×
  // season × year) row — surface the most common count below.
  const nModelsCounts = d3.rollup(_data, v => v.length, d => d.n_models);
  const nModelsMode = nModelsCounts.size > 0
    ? d3.greatest([...nModelsCounts.entries()], ([, c]) => c)?.[0]
    : null;
  const nModelsText = nModelsMode != null ? ` (${nModelsMode}-member ensemble)` : "";
  const captionLines = [
    ...
    `Line = mean across the NEX-GDDP-CMIP6 GCMs (r1i1p1f1 realization). Shaded ribbon = inter-model 17–83 % range${nModelsText} — the IPCC AR6 'likely' calibrated-language band, computed directly from the per-GCM percentiles (no Gaussian approximation).`,
  ];
  if (_showAnomaly) {
    captionLines.push(
  ...
  const chartBody = html`<div>${chunks.map((r, idx) =>
      renderRow(r, idx, idx === 0, idx === chunks.length - 1),
    )}</div>`;
  return html`<div>
    ${chartBody}
    ${captionDetails(caption, undefined, chartDownloadButton(chartBody, { filename: "futureProjections", data: futureProjections_plotData }))}
  </div>`;
};
```
...
summary_futureProjections = () => {
  ...
  const plotValue_mean = _showAnomaly ? "mean_anomaly" : "mean";
  // CR-060: whiskers use the inter-model 17–83 % percentile range
  // (AR6 'likely'), averaged across the years of the selected
  // timeperiod. Falls back to the mean when columns are missing.
  const plotValue_q17 = _showAnomaly ? "q17_anomaly" : "q17";
  const plotValue_q83 = _showAnomaly ? "q83_anomaly" : "q83";
  ...
  // Aggregate per (admin × scenario): mean of annual `mean` plus the
  // mean-of-annual q17 / q83 percentiles. Plot.js doesn't need a
  // separate groupBy mark — we pre-aggregate into one row per (admin,
  // scenario). Averaging the per-year inter-model percentiles is the
  // right shape for a 20-year-window dot-and-whisker: each row is "the
  // average year's 17–83 % spread", not "the spread of multi-year
  // means" (which would shrink the band toward the central tendency).
  ...
  const summary = [];
  // Baseline row per admin: 1995–2014 mean ± interannual σ. This row
  // shows interannual variability (within the observed period), not
  // inter-model spread — different concept from the SSP whiskers,
  // intentionally kept so the reader can compare the future model
  // uncertainty against the historical interannual swing.
  const baselineLabel = "1995–2014";
  const adminsInData = [...new Set(_data.map((d) => d.adminName))];
  for (const a of adminsInData) {
    const m = baselineMeanByAdmin.get(a);
    const s = baselineStdByAdmin.get(a);
    if (m == null) continue;
    summary.push({
      adminName: a,
      scenario: "baseline",
      scenarioUpper: baselineLabel,
      mean: _showAnomaly ? 0 : m,
      low: (_showAnomaly ? 0 : m) - (s ?? 0),
      high: (_showAnomaly ? 0 : m) + (s ?? 0),
    });
  }
  for (const [adminName, byScen] of groups) {
    for (const [scen, scenRows] of byScen) {
      const meanVals = scenRows
        ...
        .filter((v) => v != null);
      const q17Vals = scenRows
        .map((r) => r[plotValue_q17])
        .filter((v) => v != null);
      const q83Vals = scenRows
        .map((r) => r[plotValue_q83])
        .filter((v) => v != null);
      if (meanVals.length === 0) continue;
      const meanVal = d3.mean(meanVals);
      summary.push({
        adminName,
        scenario: scen,
        scenarioUpper: scen.toUpperCase(),
        mean: meanVal,
        low:  q17Vals.length > 0 ? d3.mean(q17Vals) : meanVal,
        high: q83Vals.length > 0 ? d3.mean(q83Vals) : meanVal,
      });
    }
  }
  if (summary.length === 0) return noDataPlaceholder();

  // Y-axis: baseline at the top, then canonical SSP order filtered to
  // scenarios actually in the data (CR-074 option 6a).
  const yDomain = [
    baselineLabel,
    ...canonicalOrder
  ...
  const xMin = d3.min(summary, (d) => d.low);
  const xMax = d3.max(summary, (d) => d.high);
  ...
  const { chunks, cols } = adminGridSplit(_data, facetCols);
  const _unit = _lang(climateVarSelectFuture.unit) || "";
  const fmtSigned = d3.format("+,.1f");
  // Label: "+1.4 °C [17–83 %: +0.7 to +2.1]" — explicit about what the
  // bracket means, so it's not mistaken for ±1 σ (the prior wording).
  const labelFor = (d) => {
    const centre = `${fmtSigned(d.mean)}${_unit ? ` ${_unit}` : ""}`;
    const band = `${fmtSigned(d.low)} to ${fmtSigned(d.high)}`;
    return `${centre} [17–83 %: ${band}]`;
  };
  ...
  const renderRow = (rowAdmins, rowIdx, isFirst, isLast) => {
    ...
    return Plot.plot({
      ...
      color: {
        domain: [baselineLabel, ...scenariosInData.map((s) => s.toUpperCase())],
        range: ["#666", ...scenariosInData.map((s) => colorByScenario.get(s))],
        legend: false,
      },
      marks: [
        // Anomaly mode: vertical zero baseline (the implicit reference).
        // In absolute mode the baseline whisker itself anchors the
        // reference, so no separate rule is needed.
        Plot.ruleX(_showAnomaly ? [0] : [], {
          stroke: "#666",
          strokeDasharray: [4, 4],
        }),
        // Whisker body — horizontal rule from low (q17) to high (q83).
        // For the baseline row low/high carry interannual ±1 σ instead
        // (different concept; the row's tooltip + label makes this
        // explicit so the reader doesn't conflate the two).
        Plot.ruleY(rowData, {
          x1: "low",
          x2: "high",
          y: "scenarioUpper",
        ...
        // height — small symmetric insetTop/insetBottom shorten the
        // cap so it reads as a brief serif rather than a full bar,
        // while staying clearly visible.
        Plot.tickX(rowData, {
          x: "low",
          y: "scenarioUpper",
          stroke: "scenarioUpper",
  ...
  return html`<div>
    ...
    ${captionDetails(
      caption,
      undefined,
      chartDownloadButton(chartBody, { filename: "futureProjections_summary", data: futureProjections_plotData }),
    )}
  </div>`;
};
```
...
bars_extremeEvents = () => {
  ...
  const renderRow = (rowAdmins, rowIdx, isFirst, isLast) => {
    ...
    return Plot.plot({
      ...
      title: isFirst
        ? `Extreme events: ${_lang(climateVarSelectFuture.name)}`
        : undefined,
      facet: { data: rowData, x: "adminName", label: null },
  ...
  return html`<div>
    ${chartBody}
    ${captionDetails(caption, undefined, chartDownloadButton(chartBody, { filename: "extremeEvents", data: extremeEvents_plotData }))}
  </div>`;
};
...
stackbars_hazardExposure = () => {
  ...
  // Caption text reflects the active Severity tier + Threshold definition
  // so reopening "About this plot" after a control change shows accurate
  // method copy. Indices listed depend on the threshold-style toggle
  // (generic uses NDWS + NTx35 / THI-max + NDWL0; Ecocrop uses
  // PTOT-L + NTxS / THI-max + PTOT-G — note livestock-heat THI-max stays
  // generic in both modes).
  const cap = nbText.sections.hazardExposure.caption;
  const _severityWord = _lang(cap.severityWord[hazardSeverity]);
  const _indices = hazardThresholdStyle === "ecocrop"
    ? "PTOT-L / NTxS / THI-max / PTOT-G"
    : "NDWS / NTx35 / THI-max / NDWL0";
  const leadLine = _lang(cap.lead)
    .replace("{severityWord}", _severityWord)
    .replace("{indices}", _indices);
  const thresholdsLine = hazardThresholdStyle === "ecocrop"
    ? _lang(cap.thresholdsEcocrop)
    : _lang(cap.thresholdsGeneric);
  const caption = multiLineText(
    [
      leadLine,
      thresholdsLine,
      html`${_lang(cap.sources)} Production from <a href="https://cgspace.cgiar.org/items/5736e3ff-aadc-4999-8922-8bac50b1c916">MapSPAM 2020 v1r2 SSA</a> · <a href="https://data.apps.fao.org/glw/">Gridded Livestock of the World v4 (2020)</a> · hazards <a href="https://www.nccs.nasa.gov/services/data-collections/land-based-products/nex-gddp-cmip6">NEX-GDDP-CMIP6</a>. Data: <a href="https://radiantearth.github.io/stac-browser/#/external/digital-atlas.s3.amazonaws.com/stac/public_stac/catalog.json">STAC catalog</a>.`,
    ],
    "atlasFigCaption",
  );
  ...
  return html`<div>
    ${chartBody}
    ${captionDetails(caption, undefined, chartDownloadButton(chartBody, { filename: "hazardExposure", data: hazardExposure_plotData }))}
  </div>`;
};
```
