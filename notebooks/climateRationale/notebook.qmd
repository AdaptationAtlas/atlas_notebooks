...
futureProjections_dataAll = {
  ...
  SELECT
    ...
    sd,
    sd_anomaly
    -- CR-060 percentile columns (q17, q83, q17_anomaly, q83_anomaly,
    -- n_models) ARE in the post-2026-06-01 canonical schema but the
    -- 2026-06-05 rebake that landed them ALSO dropped iso3 + introduced
    -- thrift corruption on aggregates — see
    -- dispatches/2026-06-05_cr060-parquet-regression.md. We're holding
    -- the SELECT to the legacy column set until a clean rebake lands;
    -- the ribbon code's `?? d[plotValue_mean]` fallback means the
    -- ribbon silently collapses to the mean line until then.
