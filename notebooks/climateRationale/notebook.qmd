...
productionTrends_filtered = {
  ...
  for (const [k, rows] of grouped) {
    ...
    const rawSum = d3.sum(
      rows.filter((r) => r.type === "raw"),
      (r) => r.value,
    );
    const procSum = d3.sum(
      rows.filter((r) => r.type === "processed"),
      (r) => r.value,
    );
    rolled.push({
      iso3,
...
chart_productionTrends = () => {
  ...
  const stackedData = showSplit
    ...
    : _data.map((d) => ({ ...d, stackValue: d.value, stackType: "raw" }));
  ...
  const marks = isStacked
    ? [
        Plot.rectY(stackedData, {
          ...
          z: showSplit ? (d) => `${d.commodity}|${d.stackType}` : undefined,
          stroke: "white",
          strokeWidth: 0.4,
          ...
          channels: {
            Commodity: "commodity",
            Type: showSplit ? "stackType" : undefined,
            Region: "adminName",
    ...
    const leafEls = root.leaves().map((leaf) => {
      ...
      const tipText = isSplit
        ? `${admin} — ${label} (${leaf.data.year}): ${valueText} ` +
          ...
          `processed ${fmtVal(leaf.data.value_processed)})`
        : `${admin} — ${label} (${leaf.data.year}): ${valueText}`;
      const textFill = treemapTextColor(fillColor);
      const haloFill = textFill === "#fff" ? "#000" : "#fff";
      const layout = treemapTextLayout(
