...
```{ojs}
// Caveat — surfaces only when the active variable is non-monetary AND
// the byproducts toggle is on. The toggle stays interactive but the
// data pipeline ignores it because physical quantities (tonnes, kg/ha,
// head counts) don't combine across (raw, processed) transformation
// states. The caveat tells the user to switch variable if they want
// the rollup view.
{
  const isMonetary = productionMonetaryVars.has(productionVar.id);
  if (isMonetary || !productionIncludeByproducts) return html`<span></span>`;
