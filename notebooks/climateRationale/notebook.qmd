...
viewof includeAdmin0 = Inputs.toggle({
...
**Heads up.** The "Agricultural Production" plot in the Key Facts section below uses a **different** production dataset (MapSPAM 2020, nominal 2021 US$, subnational snapshot). This section uses FAOSTAT — national totals, constant 2014–2016 dollars, deep time-series. The two are **not directly comparable**. See [Methods and data sources](#methods-production) below for the full explanation.
:::
...
```{ojs}
// Single-track two-handle year-range slider. Two `<input type="range">`
// overlaid on a shared visible track (the `.dual-range-track` div before
// them). Both inputs have transparent backgrounds + transparent runnable
// tracks (Chrome, Firefox, Safari pseudo-elements all overridden with
// !important to defeat any theme defaults). pointer-events:none on the
// input bodies + pointer-events:auto on the thumb pseudo lets both thumbs
// remain draggable on the same x-axis. The earlier two-stacked-sliders
// fallback rendered correctly but the user wanted a single track.
viewof productionYearRange = {
  const minY = productionYearBounds.min_year;
  const maxY = productionYearBounds.max_year;
  const defaultLo = Math.max(minY, 2010);
  const defaultHi = maxY;

  const loSpan = html`<span style="font-variant-numeric: tabular-nums; font-weight: 600;">${defaultLo}</span>`;
  const hiSpan = html`<span style="font-variant-numeric: tabular-nums; font-weight: 600;">${defaultHi}</span>`;
  const loInput = html`<input type="range" min="${minY}" max="${maxY}" value="${defaultLo}" step="1" />`;
  const hiInput = html`<input type="range" min="${minY}" max="${maxY}" value="${defaultHi}" step="1" />`;

  const container = html`<div class="dual-year-range" style="display: flex; flex-direction: column; gap: 6px; font: 13px sans-serif; min-width: 260px; width: 100%; max-width: 360px;">
    <label>Year range: ${loSpan} – ${hiSpan}</label>
    <div class="dual-range-track" style="position: relative; height: 24px;">
      <div class="dual-range-visible-track"></div>
      ${loInput}
      ${hiInput}
    </div>
  </div>`;
  ...
  style.textContent = `
    .dual-year-range .dual-range-track,
    .dual-year-range .dual-range-track * {
      box-sizing: border-box;
    }
    .dual-year-range .dual-range-visible-track {
      position: absolute;
    ...
    .dual-year-range .dual-range-track input[type="range"] {
    ...
    .dual-year-range .dual-range-track input[type="range"]::-webkit-slider-runnable-track {
      background: transparent !important;
      border: none;
      height: 24px;
    }
    .dual-year-range .dual-range-track input[type="range"]::-moz-range-track {
      background: transparent !important;
      border: none;
      height: 24px;
    }
    .dual-year-range .dual-range-track input[type="range"]::-moz-range-progress {
      background: transparent !important;
    }
    .dual-year-range .dual-range-track input[type="range"]::-webkit-slider-thumb {
      -webkit-appearance: none !important;
      appearance: none !important;
      pointer-events: auto;
    ...
    .dual-year-range .dual-range-track input[type="range"]::-moz-range-thumb {
      pointer-events: auto;
      width: 16px; height: 16px;
      border-radius: 50%;
      background: #2563eb;
  ...
  const update = () => {
    const lo = Math.min(+loInput.value, +hiInput.value);
    const hi = Math.max(+loInput.value, +hiInput.value);
  ...
  container.value = [defaultLo, defaultHi];
  return container;
}
```
...
//| output: false
{
  [admin0Iso3, productionVar, viewProductionTrends, paletteProduction,
   productionTopN, productionYearRange, commoditiesSelect];
...
### `{ojs} agProductionTitle`

```{ojs}
md`${_lang(nbText.sections.keyFacts.agProductionIntro)}`;
```

::: {.alert .alert-info role="note" style="font-size: 0.92em;"}
**Heads up.** The plot below uses **MapSPAM 2020 in nominal 2021 US dollars** — a single subnational (admin1) snapshot. For *historical national time-series* of the same commodities, see the [Agricultural Production Trends](#productionTrends) section above; the two datasets are **not directly comparable** (different value bases, different geographic units). See [Methods and data sources](#methods-socioeconomic) for the full explanation.
:::
...
keyFacts_combinedData = {
  ...
  const com = exposure_plotData.map((d) => ({
    iso3: d.iso3,
    admin1_name: d.admin1_name,
    metric: "commodity",
    sub_group: d.crop,
    ...
    value: d.value,
    unit: d.unit,
    source: "MapSPAM 2020 (Adaptation Atlas) + GLW 4",
  }));
  return [...pov, ...gdp, ...lu, ...com];
}
```

`{ojs} downloadButton(keyFacts_combinedData, "keyFacts", "Download Key Facts")`
...
agProductionTitle = _lang(nbText.sections.keyFacts.agProductionTitle);
...
productionTopCommodities = {
  const [yStart, yEnd] = productionYearRange;
  if (!admin0Iso3.length) return [];
...
barplot_recentChanges = () => {
  ...
  const _mainTitle = `${_variableName} (${_lang(general_translations.anomaly)}) — Season: ${_season}`;
  const _reverse = ["PTOT"].includes(climateVarSelect.id);

  const absDataMax = Math.max(
    Math.abs(d3.min(_data, (d) => d.mean_anomaly) ?? 0),
    Math.abs(d3.max(_data, (d) => d.mean_anomaly) ?? 0),
  );
  // Expand y domain to fit the ±2σ threshold lines, so "extreme" levels
  // are always visible even when the data itself doesn't reach them.
  ...
  const renderRow = (rowAdmins, rowIdx, isFirst, isLast) => {
    ...
    const rowData = _data.filter((d) => rowAdmins.includes(d.adminName));
    const rowThresholds = buildThresholdRows(rowAdmins, _tails);
    // Place labels only inside the rightmost admin facet. Both upper and
    // lower thresholds are labelled for PTOT; only upper for high-only
    // variables (handled by buildThresholdRows above).
          ...
          ...(highlightExtremes
            ? {
                stroke: (d) => {
                  ...
                  const abs = Math.abs(d.mean_anomaly / sd);
                  if (abs >= 2) return "#c00";
                  if (abs >= 1) return "#444";
                  return "transparent";
                },
  ...
  const caption = multiLineText(
    [
      "Historical annual values for 1995–2014 (20 years), by admin1 region. Bars show the anomaly from the 1995–2014 baseline mean.",
      html`Enable <em>Highlight unusual / extreme</em> to show |z|=1 (grey) and |z|=2 (red) thresholds. See the <a href="#extremeEvents">Extreme Events</a> section for full definitions.`,
...
warmingStripes_recentChanges = () => {
  ...
  const caption = multiLineText(
    [
      "Historical annual values for 1995–2014 (20 years), by admin1 region. Each stripe = one year; line traces the per-year ensemble mean.",
...
timeseries_futureProjections = () => {
  ...
  const caption = multiLineText(
    [
      ...
      "Line = mean across 18 NEX-GDDP-CMIP6 GCMs (r1i1p1f1 realization). Shaded ribbon = ensemble mean ± 1 standard deviation across the 18 models. Assuming an approximately-normal inter-model distribution, this band corresponds to the IPCC AR6 'likely' (17–83%) range. Anomalies are relative to the 1995–2014 baseline.",
