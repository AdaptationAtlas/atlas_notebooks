...
```{ojs}
//TODO: Move this to the appendix soe nb code is cleaner.
...
```{ojs}
mutable cv_result = []; //TODO: To allow rest of the cells to render and say, "run code"
mutable cv_admin0_names = [];
mutable cv_crops = [];
mutable cv_cis_impact = null;

//TODO: this gets wrapped in a function
calcCV = async () => {
  const cis_impact = options_econPerfWalkthrough.inputCisImpact.value;

  if ( // Can be computation heavy so if inputs repeat, don't recompute
      cv_admin0_names === admin0_names
      && cv_crops === select_crop 
      && cv_cis_impact === cis_impact
     ) {
    return cv_result;
  }
...
`);
  ...
  mutable cv_admin0_names = admin0_names
  mutable cv_crops = select_crop
  mutable cv_cis_impact = cis_impact
  return cv_index
}
```
...
crop_adoption = async () => {
  ...
  let adoption = options_econPerfWalkthrough.inputAdoption.value;
  let years = projectYears;
  let prod_impact = options_econPerfWalkthrough.inputProdImpact.value;
...
```{ojs}
roiResults = async () => {
  let index = await calcCV();
  let data = await crop_adoption();
  ...
  return withImpact.map((d) => {
    const project_benefit =
      ...
      10;
    const cis_impact = options_econPerfWalkthrough.inputCisImpact.value; //TODO: This can probably be removed and just called once later
...
```{ojs}
recipe_dataSummed = { 
//NOTE: Currently recipe_dataSummed in notebook
  const roi_data = await roiResults()
  
  const aggregated = d3.flatGroup(
    roi_data,
    d => d.adoption,
    d => d.prod_impact,
    d => d.cis_impact,
