...
import {
  ...
  sparkbar,
} from "/components/atlasTable.ojs";
```
...
viewof productionView = Inputs.radio(["plot", "table"], {
  value: "table",
  label: "Producition Data Preview",
})
```
...
```{ojs}
{
  if (productionView == "table") {
    ...
    return atlasTable(exposure_data, {
      ...
      header: {value: "USD", crop: "Crop", admin0_name: "Country", admin1_name: "Region"},
      sort: "value",
      reverse: true,
      format: {
        value: sparkbar({
...
renderProductionBars = () => {
  ...
  return Plot.plot({
    ...
    width,
    height: 500,
    x: { label: null, tickFormat: (d) => `${d} Regions` },
...
```{ojs}
{
  const formBenefitScale = Inputs.form(
    {
      low: Inputs.range([0.5, 1], {
        step: 0.01,
        label: "Benefit Scale Low",
        value: 0.85,
      }),
      high: Inputs.range([1, 1.6], {
        step: 0.01,
        label: "Benefit Scale High",
        value: 1.15,
      }),
    },
    {
  ...
  const formCostScale = Inputs.form(
    {
      low: Inputs.range([0.5, 1], {
        step: 0.01,
        label: "Cost Scale Low",
        value: 0.9,
      }),
      high: Inputs.range([1, 1.8], {
        step: 0.01,
        label: "Cost Scale High",
        value: 1.2,
      }),
    },
    {
  ...
  const _discountLabel = makeTooltipLabel({
    ...
    tooltipText: "The Discount rate is...",
  })

  const _roiLabel = makeTooltipLabel({
    labelText: "ROI",
    tooltipText: "The ROI is...",
  })
...
```{ojs}
viewof discountRate = Inputs.range([0, 1],{step: 0.01, format: x => (x * 100), label : "Discount Rate", value: 0.12} ) // 0-1 as a pct
