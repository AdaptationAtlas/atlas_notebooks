...
/** Estimate proportional loss reduction by simulating baseline and changed CVs. */
export function avlossCalc(
  ...
  // Generate normal samples with the Box–Muller transform.
  const rnorm = (n, sd) => {
  ...
  // Express lower-half loss reduction against the baseline total.
  return Math.max(0, (with_lh - without_lh) / totalWithout);
}

// Solve IRR with the secant method.
...
/** Repeat avlossCalc to estimate numerical uncertainty bounds. */
...
/** Simulate uncertainty from cashflow rows; unit scales reproduce deterministic inputs. */
