...
`{ojs} buildInlineDownloadButton(task_gender_data, _lang(nbText.General.download), "FemaleAgLaborParticipation")`
...
`{ojs} buildInlineDownloadButton(crop_gender_data, _lang(nbText.General.download), "FemaleCropParticipation")`
...
`{ojs} buildInlineDownloadButton(non_topo, _lang(nbText.General.download), "FemaleEmpowerment")`
...
```{ojs}
nbTitle = _lang({en: "Analyze Women's Exposure to Climate Hazards", fr: "Analyser l'exposition des femmes aux aléas climatiques"})
// h1
...
FemaleParticipation = _lang(nbText.GenderedAfriAG.FemaleParticipationLabor.title)
```

```{ojs}
nbText = FileAttachment("../../data/genderNotebook/Gender_nbText.json").json()
...
admin0_boundaries = FileAttachment("../../data/shared/atlas-region_admin0_harmonized.json").json()
...
heatmap_legend = () => {
  ...
  const legendAxis = d3
    ...
    .scale(legendScale)
    .tickValues([0, 1])
    .tickFormat((d) => (d === 0 ? _lang(nbText.GenderedAfriAG.LegendScale.low) : _lang(nbText.GenderedAfriAG.LegendScale.high)))
    .tickSize(0);
...
genderCropHeadline = () => {
  ...
  if (hover_data === null) {
    ...
    const involvementLevel = hover_data.value > crop_gender_means["country_mean"][hover_data.country] ? "more" : "less";
    const productionValue = formatDollar(selection_total * (hover_data.value / 100));
    fill_items = [{ name: "country", value: _lang(nbText.General.article_adminNames[hover_data.country]) },
                 { name: "involvement", value: _lang(nbText.General[involvementLevel]) },
                 { name: "cropType", value: _lang(nbText.GenderedAfriAG.FemaleParticipationCrops.plot.crops[hover_data.type]) },
                 { name: "cropPCT", value: hover_data.value },
                 { name: "cropValue", value: productionValue }];
    let template = _lang(nbText.GenderedAfriAG.WomenDominate.headline.valid_hover)
    text = md`${Lang.reduceReplaceTemplateItems(template, fill_items)}`
  } else {
    let template = _lang(nbText.GenderedAfriAG.WomenDominate.headline.null_hover)
     fill_items = [{ name: "country", value: _lang(nbText.General.article_adminNames[hover_data.country]) },
                  { name: "cropType", value: _lang(nbText.GenderedAfriAG.FemaleParticipationCrops.plot.crops[hover_data.type])}]
    text =  md`${Lang.reduceReplaceTemplateItems(template, fill_items)}`
...
biVar_insight = () => {
  ...
  const heat_HotSpot = filtered.reduce(
    (max, obj) => obj.hs_value > max.hs_value
      ? {
  ...
  const classifyACValue = (value) => {
    const values = country === "SSA"
      ? BiVar_merge.filter((d) => d.admin1_name === null).map((d) => d[G_variable])
      : BiVar_merge.filter((d) => d.admin1_name !== null).map((d) => d[G_variable]);
  ...
  const gender_HotSpot = filtered.reduce(
    (extreme, obj) => G_variable === "women_in_agric"
  ...
  let regionIntro = country === "SSA" ? {en: "", fr: ""} : {en: "", fr: "la region de"}
  let regionIntro_cap = country === "SSA" ? {en: "", fr: ""} : {en: "", fr: "La region de"}
  let template_fillers = [
    { name: "country", value: _lang(nbText.General.article_adminNames[country]) },
    { name: "regionIntro", value: _lang(regionIntro) },
    { name: "regionIntro_cap", value: _lang(regionIntro_cap) },
    { name: "highest_gender", value: translateA0_only(gender_HotSpot.admin_name)},
    { name: "HS_level", value: _lang(nbText.GenderHotspots.dynamicLevels[classified_hs.toLowerCase()])},
    { name: "heatstress_var", value: _lang(nbText.GenderHotspots.plot.heatLabels[heat_hazard]).toLowerCase() },
    { name: "highest_hs", value: translateA0_only(heat_HotSpot.admin_name)},
    { name: "AC_level", value: _lang(nbText.GenderHotspots.dynamicLevels[classified_ac.toLowerCase()])}];
  let md_text;
  let template
  if (G_variable === "women_in_agric") {
    ...
    template_fillers.push(
      { name: "gender_var", value:  _lang(nbText.GenderHotspots.domains[G_variable]).toLowerCase()}
    )
