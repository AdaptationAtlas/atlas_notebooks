...
hz_q2Chart = {
  ...
  const height = Math.max(
    360,
    crops.length * (hz_q2_showCompound ? 26 : 30),
  ...
  const color = { domain: hazLevels, scheme: "tableau10", legend: false }
  const legend = Plot.legend({ color: { domain: hazLevels, scheme: "tableau10" }, columns: 4 })
  ...
  const makeSidePlot = (title, sideKey) => {
    ...
    const data = rows
      .filter(r => crops.includes(r?.crop) && hazLevels.includes(String(r?.hazard ?? "")))
      .map(r => ({
        crop: r.crop,
    ...
    const groupedMarks = (rows, hazards) => {
      ...
      const step = 8 // px spacing between hazard bars within the same crop band
  ...
  const diffMarks = hz_q2_showCompound
      ? [
          Plot.barX(diffData, Plot.stackX({
            x: "value",
            y: "crop",
            fill: "hazard",
            tip: {
      ...
      : groupedMarks(diffData, hazLevels)
...
viewof hz_q3_viewMode = Inputs.radio(["side", "diff"], {
  label: "",
  value: "side",
  format: (v) => ({ side: "L/R", diff: "Δ (Right − Left)" }[v] ?? v)
})
hz_q3_viewMode.classList?.add?.("hz-viewToggle")

viewof hz_q3_includeNoHazard = hz_noJumpInput(Inputs.toggle({ label: "", value: true }))
viewof hz_q3_maxIntersections = Inputs.range([5, 30], { label: "", step: 1, value: 15 })
...
hz_q2_settingsWrap = html`
  <div class="hz-qToolbar__right">
    <div class="hz-inlineToggles">
      ...
      <div class="hz-miniSetting"><span>Compound</span>${viewof hz_q2_showCompound}</div>
      <div class="hz-miniSetting"><span>Simplify</span>${viewof hz_q2_simplifyCompound}</div>
...
hz_q3_settingsWrap = html`
  <div class="hz-qToolbar__right">
    <div class="hz-inlineToggles">
      <div class="hz-miniSetting"><span>No hazard KPI</span>${viewof hz_q3_includeNoHazard}</div>
      <div class="hz-miniSetting hz-miniSetting--range"><span>Intersections</span>${viewof hz_q3_maxIntersections}</div>
...
hz_q3DenomTotal = hz_q3DenomByCrop.reduce((s, d) => s + (Number(d?.denom) || 0), 0)
...
hz_q3PrepL = hz_q3_upsetPrep(hz_q3RowsL, hz_q3DenomTotal, { maxIntersections: hz_q3_maxIntersections })
hz_q3PrepR = hz_q3_upsetPrep(hz_q3RowsR, hz_q3DenomTotal, { maxIntersections: hz_q3_maxIntersections })
hz_q3PrepD = hz_q3_upsetDelta(hz_q3PrepL, hz_q3PrepR, { maxIntersections: hz_q3_maxIntersections })
...
hz_q3Charts = {
  ...
  const wide2 = w >= 980
  const wide3 = w >= 1320
  const colW2 = wide2 ? Math.floor((w - gap) / 2) : Math.max(420, w)
  const colW3 = wide3 ? Math.floor((w - gap * 2) / 3) : colW2

  const h = Number(hz_q3_chartHeight) || 520

  const leftFig = hz_q3_upsetFigure(hz_q3PrepL, { width: colW3, height: h, title: `Left: ${hz_s1.label}` })
  const rightFig = hz_q3_upsetFigure(hz_q3PrepR, { width: colW3, height: h, title: `Right: ${hz_s2.label}` })

  if (hz_q3_viewMode === "diff") {
    const deltaFig = hz_q3_upsetFigure(hz_q3PrepD, { width: colW3, height: h, title: "Δ (Right − Left)", isDelta: true })
    return html`
      <div style="display:grid;gap:${gap}px;grid-template-columns:${wide3 ? "repeat(3, 1fr)" : wide2 ? "repeat(2, 1fr)" : "1fr"};">
        <div class="hz-sideCard"><div class="hz-sideCard__body">${leftFig}</div></div>
        <div class="hz-sideCard"><div class="hz-sideCard__body">${rightFig}</div></div>
        <div class="hz-sideCard" style="${wide3 ? "" : wide2 ? "grid-column:1 / -1;" : ""}">
          <div class="hz-sideCard__body">${deltaFig}</div>
        </div>
      </div>`
  }

  return html`
    <div style="display:grid;gap:${gap}px;grid-template-columns:${wide2 ? "repeat(2, 1fr)" : "1fr"};">
      <div class="hz-sideCard"><div class="hz-sideCard__body">${leftFig}</div></div>
      <div class="hz-sideCard"><div class="hz-sideCard__body">${rightFig}</div></div>
    </div>`
}
...
<div class="hz-card">
  <div class="hz-card__body">
    <div class="hz-qOverview">${q3DetailText}</div>
