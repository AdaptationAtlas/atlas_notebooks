...
```{ojs}
//| output: false
// Rows currently displayed in the Q1 chart (match the UpSet combinations + ordering)
hz_q1DisplayRows = {
  const num = (x) => {
    const v = Number(x)
...
// Q2 — displayed chart data (shared by chart + table)
hz_q2DisplayState = {
...
hz_q5_chartHost = html`
  <div class="hz-chartWrap ${hz_q5Busy ? "hz-chartWrap--busy" : ""}">
    ...
    <div class="hz-chartNote">
      ${md`${_lang({
        en: "Dashed lines show the historic (baseline) exposure. When Uncertainty is on, shaded bands and whiskers show the low–high range across GCMs where available.",
        fr: "Les lignes pointillées indiquent l’exposition historique (référence). Lorsque l’option Incertitude est activée, des bandes ombrées et des moustaches montrent la plage basse–haute entre les GCM lorsque disponible."
      })}`}
    </div>
  </div>
`
...
hz_q5_settingsWrap = html`
  <div class="hz-qToolbar__right">
    <div class="hz-inlineToggles hz-inlineToggles--grid">
      ...
      <div class="hz-miniSetting hz-miniSetting--grow"><span>Hazards</span>${hz_noJumpInput(viewof hz_q5_hazards)}</div>
      <div class="hz-miniSetting hz-miniSetting--grow"><span>Scenarios</span>${hz_noJumpInput(viewof hz_q5_scenarios)}</div>
      <div class="hz-miniSetting hz-miniSetting--grow"><span>Periods</span>${hz_noJumpInput(viewof hz_q5_timeframes)}</div>
      ...
      <div class="hz-miniSetting"><span>Free Y axis</span>${hz_noJumpInput(viewof hz_q5_freeY)}</div>
...
hz_q4Rows = {
  ...
  rows.sort((a,b) => (b.total2 - a.total2) || (b.total1 - a.total1))
  return rows.slice(0, Math.max(1, Number(hz_q4_topN ?? 10) || 10))
}
...
hz_q4RowsChart = {
  ...
  const n = Math.max(1, Number(hz_q4_topN ?? 10) || 10)
...
hz_q1_settingsWrap = {
  ...
  <div class="hz-qToolbar__right hz-qToolbar__right--grow">
    <div class="hz-q2SettingsGrid">
      ...
      <div class="hz-miniSetting"><span>Show differences</span>${viewof hz_difference}</div>
...
hz_q4_selectedGeographyCount = {
  ...
  const fromHaz = Array.isArray(hz_q4HazSetActive)
    ? Array.from(new Set((hz_q4HazSetActive ?? []).map((d) => normName(d?.admin)).filter(Boolean)))
    : []
  if (fromHaz.length) return fromHaz.length
...
viewof hz_q4_topN = hz_noJumpInput(Inputs.range([1, Math.max(1, hz_q4_selectedGeographyCount || 1)], {
  label: "",
  step: 1,
  value: Math.max(1, hz_q4_selectedGeographyCount || 1)
}))

viewof hz_q4_viewMode = hz_noJumpInput(Inputs.radio(["chart", "table"], {
  label: "",
  value: "chart",
  format: (v) => ({ chart: _lang({ en: "Chart", fr: "Graphique" }), table: _lang({ en: "Summary table", fr: "Tableau" }) }[v] ?? v)
}))
...
hz_q2_settingsWrap = html`
  <div class="hz-qToolbar__right hz-qToolbar__right--grow">
    <div class="hz-q2SettingsGrid">
      ...
      <div class="hz-miniSetting" data-hz="q2-includeDecreases" style="visibility:hidden; pointer-events:none;"><span>Include decreases</span>${viewof hz_q2_includeDecreases}</div>
...
// --------------------------
// Synced per-question filter copies (bound to shared controls)
// --------------------------
...
hz_makeBoundCommodityMulti = (masterView, options, cfg = {}) => {
  const local = hz_safeEnhancedMultiSelect(
    (options ?? []).map(v => ({ value: v, label: hz_commodityLabelOf(v) })),
    {
      label: cfg.label ?? "Commodities",
      value: Array.isArray(masterView?.value) ? masterView.value : [],
      disabled: !!cfg.disabled,
      placeholder: cfg.placeholder ?? "Search commodities…",
      maxHeight: cfg.maxHeight ?? 200,
      width: cfg.width ?? "100%"
    }
  )
  return Inputs.bind(local, masterView)
}
...
viewof hz_q1sync_commodityGroup = hz_makeBoundSelect(viewof hz_commodityGroup, hz_commodityGroupOptions ?? [], {
  label: "Group",
  value: hz_commodityGroup,
  format: hz_commodityGroupLabel
})
viewof hz_q1sync_commodities = hz_makeBoundCommodityMulti(viewof hz_commodities, hz_commodityOptions ?? [], {
  label: "Commodities",
  disabled: (hz_commodityGroup === "all")
})
viewof hz_q1sync_admin0 = Inputs.bind(renderA0Multi({ key: "sync-q1", maxSelections: 8 }), viewof hz_admin0)
viewof hz_q1sync_admin1 = Inputs.bind(renderA1Multi({ key: "sync-q1", a0: viewof hz_q1sync_admin0, maxSelections: 500 }), viewof hz_admin1)
viewof hz_q1sync_admin2 = Inputs.bind(renderA2Multi({ key: "sync-q1", a0: viewof hz_q1sync_admin0, a1: viewof hz_q1sync_admin1, maxSelections: 20 }), viewof hz_admin2)
viewof hz_q1sync_scen1 = hz_makeBoundSelect(viewof hz_scen1, hz_scenXTime ?? [], { label: "Scenario A:", value: hz_scen1, format: hz_formatScenXTime })
viewof hz_q1sync_scen2 = hz_makeBoundSelect(viewof hz_scen2, hz_scenXTime ?? [], { label: "Scenario B:", value: hz_scen2, format: hz_formatScenXTime })
hz_q1sync_filtersPanel = {
  const el = html`
        ...
        <div class="hz-controlsCompact hz-controlsPanel">
          <div class="hz-controlsRows">
            <div class="hz-row hz-row--2 hz-row--grow">
              <div class="hz-field">${viewof hz_q1sync_commodityGroup}</div>
              <div class="hz-field">${viewof hz_q1sync_commodities}</div>
            </div>
            <div class="hz-row hz-row--3">
              <div class="hz-field">${viewof hz_q1sync_admin0}</div>
              <div class="hz-field">${viewof hz_q1sync_admin1}</div>
              <div class="hz-field">${viewof hz_q1sync_admin2}</div>
            </div>
            <div class="hz-row hz-row--2">
              <div class="hz-field">${viewof hz_q1sync_scen1}</div>
              <div class="hz-field">${viewof hz_q1sync_scen2}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  `
  el.open = mutable hz_q1SyncOpen
  el.addEventListener("toggle", () => (mutable hz_q1SyncOpen = el.open))
  return el
}
hz_q1_mountFilters = {
  hz_q1_filtersHost.replaceChildren(hz_q1sync_filtersPanel)
  return null
}
...
viewof hz_q2sync_commodities = hz_makeBoundCommodityMulti(viewof hz_commodities, hz_commodityOptions ?? [], {
  label: "Commodities",
  disabled: (hz_commodityGroup === "all")
})
viewof hz_q2sync_admin0 = Inputs.bind(renderA0Multi({ key: "sync-q2", maxSelections: 8 }), viewof hz_admin0)
viewof hz_q2sync_admin1 = Inputs.bind(renderA1Multi({ key: "sync-q2", a0: viewof hz_q2sync_admin0, maxSelections: 500 }), viewof hz_admin1)
viewof hz_q2sync_admin2 = Inputs.bind(renderA2Multi({ key: "sync-q2", a0: viewof hz_q2sync_admin0, a1: viewof hz_q2sync_admin1, maxSelections: 20 }), viewof hz_admin2)
viewof hz_q2sync_scen1 = hz_makeBoundSelect(viewof hz_scen1, hz_scenXTime ?? [], { label: "Scenario A:", value: hz_scen1, format: hz_formatScenXTime })
viewof hz_q2sync_scen2 = hz_makeBoundSelect(viewof hz_scen2, hz_scenXTime ?? [], { label: "Scenario B:", value: hz_scen2, format: hz_formatScenXTime })
hz_q2sync_filtersPanel = {
  const el = html`
    <details class="hz-furl hz-furl--filters">
      <summary>
        <span>Filters</span>
      ...
      <div class="hz-furl__body">
        <div class="hz-controlsCompact hz-controlsPanel">
          <div class="hz-controlsRows">
            <div class="hz-row hz-row--2 hz-row--grow">
              <div class="hz-field">${viewof hz_q2sync_commodityGroup}</div>
              <div class="hz-field">${viewof hz_q2sync_commodities}</div>
            </div>
            <div class="hz-row hz-row--3">
              <div class="hz-field">${viewof hz_q2sync_admin0}</div>
              <div class="hz-field">${viewof hz_q2sync_admin1}</div>
              <div class="hz-field">${viewof hz_q2sync_admin2}</div>
            </div>
            <div class="hz-row hz-row--2">
              <div class="hz-field">${viewof hz_q2sync_scen1}</div>
              <div class="hz-field">${viewof hz_q2sync_scen2}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  `
  el.open = mutable hz_q2SyncOpen
  el.addEventListener("toggle", () => (mutable hz_q2SyncOpen = el.open))
  return el
}
hz_q2_mountFilters = {
  hz_q2_filtersHost.replaceChildren(hz_q2sync_filtersPanel)
  return null
}
...
viewof hz_q3sync_commodities = hz_makeBoundCommodityMulti(viewof hz_commodities, hz_commodityOptions ?? [], {
  label: "Commodities",
  disabled: (hz_commodityGroup === "all")
})
viewof hz_q3sync_admin0 = Inputs.bind(renderA0Multi({ key: "sync-q3", maxSelections: 8 }), viewof hz_admin0)
viewof hz_q3sync_admin1 = Inputs.bind(renderA1Multi({ key: "sync-q3", a0: viewof hz_q3sync_admin0, maxSelections: 500 }), viewof hz_admin1)
viewof hz_q3sync_admin2 = Inputs.bind(renderA2Multi({ key: "sync-q3", a0: viewof hz_q3sync_admin0, a1: viewof hz_q3sync_admin1, maxSelections: 20 }), viewof hz_admin2)
viewof hz_q3sync_scen1 = hz_makeBoundSelect(viewof hz_scen1, hz_scenXTime ?? [], { label: "Scenario A:", value: hz_scen1, format: hz_formatScenXTime })
viewof hz_q3sync_scen2 = hz_makeBoundSelect(viewof hz_scen2, hz_scenXTime ?? [], { label: "Scenario B:", value: hz_scen2, format: hz_formatScenXTime })
hz_q3sync_filtersPanel = {
  const el = html`
    <details class="hz-furl hz-furl--filters">
      <summary>
        <span>Filters</span>
      ...
      <div class="hz-furl__body">
        <div class="hz-controlsCompact hz-controlsPanel">
          <div class="hz-controlsRows">
            <div class="hz-row hz-row--2 hz-row--grow">
              <div class="hz-field">${viewof hz_q3sync_commodityGroup}</div>
              <div class="hz-field">${viewof hz_q3sync_commodities}</div>
            </div>
            <div class="hz-row hz-row--3">
              <div class="hz-field">${viewof hz_q3sync_admin0}</div>
              <div class="hz-field">${viewof hz_q3sync_admin1}</div>
              <div class="hz-field">${viewof hz_q3sync_admin2}</div>
            </div>
            <div class="hz-row hz-row--2">
              <div class="hz-field">${viewof hz_q3sync_scen1}</div>
              <div class="hz-field">${viewof hz_q3sync_scen2}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  `
  el.open = mutable hz_q3SyncOpen
  el.addEventListener("toggle", () => (mutable hz_q3SyncOpen = el.open))
  return el
}
hz_q3_mountFilters = {
  hz_q3_filtersHost.replaceChildren(hz_q3sync_filtersPanel)
  return null
}
...
viewof hz_q4sync_commodities = hz_makeBoundCommodityMulti(viewof hz_commodities, hz_commodityOptions ?? [], {
  label: "Commodities",
  disabled: (hz_commodityGroup === "all")
})
viewof hz_q4sync_admin0 = Inputs.bind(renderA0Multi({ key: "sync-q4", maxSelections: 8 }), viewof hz_admin0)
viewof hz_q4sync_admin1 = Inputs.bind(renderA1Multi({ key: "sync-q4", a0: viewof hz_q4sync_admin0, maxSelections: 500 }), viewof hz_admin1)
viewof hz_q4sync_admin2 = Inputs.bind(renderA2Multi({ key: "sync-q4", a0: viewof hz_q4sync_admin0, a1: viewof hz_q4sync_admin1, maxSelections: 20 }), viewof hz_admin2)
viewof hz_q4sync_scen1 = hz_makeBoundSelect(viewof hz_scen1, hz_scenXTime ?? [], { label: "Scenario A:", value: hz_scen1, format: hz_formatScenXTime })
viewof hz_q4sync_scen2 = hz_makeBoundSelect(viewof hz_scen2, hz_scenXTime ?? [], { label: "Scenario B:", value: hz_scen2, format: hz_formatScenXTime })
hz_q4sync_filtersPanel = {
  const el = html`
    <details class="hz-furl hz-furl--filters">
      <summary>
        <span>Filters</span>
      ...
      <div class="hz-furl__body">
        <div class="hz-controlsCompact hz-controlsPanel">
          <div class="hz-controlsRows">
            <div class="hz-row hz-row--2 hz-row--grow">
              <div class="hz-field">${viewof hz_q4sync_commodityGroup}</div>
              <div class="hz-field">${viewof hz_q4sync_commodities}</div>
            </div>
            <div class="hz-row hz-row--3">
              <div class="hz-field">${viewof hz_q4sync_admin0}</div>
              <div class="hz-field">${viewof hz_q4sync_admin1}</div>
              <div class="hz-field">${viewof hz_q4sync_admin2}</div>
            </div>
            <div class="hz-row hz-row--2">
              <div class="hz-field">${viewof hz_q4sync_scen1}</div>
              <div class="hz-field">${viewof hz_q4sync_scen2}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  `
  el.open = mutable hz_q4SyncOpen
  el.addEventListener("toggle", () => (mutable hz_q4SyncOpen = el.open))
  return el
}
hz_q4_mountFilters = {
  hz_q4_filtersHost.replaceChildren(hz_q4sync_filtersPanel)
  return null
}

// Q5 mirrors (geography + commodities only; scenarios stay independent in Q5 settings)
viewof hz_q5sync_commodityGroup = hz_makeBoundSelect(viewof hz_commodityGroup, hz_commodityGroupOptions ?? [], {
  label: "Group",
  value: hz_commodityGroup,
  format: hz_commodityGroupLabel
})
viewof hz_q5sync_commodities = hz_makeBoundCommodityMulti(viewof hz_commodities, hz_commodityOptions ?? [], {
  label: "Commodities",
  disabled: (hz_commodityGroup === "all")
})
viewof hz_q5sync_admin0 = Inputs.bind(renderA0Multi({ key: "sync-q5", maxSelections: 8 }), viewof hz_admin0)
viewof hz_q5sync_admin1 = Inputs.bind(renderA1Multi({ key: "sync-q5", a0: viewof hz_q5sync_admin0, maxSelections: 500 }), viewof hz_admin1)
viewof hz_q5sync_admin2 = Inputs.bind(renderA2Multi({ key: "sync-q5", a0: viewof hz_q5sync_admin0, a1: viewof hz_q5sync_admin1, maxSelections: 20 }), viewof hz_admin2)
hz_q5sync_filtersPanel = {
  const el = html`
    <details class="hz-furl hz-furl--filters">
      <summary>
        <span>Filters</span>
        <span class="hz-furl__hint">${hz_q5_sharedControlsSummary}</span>
      </summary>
      <div class="hz-furl__body">
        <div class="hz-controlsCompact hz-controlsPanel">
          <div class="hz-controlsRows">
            <div class="hz-row hz-row--2 hz-row--grow">
              <div class="hz-field">${viewof hz_q5sync_commodityGroup}</div>
              <div class="hz-field">${viewof hz_q5sync_commodities}</div>
            </div>
            <div class="hz-row hz-row--3">
              <div class="hz-field">${viewof hz_q5sync_admin0}</div>
              <div class="hz-field">${viewof hz_q5sync_admin1}</div>
              <div class="hz-field">${viewof hz_q5sync_admin2}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  `
  el.open = mutable hz_q5SyncOpen
  el.addEventListener("toggle", () => (mutable hz_q5SyncOpen = el.open))
  return el
}
hz_q5_mountFilters = {
  hz_q5_filtersHost.replaceChildren(hz_q5sync_filtersPanel)
  return null
}
...
hz_q3_settingsWrap = html`
  <div class="hz-qToolbar__right hz-qToolbar__right--grow">
    <div class="hz-q2SettingsGrid">
      ...
      <div class="hz-miniSetting"><span>Free X axis</span>${viewof hz_q3_freeX}</div>
      <div class="hz-miniSetting hz-miniSetting--range"><span>Number of commodities</span>${viewof hz_q3_topN}</div>
      <div class="hz-miniSetting hz-miniSetting--range"><span>Height</span>${viewof hz_q3_chartHeight}</div>
    </div>
  </div>
`

hz_q3ReqBody = hz_q2ReqBody

// Reuse the single shared Q2 response instead of triggering a second API call.
hz_q3Resp = hz_q2Resp
...
`
  ...
  // Compute x-domain from stacked totals (not individual segments), so bars never overflow.
  ...
  const buildPanel = (panel, i) => {
    // Show the x-axis on every hazard panel. Restricting it to only the
    // bottom row causes upper-row panels (such as dry/heat when wet is below)
    // to appear without an x-axis.
    const showX = true
  ...
  return html`
    <div class="hz-q4InsightBlock">
      <div class="hz-q4InsightLead">
        Top ${Math.max(1, Number(hz_q4_topN ?? 10) || 10)} geographies currently shown cover <strong>${fmtPct(topShareL)}</strong> of total exposure in <strong>${hz_s1.label}</strong>
...
hz_q4_settingsWrap = html`
  <div class="hz-qToolbar__right hz-qToolbar__right--grow">
    <div class="hz-q2SettingsGrid">
      <div class="hz-miniSetting"><span>Show %</span>${viewof hz_rel}</div>
      ...
      <div class="hz-miniSetting hz-miniSetting--range"><span>No. geographies to show</span>${viewof hz_q4_topN}</div>
...
hz_mountDash = {
  try {
    // Mount per-question synced filter panels before attaching the dashboard shell
    hz_q1_mountFilters
    hz_q2_mountFilters
    hz_q3_mountFilters
    hz_q4_mountFilters
