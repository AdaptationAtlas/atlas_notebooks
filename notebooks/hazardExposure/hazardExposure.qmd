...
viewof hz_question = {
  const options = [
    { value: "q1", label: "By hazard" },
    { value: "q2", label: "By crop" },
    { value: "q3", label: "Hazard × crop" },
    { value: "q4", label: "By admin" }
  ];
...
hz_registerUi = {
  ...
  ui.setBusy = (on, msg = "Updating…") => {
    try {
      if (!ui.statusPill || !ui.chartWrap) return
      ...
      if (on) {
        ...
        ui.chartWrap.classList.add("hz-chartWrap--busy")
        return
      }

      ui._busyCount = Math.max(0, ui._busyCount - 1)
      if (ui._busyCount === 0) {
        ui.chartWrap.classList.remove("hz-chartWrap--busy")
      }
    } catch (e) {}
  }
    ...
    <div class=\"hz-card\">
      <div class=\"hz-card__head\">
        <div>
          ...
          <div class=\"hz-card__sub\">${hz_dashSub}</div>\n          <div class=\"hz-card__tabs\">${viewof hz_question}</div>
        </div>
        ${hz_statusPill}
      </div>
      ${hz_chartHost}
...
hz_updateChart = {
  ...
  if (errors.length) {
    hz_chartHost.replaceChildren(html`<div class="warn"><b>Data load failed</b><br/><div class="hz-muted">${errors[0].error}</div></div>`)
    return html``
  }
