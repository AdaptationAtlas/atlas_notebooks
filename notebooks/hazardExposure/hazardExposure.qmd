...
/* Map view */
.hz-mapWrap { min-height: 460px; }
.hz-mapWrap--busy { opacity: 0.7; }
.hz-mapPlot { width: 100%; overflow: hidden; }
.hz-mapPlot svg { max-width: 100%; height: auto; display: block; }
.hz-mapMetric { display: inline-flex; align-items: center; gap: 8px; }
.hz-mapSub { margin-top: -2px; margin-bottom: 10px; }
.hz-miniControls .observablehq--inspect { display: none; }
...
viewof hz_viewMode = {
  const options = [
    { value: "chart",   label: "Chart",       icon: "📈", hint: "Interactive charts & comparisons" },
    { value: "both",    label: "Dashboard",   icon: "▤",  hint: "Chart + summary table (grouped totals)" },
    { value: "table",   label: "Summary",     icon: "▦",  hint: "Aggregated totals (paginated)" },
    { value: "map",     label: "Map",         icon: "🗺️", hint: "Choropleth by admin area (uses current filters)" },
    { value: "records", label: "Raw records", icon: "🧾", hint: "Row-level values (paginated)" }
  ]
  ...
  lab.className = "hz-viewToggle__label"
  lab.textContent = "View"

  const grp = document.createElement("div")
  grp.className = "hz-viewToggle__group"
  ...
  const btns = options.map(op => {
    ...
    b.title = op.hint || op.label
    b.innerHTML = `<span class="hz-viewBtn__icon" aria-hidden="true">${op.icon || ""}</span><span class="hz-viewBtn__txt">${op.label}</span>`
    b.onclick = () => {
      if (root.value === op.value) return
      root.value = op.value
      paint()
      root.dispatchEvent(new Event("input", { bubbles: true }))
    }
  ...
  const paint = () => {
    for (const b of btns) b.classList.toggle("is-active", b.dataset.value === root.value)
  }
  ...
  ui.setBusy = (on, msg = "Updating…") => {
    try {
      ...
      if (on) {
        ...
        try { ui.mapWrap?.classList?.add?.("hz-mapWrap--busy") } catch (_) {}
        return
      }
      ...
      if (ui._busyCount === 0) {
        ...
        try { ui.mapWrap?.classList?.remove?.("hz-mapWrap--busy") } catch (_) {}
      }
    } catch (e) {}
  }
        ...
        (hz_viewMode === "records")
          ? html`<div class="hz-sideCard">
              <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:8px">
                <div class="hz-sideCard__title" style="margin:0">Raw records</div>
          ...
          : (hz_viewMode === "map")
            ? html`<div class="hz-sideCard">
                <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:8px">
                  <div class="hz-sideCard__title" style="margin:0">Map</div>
                  <div class="hz-miniControls">${viewof hz_mapMetric}</div>
                </div>
                <div class="hz-muted hz-mapSub">${hz_mapSub}</div>
                ${hz_mapHost}
              </div>`
            : (hz_viewMode === "chart")
              ...
              : (hz_viewMode === "table")
                ? html`<div class="hz-sideCard">
                    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:8px">
                      <div class="hz-sideCard__title" style="margin:0">Summary table</div>
                      ${hz_tableActionsHost}
                    </div>
                    ${hz_tableHost}
                  </div>`
...
hz_updateChart = {
  ...
  if (hz_viewMode === "map") {
    ...
    // Loading
    if (meta.loading) {
      hz_statusPill.className = "hz-pill hz-pill--loading"
      hz_statusPill.textContent = "Loading…"
    ...
    // Error
    if (!meta.ok) {
      hz_statusPill.className = "hz-pill hz-pill--error"
      hz_statusPill.textContent = "Error"
    ...
    // Ready
    hz_statusPill.className = "hz-pill hz-pill--ready"
    hz_statusPill.textContent = "Ready"
