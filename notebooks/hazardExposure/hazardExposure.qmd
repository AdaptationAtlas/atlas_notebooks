...
hz_noJumpRoot = (node) => {
  ...
  if (root && !root[key]) {
    ...
    const settle = () => { restore(); hz_blurFocusChain(root) }
...
hz_q2_selectedCommodityCount = {
  ...
  if (picked.includes("all")) {
    const opts = Array.isArray(hz_q2_commodityOptions)
      ? hz_q2_commodityOptions.map(v => String(v ?? "").trim()).filter(Boolean)
      : []
...
hz_q1_downloadMount = {
  try {
    ...
    const data = rows.map((r) => {
      const base = { "Hazard": r?.hazard ?? "" }
      if (useDiff) {
        base[usePct ? "Δ (pp)" : "Δ (abs)"] = usePct ? (r?.pct_diff ?? null) : (r?.total_diff ?? null)
      } else {
        base[usePct ? "Left (%)" : "Left (abs)"] = usePct ? (r?.perc1 ?? null) : (r?.total1 ?? null)
        base[usePct ? "Right (%)" : "Right (abs)"] = usePct ? (r?.perc2 ?? null) : (r?.total2 ?? null)
      }
      return base
    })
...
hz_q4_selectedGeographyCount = {
  const gf = hz_q4_geoFilter ?? { admin0: [], admin1: [], admin2: [] }
...
viewof hz_q2_admin1 = renderA1Multi({ key: "q2", a0: viewof hz_q2_admin0, maxSelections: 500 })
...
viewof hz_q3_admin1 = renderA1Multi({ key: "q3", a0: viewof hz_q3_admin0, maxSelections: 500 })
...
viewof hz_q4_admin1 = renderA1Multi({ key: "q4", a0: viewof hz_q4_admin0, maxSelections: 500 })
    ...
    <div style="max-height:420px;overflow:auto;border:1px solid rgba(148,163,184,.35);border-radius:10px;">
      <table style="width:100%;border-collapse:collapse;font-size:12px;">
        <thead style="position:sticky;top:0;background:rgba(248,250,252,.98);z-index:1;">
          <tr>
            <th style="text-align:left;padding:8px;border-bottom:1px solid #e2e8f0;">${_lang({en:"Side",fr:"Côté"})}</th>
            <th style="text-align:left;padding:8px;border-bottom:1px solid #e2e8f0;">${_lang({en:"Geography",fr:"Géographie"})}</th>
            <th style="text-align:right;padding:8px;border-bottom:1px solid #e2e8f0;">${hz_rel ? _lang({en:"Percent",fr:"Pourcentage"}) : (hz_varLabelAbs ?? _lang({en:"Exposure",fr:"Exposition"}))}</th>
          </tr>
        </thead>
        <tbody>
          ${shown.map((r) => html`<tr>
