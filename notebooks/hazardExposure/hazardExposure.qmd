...
hz_q2Chart = {
  ...
  const makeSidePlot = (title, sideKey) => {
    ...
    const groupedMarks = (rows, hazards) => {
      // Use a *single* bar mark with per-row dy offsets to avoid overlapping tooltips
      const n = hazards.length
      const step = 12 // px spacing between hazard bars within the same crop band
      const idx = new Map(hazards.map((h, i) => [h, i]))
      const data2 = rows.map((d) => ({
        ...d,
        __dy: ((idx.get(d.hazard) ?? 0) - (n - 1) / 2) * step
      }))
      return [
        Plot.barX(data2, {
          x: "value",
...
hz_q5Plot = {
  ...
  const plotOpts = {
    ...
    y: { label: yLabel, grid: true, tickFormat: tickFmtY },
    color: { legend: true, label: "Scenario" },
    marks
  }
  if (modeView === "all") {
    plotOpts.fy = { label: null, domain: ["any", "dry (any)", "heat (any)", "wet (any)"], axis: "right" }
  }
...
// Hide/disable the hazard selector unless "Single hazard" is selected
hz_q5_wireHazardCtl = {
...
hz_q2_applyView = {
  ...
  // Keep toolbar stable: toggle 'Include decreases' without re-rendering (prevents scroll jumps)
  try {
...
`
  ...
  // Compute y-domain from stacked totals (not individual segments), so bars never overflow.
  ...
  for (const p of panels) {
    ...
    for (const s of scenarios) {
      yMax = Math.max(yMax, sumsPos.get(s) || 0)
      yMin = Math.min(yMin, sumsNeg.get(s) || 0)
    }
  }

  if (!includeDecreases) yMin = 0
  ...
  const buildPanel = (panel, i) => {
    ...
    const barMark = Plot.barY(data, Plot.stackY({
      x: "scenario",
      y: "value",
      fill: "crop",
      tip: { format: tipFmt }
      ...
      <div class="hz-sideCard hz-sideCard--plot">
        <div class="hz-sideCard__body">
          ...
          ${Plot.plot({
            ...
            y: { label: showY ? yLabel : null, grid: true, axis: showY ? "left" : null, domain: yDomain, tickFormat: hz_q3_relative ? fmtPct : fmtAbs },
