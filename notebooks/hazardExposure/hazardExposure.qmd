...
getExposureData = async () => {
  //TODO: this is technically incorrect as the usd column in the exposure table is vop_usd21
  // but due to some data issues, this is what we have to do for now.
  const resp = await db.query(`
    SELECT *,
      CASE unit
        WHEN 'intld15' THEN 'vop_intld15'
        WHEN 'usd' THEN 'vop_usd15'
      END AS variable
    FROM exposure
    WHERE iso3 in ('${admin0Select.map((d) => d.iso3c).join("', '")}')
    AND variable = '${mainHazVar}'
    LIMIT 5
  `);
  return resp;
};
...
viewof mainHazVar = Inputs.input("vop_usd15")
