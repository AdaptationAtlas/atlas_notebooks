...
//| output: false
{
  renderToDiv("plotRecentChanges", () => {
    ...
    return dataTable(recentChanges_plotData, 500);
  });
}
```
...
//| output: false
{
  renderToDiv("plotFutureProjections", () => {
    ...
    return dataTable(futureProjections_plotData, 500);
  });
}
```
...
futureProjections_plotData = {
  ...
  FROM futureProjections
    ...
    ${sqlWhereNull({ field: "season", value: _season })}
    ${sqlWhereNull({ field: "hazard", value: _variable })}
    AND timeperiod in ${sqlList(["2021-2040"])}
    AND scenario in ${sqlList(["ssp245", "ssp585"])}
  `);
...
admin1NamesClean = admin1Select.map((d) =>
  cleanAdminInput_SQL(d.admin1_name),
);
```
