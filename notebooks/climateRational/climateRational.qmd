...
//| output: false
{
  ...
  div.innerHTML = "";
  const viz = stackbars_hazardExposure();
  div.appendChild(viz);
}
```
...
```{ojs}
gdp_plotData = {
  const _iso3 = admin0Select.map((d) => d.iso3c);
...
pov_plotData = {
  const _iso3 = admin0Select.map((d) => d.iso3c);
  const _admin1 = admin1Select.map((d) => cleanAdminInput_SQL(d.admin1_name));
  ...
  const resp = await db.query(`
    ...
    WHERE 1=1
    AND iso3 in ('${_iso3.join("', '")}')
    AND (
     admin1_name in ('${_admin1.join("', '")}')
...
recentChanges_plotData = {
  ...
  const _iso3 = admin0Select.map((d) => d.iso3c);
  const _admin1 = admin1Select.map((d) => d.admin1_name);
...
gdpBar_keyFacts = () => {
  return createStackedBarChart({
    ...
    caption: multiLineText([
      "Data is from 2022, measured in 2015 US dollars",
      "Source: FAOSTAT",
    ]),
  });
};
...
areaBar_keyFacts = () => {
  return createStackedBarChart({
    ...
    caption: multiLineText(["Data is based on 2021 values", "Source: FAOSTAT"]),
  });
};
```
...
stackbars_hazardExposure = () => {
  // --- 1. Assign category to each crop (same as first chart) ---
  ...
  // --- 2. Compute category totals using hazard === "any" ---
  const totalsByCategory = new Map(
    d3.rollup(
      dataWithCategory.filter((d) => d.hazard === "any"),
  ...
  // --- 3. Filter to only needed timeframe/scenario AND remove "any" ---
  ...
  // --- 4. Apply prod_type logic (same as first chart) ---
  let dataForAggregation;
  if (!prod_type) {
    // Aggregate hazards by category (not crop)
    dataForAggregation = Array.from(
      d3.rollup(
        baseFiltered,
        (v) => d3.sum(v, (d) => d.value),
        (d) => d.adminName,
      ...
      ([adminName, catMap]) =>
        Array.from(catMap, ([category, tMap]) =>
          Array.from(tMap, ([timeframe, sMap]) =>
            Array.from(sMap, ([scenario, hMap]) =>
              Array.from(hMap, ([hazard, value]) => ({
                adminName,
                crop: category,
                timeframe,
                scenario,
