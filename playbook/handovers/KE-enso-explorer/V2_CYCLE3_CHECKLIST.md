...
below refer to v2.1 numbering — several will shift once reordering lands. **Status: QUEUED — do not
start until usage resets.** Cycle 3 = (re-run adversarial round 3, which was stopped mid-flight:
...
- [ ] **A1 [P2, P4] Adopt climateRationale plot components with minimal changes.** Study
...
- [ ] **A2 [P5] "About this plot" inline with the download button** — same row, not below it.
- [ ] **A3 [P8] "Download full dataset" per section** + dual citation. Link to the current store
...
- [ ] **A4 [P12, P20, P21a, P22, P24a] Time-axis policy: every figure runs from its data start to
...
- [ ] **A5 [P25] Clear section dividers/headers.** Reader currently can't tell when a new beat
  starts — add strong visual section breaks (banner/rule/numbered header treatment).
- [ ] **A6 [P10] Tooltips on every figure** (2.1 currently reads as having none — verify tip
  rendering in a real browser; climateRationale mouseover pattern per A1).
- [ ] **A7 [P27, P28] Kill the §3-vs-§5 duplication via integrated county multi-select.** 3.3↔5.2
...
- [ ] **A8 [P19, P30, P30.1b] Driver framework across all event/impact figures (3.2–3.9).**
...
- [ ] **B1 [P1] Reorder: produce figure (1.3) before VoP (1.2).**
- [ ] **B2 [P2] Rebuild 1.2 + 1.3 on the climateRationale plot code** (controls, views, tooltips).
- [ ] **B3 [P3] 1.3 "Show" = multiselect** (Crops + Livestock + Products selectable together),
  all selected by default.
- [ ] **B4 [P6] 1.3: drop "Show data table"** — redundant with the Table view.
- [ ] **B5 [P14] 1.3 explicit year control.** Bars/treemap currently ambiguous about the year.
  Bars: multi-year = grouped side-by-side (not stacked). Treemap: single-year selector.
- [ ] **B6 [P15] Kitui livestock 2021–2023 vs crops 2019–2024 — CONFIRMED CORRECT** (KNBS NAPR
...
- [ ] **B7 [P7] 1.4 GESI:** add a section introduction; explain the indicators (truncated labels
...
- [ ] **C1 [P9] 2.1 distribution toggle.** Bars are county pixel means — `chirps_county.value_sd`
  is already served; add a spread/distribution view (error bars / band) as a toggle.
- [ ] **C2 [P10] 2.1 mouseover** (see A6).
- [ ] **C3 [P11] 2.1 driver grid upgrades:** diverging palette encoding **strength** (not just
  categorical phase); toggle continuous strip vs discrete phase blocks.
- [ ] **C4 [P13] 2.1 has no x-axis — fix** (the marginBottom:8 main-panel/strip split suppressed
  the axis; ensure a visible year axis).
- [ ] **C5 [P16] 2.3 contingency redesign (cognitive load too high):** (1) % labels on the bars;
  ...
  selector.
- [ ] **C6 [P12] Extend to MAM 2026** where served.

## D · Section 3 (events) — includes the flagship rework

- [ ] **D1 [P17] 3.1 event timeline unreadable** (label collisions, screenshot on file). Redesign:
  proper label layout (leader lines / two-row lanes / smaller set), or fold into a cleaner device.
- [ ] **D2 [P18] 3.2: explain the ENSO/La Niña band shading** (legend for the event-band colours).
- [ ] **D3 [P19] Driver selector + background-shading toggle for 3.2–3.9** (see A8).
- [ ] **D4 [P21b] 3.4 driver-strength discrimination:** strong vs moderate/weak groupings (not just
  phase) in the by-phase view.
- [ ] **D5 [P31] 3.4 layout: OND left, MAM right, selector to show/remove MAM.**
- [ ] **D6 [P21a] 3.6 prices:** default view = **change vs year earlier**; x from data start;
  document the method carefully (inflation + lagged effects).
- [ ] **D7 [P22] 3.7 ToT:** x starts 2010 (data start); replace faint background bands with
  **bars/points coloured by phase**.
- [ ] **D8 [P23] 3.8 production panel = THE most important plot — major rework:**
...
- [ ] **D9 [P24] 3.9 ReliefWeb:** x from 2010; replace hard-to-see background bands with the
  2.1-style phase/strength strip.
- [ ] **D10 [P27-second] Move Fig 4.3 (MAM Western-V history) into §3** — it is historical
  material, not outlook.

## E · Section 4 (coming season)

- [ ] **E1 [P26] Fix the temporal ambiguity (current vs coming season).** Minimal forecast
  content/effort; primarily LINK OUT to mandated sources (KMD / Kenya gov / met office). Trim
  accordingly.
- [ ] **E2 [P30.1a] Sidebar/top-bar fixed strip: current/next-season rainfall prediction for the
...
- [ ] **E3 [P30.1b] "Similar seasons" highlight/filter** across notebook visualizations (see A8d).

## F · Explore / Annex

- [ ] **F1 [P29] 5.3 imports × commodity-exchange prices:** investigate combining the import/export
...
- [ ] **F2 [P30] A2.1 takeaway drives a strategic revision:** if Western-V predicts little of MAM,
...
- [ ] Re-run adversarial round 3 (stopped for usage) and fix its findings.
