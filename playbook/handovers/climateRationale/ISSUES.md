...
- **STATUS:** ✗ **OBSOLETE — target plots no longer exist.** Closed 2026-06-05 without code change. Rationale: `barplot_recentChanges` + `warmingStripes_recentChanges` were deleted on 2026-05-21 in commit `5c730e2` ("integrate observational sandbox into Recent Changes") when the Recent Changes section was rebuilt around the CHIRPS v3 + CHIRTS-ERA5 observational record (CR-062's premise). The observational view is a single-realisation record (not an ensemble) so the inter-model ribbon CR-061 proposed is not a meaningful quantity there — the observational plot already carries an indicative observational-uncertainty band (CR-093) and the Mann-Kendall / Theil-Sen trend line. The substantive uncertainty-ribbon work has effectively migrated into the [[CR-060]] notebook-side delivery (Future Projections q17/q83 swap) which landed 2026-06-05. Prior 2026-05-18 deprioritisation notes preserved below for historical context.
  - *2026-05-18 historical note:* Deprioritised pending CR-062 because the Recent Changes section was about to flip to observational. CR-062 landed 2026-05-21 (commit 5c730e2); the model-hindcast bar/stripes plots were removed in the same commit, retiring CR-061's target surface.
  - *2026-05-27 historical note:* `ensemble_season_timeseries.parquet` was republished with `min` / `max` / `min_anomaly` / `max_anomaly` columns added — would have been tempting raw material for the ribbon, but `min`/`max` ≠ AR6 calibrated language anyway. CR-060 percentile columns are the right inputs, and they landed in the Future Projections plot instead.
- **before-string:**
  ```js
...
- **STATUS: ✅ RESOLVED — ROUTED (2026-06-15 pipeline triage).** Probes had in fact run 2026-06-01 (results in pipeline memory). Final disposition, each finding routed to a single owner:
  ...
  - **(b) historic NDWS saturation + (c) Luanda NaN residual — UPSTREAM**, `AdaptationAtlas/hazards`. (c) is downstream of (b) (180/6,021 AGO cells, all Luanda admin1 = the saturation signature) — closes when (b) is fixed and historic NDWS republished. Filed 2026-06-15 → [AdaptationAtlas/hazards#19](https://github.com/AdaptationAtlas/hazards/issues/19) (cross-linked from hazards_prototype#12; only a dispatch existed before).
...
- **FOLLOW-UP 2026-07-09 (commit `830b247`) — width clamp so the *open* TOC also can't overlap.** The toggle + auto-collapse (< 1480 px) fixed first-paint overlap, but between ~1480 px and the point the content column stops growing the open TOC still crossed into the main pane (Pete screenshot, Kenya map partly under the sidebar). Fixed by capping the panel to the free gutter beside the 1180 px body column: `width: clamp(100px, calc((100vw - 1180px) / 2 - 30px), 300px)` + `box-sizing: border-box`. **Scope divergence from the original ticket:** CR-074 deliberately kept its rules in `notebook.qmd` only, "does not touch `helpers/toc.ojs`". This clamp is a property of the shared TOC geometry, so it was applied in **both** the runtime-injected rule (`helpers/toc.ojs` → `ensureAtlasTOCStyles`, which wins the cascade) **and** the static `styles.css` `.atlas-toc` rule, with a cross-reference comment in each. Other notebooks that call `atlasTOC` inherit the same clamp — desired (they share the 1180 px body-width). If a future notebook uses a different `grid.body-width`, the 1180 constant needs to move to a CSS variable.
...
- **STATUS:** ✓ **DELIVERED — pipeline + canonical + notebook all landed.**
  - Pipeline code SHIPPED 2026-06-01 (`hazards_prototype` commit `f42d720`); `R/2.1_create_monthly_haz_tables.R` section 3.3 computes `q5`, `q17`, `q50`, `q83`, `q95` and their `_anomaly` variants + `n_models` alongside the existing `mean`/`sd` (schema extension, backwards-compatible).
  - Canonical published 2026-06-05 (probe 20/20 PASS).
  - **Notebook side landed 2026-06-05** (this commit). `futureProjections_dataAll` SQL now SELECTs `q17`, `q83`, `q17_anomaly`, `q83_anomaly`, `n_models` alongside the existing `mean`/`sd`. `timeseries_futureProjections` ribbon swapped from `mean ± sd_anomaly` to `q17_anomaly..q83_anomaly` (anomaly mode) / `q17..q83` (absolute mode). `summary_futureProjections` whiskers reuse the same percentiles; baseline row still uses interannual ±1 SD (different quantity, kept and explained in the new caveat). Caption + nbText strings updated to "inter-model 17–83 % range = IPCC AR6 'likely' calibrated language, computed directly from the per-GCM percentiles (no Gaussian approximation)". Tooltip channels gain `q17_83` (range) + `n_models` (ensemble size).
- **before-string:** n/a (schema + aggregation change).
...
- **STATUS:** ⤳ **MERGED into [[CR-115]] (2026-06-15).** Same root cause — disputed-territory slivers (Abyei/Bīr Ṭawīl/Hala'ib/Ilemi) attributed to claimant iso3 at adm0, with obs vs NEX-GDDP using different conventions (separate adm0 rows vs collapse-to-admin1 + double-attribution). Both pipelines need the **same Atlas-wide convention**, now escalated to Brayden in [data-management#3](https://github.com/AdaptationAtlas/data-management/issues/3) (producer tracking [hazards_prototype#11](https://github.com/AdaptationAtlas/hazards_prototype/issues/11)). The obs-vs-NEX-GDDP **parity** requirement (point 3 below) + the double-attribution resolution (point 2) are added scope on the data-management#3 thread. Pipeline fix ON HOLD pending the convention. **Short-term notebook workaround (2026-05-20):** filter observational data to the `gaul0_code` with the largest number of admin1 polygons per iso3 (always picks the main country — disputed-territory polygons reliably have exactly 1 admin1 each). Stays meanwhile.
...
- **UPDATE 2026-06-15 — Option 1 chosen (Pete): make R/2.2 a single canonical, re-usable source (no divergent legacy/notebook copies). Producer fix DONE + committed (`hazards_prototype@17ce8af`, develop).** The outputs were unconsumable for the same 3 reasons CR-119 fixed: iso3 dropped at `merge_admin_extract`, legacy `s3://digital-atlas/hazards/...` path, plain `arrow::write_parquet` (no pruning). Fix: retain + thread iso3 through all 5 ensemble `by=` keys + 3 base_areas merges; all 10 writes → `write_parquet_pushdown` (iso3-first); added `SKIP_R22_SEC{1,2,3,4}` section guards + timestamped logging (R/2.1-saga best practice). Also fixed a latent **pre-existing** bug: SEC1 (PTOT) referenced an undefined `file_hist` that only resolved via a global leaked from `2_calculate_haz_freq.R` — restored the per-historic-file loop with loud kill-gates. Reviewer-clean; local probe PASS. **NEXT (gated):** cglabs run of R/2.2 → `R/validate_cr093_real.R` hard gate → only then wire `push_to_s3` to a canonical `domain=` path + dispatch the notebook session to confirm tokens. **Publish path + the 3 notebook feature directions remain parked until the real-output gate passes.**

- **STATUS 2026-06-24 — ✅ RESOLVED (producer + publish). Canonical consumable source delivered & live.** The cglabs Claude Code session (running on-server with live `Data/`) revived R/2.2 end-to-end: migrated to the new `admin_extract(boundaries_zonal, boundaries_index)` API (mirrors R/2.1:111-132, dedup index by zone_id, gaul-code joins); fixed SEC1 (drop `_sd`/ENSEMBLE, dual historic-prefix dedup) and reconciled SEC2/3/4 to the dash-delimited risk-dir grammar (`THI-max-max-G..`, `NTxNN-mean-G..`, `NDWS-mean-G..`). **`R/validate_cr093_real.R` = 10 PASS / 0 FAIL on live Data/.** Published to canonical keys (`hazards_prototype@c9f1e73`, `R/publish_cr093_r22.R`) and verified live over HTTPS — all 10 reachable + iso3-prunable, null-stat=0: `s3://digital-atlas/domain=climate/type=hazard-indices/source=nex-gddp-cmip6/region=africa/processing=hazard-change/timeframe=annual/variable=<name>.parquet`. Catalogued: `hazard_change` record in `metadata/data.json` (`@9ca3c23`). Full detail: `hazards_prototype/R/HANDOFF_cr093_r22.md` + `HANDOVER_macbook_cr093.md`.
  - **Carve-outs (do NOT block this closure; tracked separately):**
    1. **Notebook FEATURE surfacing** (the 3 directions above — % area-exposed view, model-agreement `sd` badge, drought event-count overlay) remain **OPEN as follow-ups**. The data is now published + consumable; wiring it into the notebook UI is net-new feature work, not part of the producer/publish goal closed here.
    2. **Desert false-"increase" in PTOT %-change** (`ISSUE_cr093_nan_zeroprecip.md`, cause 1): hyper-arid `past≈0` makes `100·(future−past)/past` → Inf, counted as an increase. R/2.2 NA-cleans non-finite payloads (ships clean NULLs) but that's a band-aid — the finite-but-wrong over-count needs an R/2-rebake baseline mask. **Parameterized fix DRAFTED** (`hazards_prototype@ffb3cdd`, `PTOT_BASELINE_MIN_MM`, default-off); threshold X is a science decision, activated at the next R/2 rebake (bundle with poultry-THI `#13` + CR-068). That issue stays OPEN until the rebake.
...
| **L** | `feat/cr-recent-changes-uncertainty-band` | CR-061 | ✗ **OBSOLETE 2026-06-05.** Target plots removed 2026-05-21 (CR-062 observational rebuild). The substantive ribbon swap landed inside the [[CR-060]] notebook-side delivery on `timeseries_futureProjections` + `summary_futureProjections`. | — |
...
| **U-2** | [[CR-060]] — Bake `q5` / `q17` / `q50` / `q83` / `q95` / `n_models` into projections parquet | `timeseries_futureProjections` + `summary_futureProjections` ribbon/whisker swap to `q17_anomaly..q83_anomaly`. PR-L (CR-061) closed as obsolete (Recent Changes is observational since 2026-05-21). | ✓ **DELIVERED 2026-06-05** end-to-end — pipeline, canonical (probe 20/20 PASS), AND notebook landed. Brayden flagged: live consumers of `ensemble_season_timeseries.parquet` see new `q5..q95` + `n_models` columns alongside the existing `mean`/`sd`; downstream code that only reads `mean`/`sd` keeps working. | M (pipeline + notebook) |
...
- **ESCALATED 2026-06-15 (pipeline triage) — root cause is an Atlas-wide convention, not a local fix.** Diagnosed in `hazards_prototype/R/observational/3_extract_obs_admin.R:245` (`zone_id = gaul0_code` at adm0) + `:527` merge. The duplicate rows are disputed-territory slivers (gaul0 100 Abyei / 110 Bīr Ṭawīl / 133 Hala'Ib / 135 Ilemi; mapping in `R/misc/calc_disputed_weights.R`) attributed to a claimant iso3 → a country gets its main gaul0 row + each claimed sliver row. **How disputed territory is attributed/aggregated affects EVERY admin-extracted table in the AAA Atlas (exposure, hazard, observational, …), so it's a cross-cutting convention decision — not the pipeline session's to set unilaterally.** Pete's call: flag for Brayden to define the convention. Options surfaced: (1) drop slivers (country = main polygon only); (2) area-weighted collapse counting each sliver **for every claimant** (politically-neutral; pooled mean/sd by obs-grid cell count — Pete's preferred direction); (3) simple mean (current client hack, over-weights slivers). **Filed 2026-06-15:** convention escalation for Brayden → [data-management#3](https://github.com/AdaptationAtlas/data-management/issues/3); producer-side tracking issue → [hazards_prototype#11](https://github.com/AdaptationAtlas/hazards_prototype/issues/11) (cross-linked). Pipeline fix ON HOLD pending the convention. Sandbox `GROUP BY/AVG` mitigation stays meanwhile.
- **BRAYDEN ANSWERED 2026-06-15 ([data-management#2](https://github.com/AdaptationAtlas/data-management/issues/2) comments) — convention = OPTION 2, function already exists.** He agrees option #2 (count disputed area for **every claimant**) "for now", and a producer-side implementation already exists: **`hazards_prototype/R/haz_functions.R` → `aggregate_disputedRegions(data, columns, fun)`** (on github `main`; local repo copy is stale, scripts source from `main` via URL). It carries the disputed→claimant map + obs-grid cell-count weights (`metadata/disputed_areas.csv`), targets adm0 rows (`gaul0_code %in% disputed & is.na(admin1_name)`), merges each sliver into every claimant; `fun="mean"` = **cell-count weighted mean** (= Pete's preferred pooled stat); idempotent. So the convention question is answered; wiring = call it at the adm0 aggregation step in the producers + re-bake.
- **IMPLEMENTATION ON HOLD (Pete, 2026-06-15) — pending Brayden's H3/A5 test.** Brayden is testing an alternative: extract to an **H3 / A5 grid index** instead of 3 admin extractions, convert admin polygons to the same index, match on a standardized grid id → swap polygons in/out with no re-extraction (wants it for the hub; checking accuracy vs zonal this afternoon). If that approach wins the wiring differs, so the pipeline does NOT wire `aggregate_disputedRegions` + re-bake until his result lands. Sandbox mitigation stays meanwhile.
- **NOTEBOOK-SIDE MAP FIX 2026-07-09 (commit `830b247`) — separate surface from the data-query workaround.** The disputed-territory duplication also broke the **Recent Changes observational map** (`recentChangesMap_obs`), a path the CR-075 data-query workaround (pick-the-gaul0-with-most-admin1s) does NOT cover. `admin0_feature_obs` fetched the country outline with `admin0_geo_obs.features.find(iso3===…)` — for KEN that returned the **first** matching adm0 polygon, the Ilemi Triangle sliver (gaul0 135), not Kenya proper (gaul0 137). Downstream the COG fetch bbox = the sliver's bbox → the whole rendered map clipped to a tiny triangle (Pete screenshot; status line showed `sub-window 24×9`). Same latent breakage for **EGY / SDN / SSD** (each ships 2–4 adm0 polygons per iso3). **Fix:** `admin0_feature_obs` now `.filter()`s *all* iso3 matches and merges their rings into one `MultiPolygon` (mask + outline paths already handle MultiPolygon), so the bbox/mask cover the whole country. This is a **stop-gap that stays until CR-115 lands the pipeline convention** — once the producer collapses/attributes slivers per `aggregate_disputedRegions`, the topojson will carry one adm0 polygon per iso3 and the merge becomes a no-op (harmless). Note it is a *geometry/COG-fetch* fix, independent of the *data-aggregation* question CR-115 is really about (national mean over sliver rows) — the map paints raster cells, not admin-averaged values, so it was never subject to the double-attribution mean bias.
...
- **STATUS:** ✅ RESOLVED 2026-06-12 (see the RESOLVED bullet at the end of this block) — A republished prunable + notebook backtick blocker fixed (`b44f19d`); only the in-browser perf number is unconfirmed. History below. ~~🔄 Dispatched 2026-06-05. Awaiting canonical rollback. Notebook SELECT defensively reverted to legacy columns.~~ **Permanent-fix architecture** lives in [`dispatches/2026-06-05_cmip6-parquet-permanent-fix.md`](dispatches/2026-06-05_cmip6-parquet-permanent-fix.md) — three changes ranked by leverage: (1) drop `models` from rows → parquet kv-metadata (~−40 % size), (2) per-iso3 hive partitioning (~−50× cold-fetch I/O for single-country reads), (3) iso3-prefixed sort + smaller row groups + pre-publish `probe_parquet()` smoke test that would have caught all three of today's regressions before they shipped. Cross-referenced against `hazards_prototype` commits since 2026-06-02 — sec 3.3 was parallelised on 2026-06-03 (commit `9d54147`), which is the most likely thrift-corruption suspect (concurrent `future_lapply` workers may be racing the same `write_parquet_pushdown` output path).

- **UPDATE 2026-06-09 — browser verification + size diagnosis CORRECTED (notebook side):** Drove a real chromium browser against the rendered `_site` production notebook + ran the notebook's exact query directly in DuckDB-WASM against the live S3 files. Findings:
  - **Live canonical = version published 2026-06-05 16:34 UTC, ~107 MB/future file.** S3 version history (`aws s3api list-object-versions`) shows the rollback target is NOT a small file — futures have been **~107 MB for their entire history** (2025-11-18 110 MB · 2026-01-21 107 MB · 2026-05-26 107 MB · 2026-06-01 124 MB · **2026-06-05 12:02 = 310 MB (the bad rebake)** · **2026-06-05 16:34 = 107 MB (current)**). The 14× claim was vs an imagined ~20 MB baseline that never existed for the future files.
  - **#1 iso3 ✅ / #3 thrift ✅ confirmed on the live 16:34 file** — `iso3` present (55 distinct), full GROUP BY/DISTINCT over 7 M rows clean, MD5 == ETag. Rollback fixed the two *error* regressions.
  - **"Not loading" is now a PERFORMANCE failure, not an error.** Notebook's Future Projections / Extreme Events chart sticks on "Loading data…" indefinitely. DuckDB-WASM running the notebook's exact Angola-single-country query: view-create 5.4 s + query **39.7 s** isolated; **>120 s in-page** under section contention → loader never clears. Desktop DuckDB returns the same 320 rows instantly → pure WASM I/O/decompress overhead over 5 × 107 MB.
  - **❌ CORRECTION — `models` is NOT the size driver. Removing it saves ~0 MB.** `parquet_metadata` per-column compressed size on the live file: **`models` = 0.0 MB** (dict-encoded, 1 distinct value). The weight is **8 float64 stat columns** (`mean, max, min, sd` + 4× `_anomaly`), **~8–15 MB each = ~107 MB total**. Proposed-change #2(c) and permanent-fix item (1) ("drop models → −40 %") are WRONG — `models` already compresses to nothing. The 310 MB spike was the **CR-060 quantile columns** (extra float64 cols) ± row duplication, never `models`.
  - **~107 MB is the longstanding norm, so the WASM slowness is baseline — not introduced by the regression.** Rollback could never fix it; the pre-regression files were also ~107 MB.
  - **REPRIORITIZED permanent fix (what actually unblocks the WASM reader):**
    1. **Per-iso3 hive partitioning** (was item 2, now #1) — single-country read ≈ 5 MB vs scanning 5 × 107 MB. The real lever.
  ...
  - **→ FIX RECOMMENDATION for the pipeline session (2026-06-11):** [`dispatches/2026-06-11_pipeline-iso3-prunability-guard.md`](dispatches/2026-06-11_pipeline-iso3-prunability-guard.md). `R/2.1` already sorts iso3-first in the canonical write, so a **sec-3.3 rerun + republish** produces a prunable file (the live file is just stale, pre-fix); optional cheap guard = add `iso3` to `verify_stats_on`. Validated synthetically (140 row groups, 0 null iso3 stats). Pipeline edits are the pipeline session's to make.
  - **✅ ROOT CAUSE CONFIRMED (2026-06-10):** the file has **no `iso3` row-group statistics** (`parquet_metadata` → `stats_min`/`stats_max` null on all 7 row groups). DuckDB-WASM therefore can't prune by country; every single-country query scans the whole file (~60 MB) → 10–50 s → chart never renders in-browser. Verified against the working sibling (**Hazard Exposure**, same gate, prunable file, renders fine). **No notebook fix possible** — exhausted per-period filter, single-file view, `read_parquet` vs `parquet_scan`. **The one fix is pipeline-side: write rows sorted by `iso3` so row-group stats prune** (cheapest), ± per-iso3 partition ± drop the 4 unused stat columns. NB: headless browser mis-reproduces these gated DuckDB sections (hangs even the working Hazard Exposure) → FP fixes must be verified in a REAL browser.
  - **✅ RESOLVED (2026-06-12) — pipeline republished A prunable + notebook blocker fixed.** Two independent failures, both now closed:
    1. **Pipeline (the perf wall):** pipeline session republished the canonical `ensemble_season_timeseries.parquet` (5 keys: 4 futures + baseline), **iso3-first sorted with non-null row-group stats** — verified against live S3 (155 row groups, 0 NULL iso3 stats, min=AGO/max=ZWE). Single-country `iso3='X'` now prunes to a handful of row groups. CR-060 ribbon cols (`q17/q83/q17_anomaly/q83_anomaly/n_models=18`) restored to the schema + confirmed populated. Dropped the 4 unused stat cols (`max/min/max_anomaly/min_anomaly`) + `q5/q50/q95`. Dispatches: [`2026-06-12_notebook-consumes-A-not-B.md`](dispatches/2026-06-12_notebook-consumes-A-not-B.md), [`2026-06-12_notebook-confirms-A-republish.md`](dispatches/2026-06-12_notebook-confirms-A-republish.md).
...
- **UPDATE 2026-06-12 — a simpler ensemble trend product (B) is being published, distinct from this ticket.** The pipeline is publishing `ensemble_season_trends.parquet` (call it "B") alongside the republished A — schema `iso3, admin0_name, admin1_name, scenario, timeframe, season, hazard, stat, mean, sd` with `stat ∈ {value_slope, value_decade, value_pval}`, `mean`/`sd` = across-GCM ensemble; iso3-first sorted + prunable; single `baseline=1995-2014` variant (trend metrics are baseline-invariant). **This is NOT CR-117** — B is the *ensemble-mean* trend (one slope per group), whereas CR-117 is *per-GCM quantile-trend slopes* (q10/q50/q90, tail-vs-median question). B supports a basic future-projection **trend/significance map**; CR-117 supports the **quantile-ladder** (upper-tail-acceleration). They are orthogonal; both could feed the same future sandbox section.
  - **Notebook position (dispatch [`2026-06-12_publish-B-recommendation.md`](dispatches/2026-06-12_publish-B-recommendation.md)):** publishing B is fine — separate keys, tiny, zero impact on A; and it's the *only* path to a future-projection trend-significance layer (the notebook's MK+Theil-Sen runs client-side on the **observed record only**; future = 4 ensemble-period points, can't fit a test). Two caveats relayed: (1) confirm `value_slope` is per-GCM-then-ensembled per [[ensembling-is-always-last]], not slope-of-the-mean; (2) `mean`/`sd` of `value_pval` is statistically weak — prefer an AR6 sign-agreement / `pct_gcms_sig` metric for the significance layer (matches CR-116's SNR robustness convention).
  - **Build plan:** B stays **unwired in production**. Prototyped in a **dedicated standalone sandbox** [`notebooks/sandbox/future_trend_map.qmd`](notebooks/sandbox/future_trend_map.qmd) (separate page, own data layer — NOT folded into obs_month_overlay.qmd). Promoted to the production notebook after Pete signs off (CR-097/CR-116 pattern + [`sandbox-vs-notebook-parity.md`](sandbox-vs-notebook-parity.md)).
  - **✅ RESOLVED 2026-06-12 — both caveats handled, B published.** Pipeline reply: **(1)** `value_slope` confirmed per-GCM-then-ensembled (fit_keys include `model`; mean/sd across models in §3.7.1 — inter-model `sd` preserved). **(2)** `value_pval` **dropped** (mean-of-pvals uninterpretable); B ships `value_slope` + `value_decade` only. The AR6 agreement metric (`pct_gcms_sig` = frac GCMs w/ significant slope; `pct_sign_pos` = frac slope>0) will be computed per-GCM in §3.7.1 and added later under this ticket — member-level pvals retained upstream in `*_trends.parquet` so no refit needed. **B is live on S3** (4 future periods, iso3-first/prunable; verified: AGO = 18 admin1 × 4 SSP × 13 seasons × {slope,decade}). The sandbox renders real B data now (29 admin1 paths painted, 0 no-data); significance overlay left as a documented stub until the agreement metric lands. See [`dispatches/2026-06-12_publish-B-recommendation.md`](dispatches/2026-06-12_publish-B-recommendation.md).

- **see also:** [[CR-060]] (level percentiles — separate stat); [[CR-093]] (R/2.2 ensemble-change info already in pipeline — different but adjacent surface); [[CR-094]] (TFPW alignment for ensemble-mean trend, also pipeline-side); [[CR-116]] (sandbox period-maps — robustness/agreement convention B's significance layer should match).
...
- **✅ PUBLISHED 2026-06-15 (pipeline).** Built standalone (`hazards_prototype/R/build_publish_C.R`, no §3.4 rerun) — detrend `value` by B's per-GCM Theil-Sen slope, `iav_sd = sd(resid)` per GCM; per-GCM `delta = future − baseline` then ensembled; `pct_gcms_increase` agreement (not mean-pval, per [[ensembling-is-always-last]] / the same call that dropped `value_pval` from B). Live on S3 at `variable=ensemble_season_variability.parquet`, period={1995-2014, 4 futures}/baseline=1995-2014, iso3-first sorted + prunable (verified null-stat=0). Schema `iso3, admin0_name, admin1_name, scenario, timeframe, season, hazard, stat∈{iav_sd,iav_delta}, mean, sd, pct_gcms_increase` (baseline file = `iav_sd` only). **Caveat:** NDD has no 1995-2014 historical baseline → its `iav_delta`/`pct_gcms_increase` null by design (~11% of future `iav_delta` rows, 98% NDD; member-level match 97%); absolute `iav_sd` present for all. n=20/σ sampling caveat noted. Reply: [`dispatches/2026-06-15_pipeline-C-variability-published.md`](dispatches/2026-06-15_pipeline-C-variability-published.md). Added to STAC issue [data-management#2](https://github.com/AdaptationAtlas/data-management/issues/2). Notebook side (sandbox IAV metric) remains CR-120 follow-up.
