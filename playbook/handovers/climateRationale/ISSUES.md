...
- **STATUS:** 🔄 **Pipeline code SHIPPED 2026-06-01** (`hazards_prototype` commit `f42d720`). `R/2.1_create_monthly_haz_tables.R` section 3.3 now computes `q5`, `q17`, `q50`, `q83`, `q95`, `q5_anomaly`, `q17_anomaly`, `q50_anomaly`, `q83_anomaly`, `q95_anomaly`, `n_models` alongside the existing `mean`/`sd` columns (schema extension, backwards-compatible). Also adds `q17_anomaly`, `q83_anomaly`, `n_models` to the period-aggregate `data_ag_ens`. The parquets on S3 still reflect the OLD schema — columns will land once the **R/2.1 rerun** completes and publishes to canonical (see "Strategy" section). Notebook-side CR-061 ribbon swap can be dispatched once the new canonical is live.
- **before-string:** n/a (schema + aggregation change).
