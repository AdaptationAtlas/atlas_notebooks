...
- **Latent follow-up not addressed**: when threshold expansion DOES fire (Highlight ON), `maxStd2 = 2 × max(baselineStdByAdmin.values())` uses whichever hazard Recent Changes happens to be on — Recent Changes' `climateVarSelect` and Future Projections' `climateVarSelectFuture` are independent since `bb18ba2` (session 16). The threshold span can be wildly wrong (e.g. PTOT's mm-scale std stretching a TAVG °C chart). Possible fixes: (a) compute `baselineStdByAdmin` per-hazard and look up by `climateVarSelectFuture.id` at render time; (b) restructure `recentChanges_plotData` to expose per-hazard stds in a single map. ~10-line fix once the per-hazard structure is decided. Defer until someone flags wrong-looking threshold lines with Highlight ON (the visible bug is gone now).
