...
>
> **2026-05-26 root-cause walk complete — bug located in `AdaptationAtlas/hazards`.** Stage 1 + Stage 2A probes in `hazards_prototype/scripts/2026-05-26_cr068_stage1_raster_probe.sh.txt` + `..._stage2a_ndws_root_cause.sh.txt` confirmed: classified NDWS rasters saturated for historic only (mean ≈ 1.0, pct_eq_max ≈ 100 %) and the saturation is INHERITED from the raw monthly NDWS indices at `cmip6/indices/historical_<GCM>_1995_2014/NDWS/NDWS-YYYY-MM.tif` (mean ≈ 29.3 of ~30 days per pixel per month — every pixel water-stressed nearly every day, every month, every year of 1995-2014). Same GCM (ACCESS-CM2) under ssp245 2041-2060 produces sensible variance (mean ≈ 21.7, min reaching 0). **Classification logic in `hazards_prototype/R/2_calculate_haz_freq.R` is correct on broken inputs** — the bug is at the source. Stage 2B handed to whoever maintains `AdaptationAtlas/hazards`: see `dispatches/2026-05-26_hazards-repo-ndws-historic-saturation.md`. CR-068(b) is now superseded for this repo: the root cause is no longer in `hazards_prototype`, so (b)'s work moves to the upstream repo.
>
> **2026-05-26/27 update — CR-068 (a) + (c) code shipped; AC re-bake in flight.** Sibling investigations under the same CR-068 umbrella located two more concrete bugs in this repo (separate from (b)'s upstream-NDWS issue):
>   - **(c) SSP370 2041+ ENSEMBLE NaN** — Stage 2B + 2C + 3 probes found three `mean(...)` / `terra::app(..., fun=sd)` calls in `R/2_calculate_haz_freq.R` ENSEMBLE writers (lines 794, 1137, 1394) without `na.rm = TRUE`. Per-GCM SSP370 2041+ rasters contain NaN pixels (some GCMs don't run those periods cleanly); the missing-na.rm averaging poisons every pixel of the ENSEMBLEmean output, R/3 reads NaN, parquet shows zeros. Fix shipped in commit `8d559b3` — six `na.rm = TRUE` additions across the three writer sites.
>   - **(a) Missing `hazard='none'` row** — Pete's original 2026-05-14 finding. Fix shipped in commit `41c1c00`: new `none_haz_mean <- 1 - any_haz_mean` layer at R/2 sec 5.2, named `<scen_mod_time_choice>_none`, appended to `int_any`. Propagates through R/3 sec 4 zonal aggregation so `value(none) + value(any) = total_VoP(admin1, crop)` in the published parquet — % exposed denominator becomes self-contained, unblocking [[CR-049]] Phase 1 resume once the rebake lands.
>   - **Tooling** — commit `01fce75` adds FORCE_OVERWRITE + SKIP_R2_RUN1 env-var support to R/2 (matches R/3's existing convention) and the bundled AC re-bake runbook `scripts/2026-05-26_cr068_ac_rebake.sh.txt`. Commit `f40981d` hoists `haz_timeseries_dir` out of the `run1` block so SKIP_R2_RUN1 doesn't crash section 4 (caught on the first Stage F attempt).
>   - **Status** — Stage F annual section 2 completed cleanly on the first AC re-bake run; annual sections 4 + 5.2 + all of jagermeyr remain TODO before STAGE C → D → E → cleanup → verify can flow. CR-068(a) + (c) will appear in the published parquet once that completes.
>
> **Net 2026-05-27**: (a) and (c) coded + waiting on rebake; (b) waiting on upstream `AdaptationAtlas/hazards`. Three sibling diagnostic runbooks remain in `hazards_prototype/scripts/` for post-rebake re-validation.
