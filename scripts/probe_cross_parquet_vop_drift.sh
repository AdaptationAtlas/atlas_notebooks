...
echo
echo "=== interpretation ==="
echo "  Pipeline map (hazards_prototype, per R/3 header + grep of consumers):"
echo "    R/1   — timeseries"
echo "    R/2   — haz frequency rasters (sec 5.2 adds hazard='none')"
echo "    R/3   — intersects R/2 haz frequencies with MapSPAM VoP"
echo "            rasters, zonal-aggregates to admin → the canonical"
echo "            hazard_exposure parquet (notebook's Crop & Livestock"
echo "            Exposure section)."
echo "  The sibling crop-livestock_all parquet (Key Facts denominator)"
echo "  aggregates the same MapSPAM VoP raster directly via the upstream"
echo "  exposure pipeline (R/0.6_process_exposure.R), bypassing R/3."
echo "  Cross-parquet drift therefore lives in R/3's zonal-extract"
echo "  + admin-aggregate path."
echo
echo "  (R/2.1 and R/2.2 are parallel siblings to R/3, not predecessors."
echo "   R/2.1 produces ensemble_season_timeseries.parquet which drives"
echo "   the notebook's Future Projections + Recent Changes; R/2.2"
echo "   produces ptot_change / thi_perc_area / ntx_perc_area which are"
echo "   published standalone via push_to_s3.R. Neither feeds R/3.)"
echo
echo "  Pattern A (scaling drift): median ratio offset from 1.0 but"
echo "                             low SD → MapSPAM versioning differs"
echo "                             between R/3's input VoP raster and"
echo "                             the exposure pipeline's, OR USD"
echo "                             inflation/deflator drift."
echo "  Pattern B (per-admin1):    high SD → mask alignment differs"
echo "                             between R/3's zonal extract and the"
echo "                             exposure pipeline's admin1 sums."
echo "  Pattern C (outlier crops): some crops OK, others systematic"
echo "                             → per-crop commodity-code audit in R/3."
echo "  Query C sanity:            sum_of_admin1 should equal a0_row"
echo "                             within 0.1%. Otherwise R/3's admin0"
