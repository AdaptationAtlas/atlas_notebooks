...
echo
echo "=== interpretation ==="
echo "  Pipeline map (hazards_prototype):"
echo "    R/2.1 — CMIP6 climate-timeseries tables (canonical:"
echo "            ensemble_season_timeseries.parquet → notebook's"
echo "            Future Projections + Recent Changes)"
echo "    R/2.2 — change/anomaly tables (ptot/thi/ntx, haz_freq)"
echo "    R/2   — haz frequency rasters (sec 5.2 adds hazard='none')"
echo "    R/3   — intersects R/2 haz frequencies with MapSPAM VoP"
echo "            rasters, zonal-aggregates to admin →"
echo "            hazard_exposure parquet (notebook's Crop & Livestock"
echo "            Exposure section)"
echo "  The sibling crop-livestock_all parquet aggregates the same"
echo "  MapSPAM VoP raster directly (no hazard step). Cross-parquet"
echo "  drift means R/3's pipeline produces a different admin1×crop"
echo "  VoP total than the standalone exposure pipeline does."
echo
echo "  Pattern A (scaling drift): median ratio offset from 1.0 but"
echo "                             low SD → likely MapSPAM versioning"
echo "                             differs between the two pipelines,"
echo "                             OR USD inflation/deflator drift."
echo "  Pattern B (per-admin1):    high SD → mask alignment differs."
echo "                             Check R/3's zonal extract against"
echo "                             the boundaries_zonal cache R/2.1"
echo "                             writes to boundaries_int_dir — if"
echo "                             both pipelines use that cache, the"
echo "                             mask alignment fix is upstream."
